...
proc/countJob(rank)
	var/jobCount = 0
	for(var/obj/item/card/id/O in available_ids)
		if(O.assignment == rank)
			jobCount++
		sleep(-1)
	return jobCount
...
/proc/dead_player_list(var/return_minds = 0, var/confirmation_spawn = 0, var/list/text_messages = list())
	...
	if (confirmation_spawn > 0)
		...
		text_chat_alert = "<span style=\"color:orange\"><h3>[strip_html(text_chat_alert, MAX_MESSAGE_LEN)]</h3></span>"
		text_chat_added = "<span style=\"color:orange\"><h3>[strip_html(text_chat_added, MAX_MESSAGE_LEN)]</h3></span>"
...
/proc/dectalk(msg)
	...
	if (world.timeofday > (lastDectalkUse + (nextDectalkDelay * 10)))
		...
		var/res[] = world.Export("http://cdn.spacetopia.pw/tts?words=[url_encode(msg)]")
		if (!res || !res["CONTENT"])
			return 0
...
/proc/get_selectedview(var/someone)
	if (!someone)
		return
	var/client/C = null
	if (isclient(someone))
	...
	if (!C || !C.preferences)
		return "800x600"
	return C.preferences.selectedview
