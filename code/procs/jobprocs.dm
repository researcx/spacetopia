	...
	for (var/mob/new_player/player in unassigned)
		...
		if(checktraitor(player))
			if ((ticker && ticker.mode && istype(ticker.mode, /datum/game_mode/revolution)) && J.cant_spawn_as_rev)
				...
				continue
			else if((ticker && ticker.mode && istype(ticker.mode, /datum/game_mode/gang)) && (job != "Civilian"))
				continue
...
/proc/DivideOccupations()
	...
	// This list is for jobs like Civilian which have no limits, or other special-case
	...
	spawn(0)
		...
		if (prob(10) && islist(random_pod_codes) && random_pod_codes.len)
			...
			if (V && V.lock && V.lock.code)
				boutput(src, "<span style=\"color:orange\">The unlock code to your pod ([V]) is: [V.lock.code]</span>")
				if (src.mind)
...
/mob/living/carbon/human/proc/spawnId(rank)
	...
	//boutput(src, "<span style=\"color:orange\">The PIN to your ID is: [C.pin]</span>")
	//if (src.mind)
	//	src.mind.store_memory("The PIN to your ID is: [C.pin]")
	if(getBalance(src.client.ckey) != 0)
		boutput(src, "<span style=\"color:orange\">You have &ETH;[getBalance(src.client.ckey)] in your Spacetopia bank account.</span>")
