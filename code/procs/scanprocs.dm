...
/proc/scan_health(var/mob/M as mob, var/verbose_reagent_info = 0, var/disease_detection = 1)
	...
	if (M.bodytemperature >= (optimal_temp + 60))
		...
		colored_temp = "<span style='color:orange'>[body_temp]</span>"
	else if (M.bodytemperature <= (optimal_temp - 30))
	...
	if (M.reagents)
		if (verbose_reagent_info)
			...
			if (total_amt)
				reagent_data = "<span style='color:orange'>Bloodstream Analysis located [total_amt] units of rejuvenation chemicals.</span>"
	...
	var/data = "--------------------------------<br>\
	Analyzing Results for <span style='color:orange'>[M]</span>:<br>\
...
/proc/scan_reagents(var/atom/A as turf|obj|mob, var/show_temp = 1, var/single_line = 0)
	...
	if (A.reagents)
		if (A.reagents.reagent_list.len > 0)
			...
			data = "<span style='color:orange'>[reagents_length] chemical agent[reagents_length > 1 ? "s" : ""] found in [A].</span>"
			...
			if (single_line)
				data += "<span style='color:orange'>[copytext(reagent_data, 1, -1)]</span>"
			else
				data += "<span style='color:orange'>[reagent_data]</span>"

			if (show_temp)
				data += "<br><span style='color:orange'>Overall temperature: [A.reagents.total_temperature - T0C]&deg;C ([A.reagents.total_temperature * 1.8-459.67]&deg;F)</span>"
		else
			data = "<span style='color:orange'>No active chemical agents found in [A].</span>"
	else
		data = "<span style='color:orange'>No significant chemical agents found in [A].</span>"
...
/proc/scan_forensic(var/atom/A as turf|obj|mob)
	...
	if (istype(A, /mob/living/carbon/human))
		...
		if (!isnull(H.gloves))
			...
			if (WG.glove_ID)
				glove_data += "[WG.glove_ID] (<span style='color:orange'>[H]'s worn [WG.name]</span>)"
			if (!WG.hide_prints)
				fingerprint_data += "<br><span style='color:orange'>[H]'s fingerprints:</span> [md5(H.bioHolder.Uid)]"
			else
				fingerprint_data += "<br><span style='color:orange'>Unable to scan [H]'s fingerprints.</span>"
		else
			fingerprint_data += "<br><span style='color:orange'>[H]'s fingerprints:</span> [md5(H.bioHolder.Uid)]"

		if (H.gunshot_residue) // Left by firing a kinetic gun.
			forensic_data += "<br><span style='color:orange'>Gunshot residue found.</span>"
		...
		if (H.implant && H.implant.len > 0)
			...
			for (var/obj/item/implant/I in H)
				if (istype(I, /obj/item/implant/projectile))
					wounds ++
			if (wounds)
				forensic_data += "<br><span style='color:orange'>[wounds] gunshot [wounds == 1 ? "wound" : "wounds"] detected.</span>"

		if (H.fingerprints) // Left by grabbing or pulling people.
			var/list/FFP = params2list(H:fingerprints)
			for(var/i in FFP)
				fingerprint_data += "<br><span style='color:orange'>Foreign fingerprint on [H]:</span> [i]"
		...
		if (H.bioHolder.Uid) // For quick reference. Also, attacking somebody only makes their clothes bloody, not the mob (think naked dudes).
			blood_data += "<br><span style='color:orange'>[H]'s blood DNA:</span> [H.bioHolder.Uid]"

		if (H.blood_DNA && isnull(H.gloves)) // Don't magically detect blood through worn gloves.
			var/list/BH = params2list(H:blood_DNA)
			for(var/i in BH)
				blood_data += "<br><span style='color:orange'>Blood on [H]'s hands:</span> [i]"
		...
		for (var/obj/item/check in gear_to_check)
			if (check && check.blood_DNA)
				...
				for(var/i in BC)
					blood_data += "<br><span style='color:orange'>Blood on worn [check.name]:</span> [i]"

		if (H.r_hand && H.r_hand.blood_DNA)
			var/list/BIR = params2list(H.r_hand.blood_DNA)
			for(var/i in BIR)
				blood_data += "<br><span style='color:orange'>Blood on held [H.r_hand.name]:</span> [i]"

		if (H.l_hand && H.l_hand.blood_DNA)
			var/list/BIL = params2list(H.l_hand.blood_DNA)
			for(var/i in BIL)
				blood_data += "<br><span style='color:orange'>Blood on held [H.l_hand.name]:</span> [i]"

	else

		if (!A.fingerprints)
			fingerprint_data += "<br><span style='color:orange'>Unable to locate any fingerprints.</span>"
		else
			var/list/FP = params2list(A:fingerprints)
			for(var/i in FP)
				fingerprint_data += "<br><span style='color:orange'>[i]</span>"

		if(!A.blood_DNA)
			blood_data += "<br><span style='color:orange'>Unable to locate any blood traces.</span>"
		else
			var/list/DNA = params2list(A:blood_DNA)
			for(var/i in DNA)
				blood_data += "<br><span style='color:orange'>[i]</span>"
		...
		if (istype(A, /obj/item/casing/))
			...
			if(C.forensic_ID)
				forensic_data += "<br><span style='color:orange'>Forensic profile of [C]:</span> [C.forensic_ID]"

		if (istype(A, /obj/item/implant/projectile))
			var/obj/item/implant/projectile/P = A
			if(P.forensic_ID)
				forensic_data += "<br><span style='color:orange'>Forensic profile of [P]:</span> [P.forensic_ID]"

		if (istype(A, /obj/item/gun))
			var/obj/item/gun/G = A
			if(G.forensic_ID)
				forensic_data += "<br><span style='color:orange'>Forensic profile of [G]:</span> [G.forensic_ID]"

		if (istype(A, /turf/simulated/wall))
			var/turf/simulated/wall/W = A
			if (W.forensic_impacts && islist(W.forensic_impacts) && W.forensic_impacts.len)
				for(var/i in W.forensic_impacts)
					forensic_data += "<br><span style='color:orange'>Forensic signature found:</span> [i]"
	...
	var/data = "--------------------------------<br>\
	<span style='color:orange'>Forensic analysis of <b>[A]</b></span> [contraband_data ? "[contraband_data]" : null]<br>\
	<br>\
	...
	[glove_data ? "<br><i>Material analysis:</i><span style='color:orange'> [glove_data]</span>" : null]\
	"
...
/proc/scan_atmospheric(var/atom/A as turf|obj, var/pda_readout = 0, var/simple_output = 0)
	...
	if (total_moles > 0)
		...
		if (pda_readout == 1) // Output goes into PDA interface, not the user's chatbox.
			...
			data = "--------------------------------<br>\
			<span style='color:orange'>Atmospheric analysis of <b>[A]</b></span><br>\
			<br>\
			...
			CO2: [round(co2_concentration * 100)]%<br>\
			Plasma: [round(plasma_concentration * 100)]%<br>\
			[unknown_concentration > 0.01 ? "</span><span style='color:red'>Unknown: [round(unknown_concentration * 100)]%</span><span style='color:orange'><br>" : ""]\
			Temperature: [round(check_me.temperature - T0C)]&deg;C<br>"
