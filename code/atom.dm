...
/atom
	...
	var/list/fingerprintshidden = null
...
// false if closed
	...
	proc/transfer_all_reagents(var/atom/A as turf|obj|mob, var/mob/user as mob)
		...
		if ((A.reagents.total_volume + src.reagents.total_volume) > A.reagents.maximum_volume) // Check to make sure that both containers content's combined won't overfill the destination container.
			...
			boutput(user, "<span style=\"color:orange\">You transfer [T] units into [A].</span>") // Tell the user they did a thing.
			return
		else
			...
			boutput(user, "<span style=\"color:orange\">You transfer [T] units into [A].</span>")
			return
...
/atom/MouseEntered(location, control, params)
	//world.log << "[location] / [control] / [params]"
	if(usr.client.examinemode)
		...
		if (output)
			usr.client.tooltip.show(src, params, title = src.name, content = output)
	..()
...
/atom/movable/verb/pull()
	...
	if (istype(src,/obj/item/old_grenade/light_gimmick))
		boutput(usr, "<span style=\"color:orange\">You feel your hand reach out and clasp the grenade.</span>")
		src.attack_hand(usr)
		return
	if (!( src.anchored ))
		usr.pulling = src
...
/atom/proc/add_fingerprint(mob/living/M as mob)
	...
	if(!islist(src.fingerprintshidden))
		src.fingerprintshidden = list() // Only initialize fingerprintshidden when we actually need to do so.
