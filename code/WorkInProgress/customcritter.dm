...
"Thunder" = 'sound/effects/thunder.ogg')
	...
	Topic(href, href_list)
		if (href_list["name"])
			...
			boutput(usr, "<span style=\"color:orange\">Critter current state saved as [template.name]</span>")
			...
			if (critter_creator_controller.activeCritterTypes.len)
				...
				template = CR.clone()
				boutput(usr, "<span style=\"color:orange\">Loaded [template.name].</span>")
			else
...
.active { font-weight: bold; }
		"}
		output += "</style>[css_interfaces]</head><body>"
