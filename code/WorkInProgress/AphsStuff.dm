...
/obj/machinery/vending/pizza
	...
	attack_hand(mob/user as mob)
		...
		user << browse("<HEAD><TITLE>Pizza Vendor</TITLE>[css_interfaces]</head>[dat]", "window=pizzavend")
	...
	attackby(obj/item/P as obj, mob/user as mob)
		if (istype(P, /obj/item/mars_roverpart))
			...
			if((wheel)&&(oxy)&&(battery)&&(glass)&&(motherboard))
				...
				boutput(user, "<span style=\"color:orange\">The rover has been completed!</span>")
				qdel(src)
...
/obj/machinery/computer/mars_vault
	...
	attack_hand(mob/user as mob)
		...
		user << browse("<head><title>Vault Computer</title>[css_interfaces]</head>[dat]", "window=tourconsole;size=302x245")
