	...
	proc/gather_listeners()
		...
		for (var/mob/living/carbon/human/H in world)
			for (var/obj/item/device/radio/Hs in H)
				if (Hs.frequency == frequency)
					listeners += H
					boutput(H, "<span style=\"color:orange\">A peculiar noise intrudes upon the radio frequency of your [Hs].</span>")
				break
		for (var/mob/living/silicon/robot/R in world)
			if (istype(R.radio, /obj/item/device/radio))
				...
				if (Hs.frequency == frequency)
					listeners += R
					boutput(R, "<span style=\"color:orange\">A peculiar noise intrudes upon your radio frequency.</span>")
