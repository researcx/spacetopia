...
/obj/rack/precursor
	...
	/*	if (istype(W,/obj/item/skull)) // placeholder
			...
			src.visible_message("<span style=\"color:orange\"><b>Something activates inside [src]!</b></span>")
...
/obj/precursor_puzzle/orb_stand
	...
	attack_hand(mob/user as mob)
		...
		if (!src.assembled)
			boutput(user, "<span style=\"color:orange\">[src] is missing something.</span>")
			return

		if (!src.ready)
			boutput(user, "<span style=\"color:orange\">[src] isn't ready yet.</span>")
			return
	...
	attackby(obj/item/W as obj, mob/user as mob)
		...
		if(istype(W, /obj/item/chilly_orb))
			...
			if(O.id == src.id)
				boutput(user, "<span style=\"color:orange\"><b>[O] attaches neatly to [src]. Oh dear.</span>")
				...
				src.assembled = 1
				sleep(5)
				src.visible_message("<span style=\"color:orange\"><b>[src] makes a strange noise!</b></span>")
				playsound(src.loc, "sound/machines/ArtifactPre1.ogg", 60, 1)
				src.ready = 1
				return
			else
				boutput(user, "<span style=\"color:orange\"><b>[src] don't seem to quite fit together with [O].</span>")
			...
			user.visible_message("<span style=\"color:orange\"><b>[user] slams [W] down onto [src]'s central spike.</b></span>")
			sleep(1)
...
/obj/precursor_puzzle/glowing_door
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if(istype(W, /obj/item/hell_sax) && !src.opened)
			..()
			user.visible_message("<span style=\"color:orange\"><B>[src] [pick("rings", "dings", "chimes","vibrates","oscillates")] [pick("faintly", "softly", "loudly", "weirdly", "scarily", "eerily")].</B></span>")
...
/obj/precursor_puzzle/rotator
	...
	attack_hand(mob/user as mob)
		...
		src.visible_message("<span style=\"color:orange\"><b>[user] turns [src].</b></span>")
		...
		update()
			...
			if(src.linked_shields.len)
				if(setting_red == target_red)
					src.visible_message("<span style=\"color:orange\"><b>[src]</b> beeps oddly.</span>")
					playsound(src.loc,"sound/machines/twobeep.ogg",50,1)
				sleep(2)
				if(setting_green == target_green)
					src.visible_message("<span style=\"color:orange\"><b>[src]</b> beeps strangely.</span>")
					playsound(src.loc,"sound/machines/twobeep.ogg",50,1)
				sleep(2)
				if(setting_blue == target_blue)
					src.visible_message("<span style=\"color:orange\"><b>[src] beeps curiously.</span>")
					playsound(src.loc,"sound/machines/twobeep.ogg",50,1)
				sleep(2)
	...
	proc
		...
		activate()
			...
			if(!src.active)
				...
				src.visible_message("<span style=\"color:orange\"><b>[src] powers up!</b></span>")
				light.enable()
		...
		deactivate()
			...
			if(src.active)
				...
				src.visible_message("<span style=\"color:orange\"><b>[src] powers down!</b></span>")
				changing_state = 1
				light.disable()
...
/obj/portrait_sneaky
	...
	attack_hand(var/mob/user as mob)
		boutput(user, "<span style=\"color:orange\"><b>You try to straighten [src], but it won't quite budge.</b></span>")
		..()
		return
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if(istype(W, /obj/item/crowbar))
			...
			boutput(user, "<span style=\"color:orange\"><b>You pry [src] off the wall, destroying it! You jerk!</b></span>")
