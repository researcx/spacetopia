...
/obj/machinery/sim/transmitter/proc/interact(mob/user)
	...
	user.machine = src
	var/dat = "<HEAD><TITLE>V-space Computer</TITLE><META HTTP-EQUIV='Refresh' CONTENT='10'>[css_interfaces]</head><BODY><br>"
...
/obj/machinery/sim/chair/MouseDrop_T(mob/M as mob, mob/user as mob)
	...
	if (M == usr)
		user.visible_message("<span style=\"color:orange\">[user] buckles in!</span>")
	else
		M.visible_message("<span style=\"color:orange\">[M] is buckled in by [user]!</span>")
...
/obj/machinery/sim/chair/attack_hand(mob/user as mob)
	if (src.con_user)
		...
		if (M != user)
			M.visible_message("<span style=\"color:orange\">[M] is unbuckled by [user].</span>")
		else
			M.visible_message("<span style=\"color:orange\">[M] is unbuckles.</span>")
...
/obj/machinery/sim/vr_bed/attackby(obj/item/O as obj, mob/user as mob)
	if(istype(O,/obj/item/grab))
		...
		if (!ismob(G.affecting))
			return
		if (src.occupant)
			boutput(user, "<span style=\"color:orange\"><B>The VR pod is already occupied!</B></span>")
			return
		if(..())
			return
...
/obj/machinery/sim/vr_bed/proc/log_in(mob/M as mob)
	if (src.occupant)
		if(M == src.occupant)
			return src.go_out()
		boutput(M, "<span style=\"color:orange\"><B>The VR pod is already occupied!</B></span>")
		return

	if (!iscarbon(M))
		boutput(M, "<span style=\"color:orange\"><B>You cannot possibly fit into that!</B></span>")
		return
