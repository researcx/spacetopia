...
/obj/item/electronics/frame/attackby(obj/item/W as obj, mob/user as mob)
	if(istype(W,/obj/item/electronics/))
		...
		if(!(istype(E,/obj/item/electronics/disk)||istype(E,/obj/item/electronics/scanner)||istype(E,/obj/item/electronics/soldering)||istype(E,/obj/item/electronics/frame)))
			...
			user.u_equip(E)
			//parts.Add(E)
			boutput(user, "<span style=\"color:orange\">You add the [E.name] to the [src].</span>")
			return
		else if(istype(E,/obj/item/electronics/soldering))
			if(!secured)
				secured = 1
				viewstat = 1
				boutput(user, "<span style=\"color:orange\">You secure the [src].</span>")
			else if(secured == 1)
				secured = 0
				viewstat = 0
				boutput(user, "<span style=\"color:orange\">You unsecure the [src].</span>")
			else if(secured == 2)
				secured = 0
...
/obj/item/electronics/frame/MouseDrop_T(atom/movable/O as obj, mob/user as mob)
	...
	if (!src.secured)
		...
		user.visible_message("<span style=\"color:orange\">[user] begins quickly adding components to [src]!</span>", "<span style=\"color:orange\">You begin to quickly add components to [src]!</span>")
		...
		boutput(user, "<span style=\"color:orange\">You finish adding components to [src]!</span>")
	else
	...
	switch(viewstat)
		...
		if(0)
			for(var/obj/item/electronics/P in src.contents)
				...
				user.machine = src
				user << browse("<HEAD><TITLE>Frame</TITLE>[css_interfaces]</head><TT>[dat]</TT>", "window=fkit")
				onclose(user, "fkit")
...
/obj/item/electronics/scanner/afterattack(var/obj/O, mob/user as mob)
	...
	if(istype(O,/obj/))
		...
		boutput(user, "<span style=\"color:orange\">Item scan successful.</span>")
...
/obj/machinery/rkit/attackby(obj/item/W as obj, mob/user as mob)
	...
	if(istype(W,/obj/item/electronics/scanner))
		...
		if (add_count > 0)
			boutput(user, "<span style=\"color:orange\">[add_count] new items entered into kit.</span>")
		else
...
/obj/machinery/rkit/attack_hand(mob/user as mob)
	...
	user.machine = src
	user << browse("<HEAD><TITLE>Ruckingenur Kit Control Panel</TITLE>[css_interfaces]</head><TT>[dat]</TT>", "window=rkit")
	onclose(user, "rkit")
