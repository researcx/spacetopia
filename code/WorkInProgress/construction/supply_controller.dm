...
/obj/supply_pad
	...
	examine()
		..()
		boutput(usr, "<span style=\"color:orange\">The pad is currently at [charge]% charge.</span>")
		if (has_crystal)
			boutput(usr, "<span style=\"color:orange\">The pad is complete with a telecrystal.</span>")
		else
	...
	attackby(var/obj/item/I as obj, user as mob)
		if (istype(I, /obj/item/raw_material/telecrystal))
			qdel(I)
			has_crystal++
			boutput(user, "<span style=\"color:orange\">You plug the telecrystal into the teleportation pad.</span>")
						...
						for (var/obj/item/Q in CR)
							if (!istype(Q))
								...
								for (var/mob/M in viewers(Q))
									boutput(M, "<span style=\"color:orange\">[Q] pops out of [CR]!</span>")
							else
								profit += do_sell(Q)
								qdel(Q)
						profit += do_sell(CR)
...
</style>[css_interfaces]</head><body>"}
