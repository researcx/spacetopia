...
/datum/griffening_controller
	...
	proc/update_game_status(var/message)
		for (var/mob/M in players)
			boutput(M, "<span style=\"color:orange\">[message]</span>")
...
.answer {
	...
	margin-left: 5px;
		...
		addinfo("<html><head><title>Spacemen the Grifening</title><style>[stylesheet]</style>[css_interfaces]</head><body>")
...
/obj/griffening_card_holder
	...
	Click(location, control, params)
		...
		if (!controller.players[player])
			...
			boutput(M, "<span style=\"color:orange\">You enter the game as player [player]. The game begins when both players joined. Leaving the playing area (the row in front of your card holders) or becoming braindead for more than 15 seconds will automatically forfeit the game.</span>")
		else
