...
/obj/item/potion
	...
	afterattack(atom/target as mob|obj|turf|area, mob/user as mob)
		...
		if (user == target)
			user.visible_message("<span style=\"color:orange\">[user] uncorks the potion and pours it down \his throat.</span>")
...
/obj/wizard_light
	...
	attackby(var/obj/item/W, var/mob/user)
		if (istype(W, /obj/item/wizard_crystal))
			if (!src.crystal)
				boutput(user, "<span style=\"color:orange\">You place the crystal into the socket.</span>")
				crystal = W
				user.u_equip(W)
				W.loc = src
