					...
					spawn(0)
						if(src && selected)
							...
							for (var/mob/O in observersviewers(7, src))
								O.show_message("<B><span style=\"color:orange\">The board spells out a message ... \"[selected]\"</span></B>", 1)
			else
				usr.show_text("Please wait a moment before using the board again.", "red")
		else
	...
	attack_hand(mob/user as mob)
		...
		spawn(0)
			...
			for(var/mob/O in oviewers(world.view, src))
				O.show_message("<span style=\"color:orange\">The box of fireworks magically disappears.</span>", 1)
...
/obj/item/alchemy/stone
	...
	New()
		..()
		src.visible_message("<span style=\"color:orange\"><b>[src] appears out of thin air!</b></span>")
...
/obj/item/alchemy/powder
	...
	afterattack(atom/target as mob|obj|turf|area, mob/user as mob)
		...
		boutput(user, "<span style=\"color:orange\">Your sprinkle some powder on \the [target].</span>")
...
/obj/alchemy/empty
	...
	attack_hand(mob/user as mob)
		if(symbol != null)
			...
			symbol = null
			overlays.Cut()
			boutput(usr, "<span style=\"color:orange\">You remove the Symbol.</span>")
		return
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if(istype(W,/obj/item/alchemy/symbol) && symbol == null)
			...
			symbol.loc = src
			overlays += symbol
			boutput(usr, "<span style=\"color:orange\">You put the Symbol in the Circle.</span>")
		return
	...
	proc/activate()
		...
		if((north.symbol && istype(north.symbol,north.requiredType)) && (southwest.symbol && istype(southwest.symbol,southwest.requiredType)) && (southeast.symbol && istype(southeast.symbol,southeast.requiredType)))
			...
			if(blood == 1)
				...
				boutput(usr, "<span style=\"color:orange\">The Circle glows faintly before returning to normal. Maybe something is missing.</span>")
			return
		else
...
/datum/engibox_mode/setid
	...
	used(atom/user, atom/target)
		if(hasvar(target,"id"))
			target:id = saved_var
			boutput(usr, "<span style=\"color:orange\">Done.</span>")
		else
...
/datum/engibox_mode/replicate
	...
	used(atom/user, atom/target)
		if(obj_path)
			...
			boutput(usr, "<span style=\"color:orange\">Placed: [A.name]</span>")
		else
			obj_path = target.type
			boutput(usr, "<span style=\"color:orange\">Now replicating: [target.name]s</span>")
		return
...
/datum/engibox_mode/density
	...
	used(atom/user, atom/target)
		...
		boutput(usr, "<span style=\"color:orange\">Target density now: [target.density]</span>")
		return
...
/datum/engibox_mode/opacity
	...
	used(atom/user, atom/target)
		...
		boutput(usr, "<span style=\"color:orange\">Target opacity now: [target.opacity]</span>")
		return
