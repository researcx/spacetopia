...
datum/pathogeneffects/malevolent/serious_paranoia
	...
	proc/backpack(var/mob/M, var/mob/living/O)
		...
		boutput(M, "<span style=\"color:orange\">[O] has added the [item] to the backpack!</span>")
				...
				if (prob(12))
					for (var/mob/living/O in oview(7, M))
						if (prob(50))
							...
							boutput(M, "<span style=\"color:orange\">[O] has added the suspicious item to the backpack!</span>")
						return
...
datum/pathogeneffects/malevolent/capacitor
	...
	proc/discharge(var/mob/M as mob, var/datum/pathogen/origin)
		...
		if (load > 4e6)
			...
			M.show_message("<span style=\"color:orange\">You feel discharged.</span>")
	...
	onshocked(var/mob/M as mob, var/datum/shockparam/ret, var/datum/pathogen/origin)
		...
		if (wattage > 45000)
			...
			amt /= 2
			ret.skipsupp = 1
			M.show_message("<span style=\"color:orange\">You absorb a portion of the electric shock!</span>")
		else
			amt = 0
			ret.skipsupp = 1
			M.show_message("<span style=\"color:orange\">You absorb the electric shock!</span>")
		load_check(M, origin)
		return ret
	...
	ondisarm(var/mob/M as mob, var/mob/V as mob, isPushDown, var/datum/pathogen/origin)
		...
		if (load > 1e6 && isPushDown)
			if (prob(25))
				M.visible_message("<span style=\"color:red\">[M]'s hands are glowing in a blue color.</span>", "<span style=\"color:orange\">You discharge yourself onto your opponent with your hands!</span>", "<span style=\"color:red\">You hear someone getting defibrillated.</span>")
				electrocute(V, load / 10)
				if (prob(50))
	...
	onpunch(var/mob/M as mob, var/mob/V as mob, zone, var/datum/pathogen/origin)
		...
		if (load > 2e6)
			if (prob(25))
				M.visible_message("<span style=\"color:red\">[M]'s fists are covered in electric arcs.</span>", "<span style=\"color:orange\">You supercharge your punch.</span>", "<span style=\"color:red\">You hear a huge electric crackle.</span>")
				electrocute(V, load / 10)
				if (prob(50))
					M.show_message("<span style=\"color:red\">Your shock jumps back onto you!</span>")
					electrocute(M, load / 10)
			...
			if (prob(20))
				M.visible_message("<span style=\"color:red\">[M]'s fists spark electric arcs.</span>", "<span style=\"color:orange\">You overcharge your punch.</span>", "<span style=\"color:red\">You hear a large electric crackle.</span>")
				electrocute(V, load / 10)
				if (prob(50))
					M.show_message("<span style=\"color:red\">Your shock jumps back onto you!</span>")
					electrocute(M, load / 10)
			...
			if (prob(15))
				M.visible_message("<span style=\"color:red\">[M]'s fists throw sparks.</span>", "<span style=\"color:orange\">You charge your punch.</span>", "<span style=\"color:red\">You hear an electric crackle.</span>")
				electrocute(V, load / 10)
				if (prob(50))
					M.show_message("<span style=\"color:red\">Your shock jumps back onto you!</span>")
					electrocute(M, load / 10)
				...
				if (prob(9))
					...
					if (C && PN.avail > 0)
						...
						if (amt > 2500 && load > 5000)
							M.show_message("<span style=\"color:orange\">You feel energized.</span>")
						load_check(M, origin)
				else if (prob(6))
					if (load > 0)
						discharge(M, origin)
...
datum/pathogeneffects/malevolent/sunglass
	...
	proc/glasses(var/mob/living/carbon/human/M as mob)
		M.show_message("<span style=\"color:orange\">[pick("You feel cooler!", "You find yourself wearing sunglasses.", "A pair of sunglasses grow onto your face.")]</span>")
