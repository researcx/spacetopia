...
/obj/item/reagent_containers/glass/petridish
	...
	examine()
		...
		if (src.reagents.reagent_list["pathogen"])
			...
			boutput(usr, "<span style=\"color:orange\">It contains [P.volume] units of harvestable pathogen.</span>")
		if (src.medium)
			boutput(usr, "<span style=\"color:orange\">The petri dish is coated with [src.medium.name].</span>")
		boutput(usr, "Nutrients in the dish:")
		var/count = 0
		for (var/N in nutrition)
			if (nutrition[N])
				if (nutrition[N] != 1)
					boutput(usr, "<span style=\"color:orange\">[nutrition[N]] units of [N]</span>")
				else
					boutput(usr, "<span style=\"color:orange\">[nutrition[N]] unit of [N]</span>")
				count++
		if (!count)
			boutput(usr, "<span style=\"color:orange\">None.</span>")
