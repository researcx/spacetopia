...
/obj/machinery/disposal
	...
	proc/interact(mob/user, var/ai=0)
		...
		var/dat = "<head><title>Waste Disposal Unit</title>[css_interfaces]</head><body><TT><B>Waste Disposal Unit</B><HR>"

		if(flush)
			dat += "<B>Disposal handle:</B> <i>Engaged</i><br><A href='?src=\ref[src];handle=0' class='toggle-off'>Disengage</A>"
		else
			dat += "<B>Disposal handle:</B> <i>Disengaged</i><br><A href='?src=\ref[src];handle=1' class='toggle-on'>Engage</A>"
		...
		user.machine = src
		user << browse(dat, "window=disposal;size=280x400")
