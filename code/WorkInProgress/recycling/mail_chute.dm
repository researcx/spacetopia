...
/obj/machinery/disposal/mail
	...
	interact(mob/user, var/ai=0)
		...
		var/dat = "<head><title>Mail Transport Unit</title>[css_interfaces]</head><body><TT><B>Mail Transport Unit: [src.mail_tag ? (capitalize(src.mail_tag)) : "GENERIC"]</B><HR>"

		if(flush)
			dat += "<B>Shipping handle:</B><br><I>Engaged</i><br><A href='?src=\ref[src];handle=0' class='toggle-off'>Disengage</A>"
		else
			dat += "<B>Shipping handle:</B><br><i>Disengaged</i><br><A href='?src=\ref[src];handle=1' class='toggle-on'>Engage</A>"

		dat += "<BR><HR><A href='?src=\ref[src];eject=1'>Eject contents</A><HR>"

		if(mode == 0)
			dat += "Pump<br><A href='?src=\ref[src];pump=1' class='toggle-on'>On</A><BR>"
		else if(mode == 1)
			dat += "Pump<br><A href='?src=\ref[src];pump=0' class='toggle-off'>Off</A><BR>"
		else
			dat += "Pump<br><A href='?src=\ref[src];pump=0' class='toggle-off'>Off</A><BR>"
		...
		user.machine = src
		user << browse(dat, "window=mailchute;size=280x400")
