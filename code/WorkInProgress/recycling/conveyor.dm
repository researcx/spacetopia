...
/obj/machinery/conveyor/attackby(var/obj/item/I, mob/user)
	if(istype(I, /obj/item/grab))	// special handling if grabbing a mob
		...
		if(M)
			if (M == user)
				src.visible_message("<span style=\"color:orange\">[M] ties \himself to the conveyor.</span>")
				// note don't check for lying if self-tying
			else
				if(M.lying)
					user.visible_message("<span style=\"color:orange\">[M] has been tied to the conveyor by [user].</span>", "<span style=\"color:orange\">You tie [M] to the converyor!</span>")
				else
					boutput(user, "<span style=\"color:orange\">[M] must be lying down to be tied to the converyor!</span>")
					return
			M.buckled = src
		...
		if(M && M.buckled == src)
			...
			src.add_fingerprint(user)
			if (M == user)
				src.visible_message("<span style=\"color:orange\">[M] cuts \himself free from the conveyor.</span>")
			else
				src.visible_message("<span style=\"color:orange\">[M] had been cut free from the conveyor by [user].</span>")
			return
