...
/obj/item/camera_test/haunted
	...
	afterattack(atom/target as mob|obj|turf|area, mob/user as mob, flag)
		...
		boutput(user, "<span style=\"color:orange\">[pictures_left] photos left.</span>")
		can_use = 0
		spawn(50) can_use = 1
*/
	...
	attack_hand(mob/user as mob)
		...
		if (src.booby_trapped)
			...
			if (dangerous)
				...
				user.visible_message("<span style=\"color:orange\"><b>[user.name]</b> takes [gift] out of [src]!</span>", "<span style=\"color:orange\">You take [gift] out of [src]!</span>")
		return
	...
	process()
		...
		if (host.lastattacker && (host.lastattackertime + 40) >= world.time)
			if(host.lastattacker != host)
				...
				boutput(M, "<span style=\"color:orange\">You are caught in a magical warp field!</span>")
...
/obj/machinery/bot/guardbot/future
	...
	interact(mob/user as mob)
		var/dat = "[css_interfaces]<TT><B>PR-7 Robuddy v2.9</B></tt><br><br>"
		...
		user << browse("<head><title>Robuddy v2.9 controls</title>[css_interfaces]</head>[dat]", "window=guardbot")
