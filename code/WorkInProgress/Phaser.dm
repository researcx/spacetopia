...
/obj/item/oldgun/energy/phaser
	...
	verb/remove_mods()
		...
		if(!src.contents.len)
			...
			return
		boutput(usr, "<span style=\"color:orange\">You remove all mods.</span>")
	...
	examine()
		set src in view()
		if(src.contents.len)
			boutput(usr, "<span style=\"color:orange\">Installed mods:</span>")
			boutput(usr, "")
			for(var/obj/O in src)
				boutput(usr, "<span style=\"color:orange\">[O.name]</span>")
				boutput(usr, "<span style=\"color:orange\">[O.desc]</span>")
				boutput(usr, "")
		..()
			...
			switch(G.location)
				if("ext")
					if(extension_mod)
						...
						user.drop_item()
						boutput(user, "<span style=\"color:orange\">You install the gun extension.</span>")
				...
				if("conv")
					if(converter_mod)
						...
						user.drop_item()
						boutput(user, "<span style=\"color:orange\">You install the energy-converter mod.</span>")
						playsound(user, "sound/items/Screwdriver2.ogg", 65, 1)
						G.set_loc(src)
						converter_mod = G
						generate_overlays()
