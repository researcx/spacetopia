	...
	Topic(href, href_list)
		...
		if(href_list["key"])
			if(istype(usr.equipped(), /obj/item/device/key/cheget) && !src.inserted_key)
				...
				C.set_loc(src)
				src.inserted_key = C
				boutput(usr, "<span style=\"color:orange\">You insert the key and turn it.</span>")
			...
			if(src.inserted_key)
				boutput(usr, "<span style=\"color:orange\">You turn the key and pull it out of the lock. The green light turns off.</span>")
...
/obj/iomoon_puzzle/ancient_robot_door/meat
	...
	close()
		...
		for (var/mob/living/L in src.loc)
			if (prob(10))
				boutput(L, "<span style=\"color:orange\">You just barely slip by the clenching teeth unharmed!</span>")
			else if (prob(80))
