...
#define BONE_//DEBUG(x) if (bone_system) message_coders(x)
...
/datum/bone
	...
	proc/take_damage(var/damage_type, var/amt = 1)
		...
		if (!donor || !src.parent_organ) // I can't see a reason we'd still need to damage this thing if it's just kinda floating in the void somewhere
			BONE_//DEBUG("a bone datum lacks a donor or parent_organ so take_damage() was canceled, oops")
			return 0 // ghostbones

		BONE_//DEBUG("[donor]'s [parent_organ]'s bones.take_damage() entered")
		...
		BONE_//DEBUG("[donor]'s [parent_organ]'s bones.take_damage() amt: [amt], damage_type: [dam_num2name(damage_type)], damtype_modifier: [damtype_modifier], initial bone damage: [src.damage], damage changing to [src.damage + (amt * damtype_modifier)], initial damage_status: [bone_num2name(src.damage_status)]")
		...
		if (prob((10 + src.damage) * damtype_modifier))
			...
			BONE_//DEBUG("[donor]'s [parent_organ]'s bones rolled increase in damage level at prob (20 + [src.damage]) * [damtype_modifier] = [(20 + src.damage) * damtype_modifier]")

		switch (src.damage_status)
			if (BONE_HEALTHY)
				BONE_//DEBUG("[donor]'s [parent_organ]'s bones bruised")
				src.damage_status = BONE_BRUISED
				src.donor.show_text("Your [src.parent_organ] hurts!", "red")

			if (BONE_BRUISED)
				BONE_//DEBUG("[donor]'s [parent_organ]'s bones cracked")
				src.damage_status = BONE_CRACKED
				src.donor.show_text("Your [src.parent_organ] hurts like hell!", "red")

			if (BONE_CRACKED)
				BONE_//DEBUG("[donor]'s [parent_organ]'s bones fractured")
			...
			if (BONE_FRACTURED)
				BONE_//DEBUG("[donor]'s [parent_organ]'s bones shattered")
