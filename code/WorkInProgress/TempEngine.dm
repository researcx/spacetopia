...
/obj/machinery/power/generatorTemp
	...
	New()
		...
		spawn(5)
			circ1 = locate(/obj/machinery/atmospherics/binary/circulatorTemp) in get_step(src,EAST)
			circ2 = locate(/obj/machinery/atmospherics/binary/circulatorTemp/right) in get_step(src,WEST)
			if(!circ1 || !circ2)
				stat |= BROKEN
