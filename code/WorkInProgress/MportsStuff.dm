...
/obj/infared_icon/examine()
	set src in view()
	if(usr.see_infrared)
		usr << browse(text("<HTML><HEAD><TITLE>[]</TITLE>[css_interfaces]</head><BODY><TT>[]</TT></BODY></HTML>", src.name, src.info), text("window=[]", src.name))
...
//I still need a way for the AI to actually make these
	...
	switch(select)
		...
		if("View")
			if(usr.see_infrared)
				user << browse(text("<HTML><HEAD><TITLE>[]</TITLE>[css_interfaces]</head><BODY><TT>[]</TT></BODY></HTML>", src.name, src.info), text("window=[]", src.name))
				onclose(usr, "[src.name]")
	return
