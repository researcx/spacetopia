...
/obj/item/playing_cards
	...
	MouseDrop(var/atom/target as obj|mob)
		...
		if (usr.a_intent == INTENT_GRAB && src.cards.len > 1)
			usr.visible_message("<span style=\"color:orange\"><b>[usr]</b> looks through [src].</span>",\
			"<span style=\"color:orange\">You look through [src].</span>")
			deal_face_up = 0
			var/list/availableCards = list()
			for (var/datum/playing_card/listCard in src.cards)
				availableCards += "[listCard.card_name]"
			boutput(usr, "<span style=\"color:orange\">What card would you like to deal from [src]?</span>")
		...
		if (src.cards.len == 1)
			if (target == src && src.card_tappable)
				if (src.card_tapped)
					usr.visible_message("<span style=\"color:orange\"><b>[usr]</b> untaps [src].</span>",\
					"<span style=\"color:orange\">You untap [src].</span>")
					src.card_tapped = null
					src.update_cards()
				else
					usr.visible_message("<span style=\"color:orange\"><b>[usr]</b> taps [src].</span>",\
					"<span style=\"color:orange\">You tap [src].</span>")
					src.card_tapped = pick(EAST, WEST)
					src.update_cards()
			else if (ismob(target))
				usr.tri_message("<span style=\"color:orange\"><b>[usr]</b> takes [stupid_var][usr == target ? "." : " and deals it to [target]."]</span>",\
				usr, "<span style=\"color:orange\">You take [stupid_var][usr == target ? "." : " and deal it to [target]."]</span>", \
				target, "<span style=\"color:orange\">[target == usr ? "You take" : "<b>[usr]</b> takes"] [stupid_var][target == usr ? "." : " and deals it to you."]</span>")
				src.draw_card(null, target, deal_face_up, Card)
			else if (istype(target, /obj/table))
				usr.visible_message("<span style=\"color:orange\"><b>[usr]</b> takes [stupid_var] and places it on [target].</span>",\
				"<span style=\"color:orange\">You take [stupid_var] and place it on [target].</span>")
				src.draw_card(null, target, deal_face_up, Card)
			else if (istype(target, /obj/item/playing_cards))
				usr.visible_message("<span style=\"color:orange\"><b>[usr]</b> takes [stupid_var] and adds it to [target].</span>",\
				"<span style=\"color:orange\">You take [stupid_var] and add it to [target].</span>")
			...
			if (ismob(target))
				usr.tri_message("<span style=\"color:orange\"><b>[usr]</b> draws [other_stupid_var] from [src][usr == target ? "." : " and deals it to [target]."]</span>",\
				usr, "<span style=\"color:orange\">You draw [other_stupid_var] from [src][usr == target ? "." : " and deal it to [target]."]</span>", \
				target, "<span style=\"color:orange\">[target == usr ? "You draw" : "<b>[usr]</b> draws"] a card from [src][target == usr ? "." : " and deals it to you."]</span>")
				src.draw_card(null, target, deal_face_up, Card)
			else if (istype(target, /obj/table))
				usr.visible_message("<span style=\"color:orange\"><b>[usr]</b> draws [other_stupid_var] from [src] and places it on [target].</span>",\
				"<span style=\"color:orange\">You draw [other_stupid_var] from [src] and place it on [target].[other_stupid_var]</span>")
				src.draw_card(null, target, deal_face_up, Card)
			else if (istype(target, /obj/item/playing_cards))
				usr.visible_message("<span style=\"color:orange\"><b>[usr]</b> draws [other_stupid_var] from [src] and adds it to [target].</span>",\
				"<span style=\"color:orange\">You draw [other_stupid_var] from [src] and add it to [target].</span>")
				src.draw_card(target, null, deal_face_up, Card)
			else
		...
		if (get_dist(user, src) <= 0 && src.cards.len)
			if (user.l_hand == src || user.r_hand == src)
				...
				if (user.a_intent == INTENT_GRAB && src.cards.len > 1)
					user.visible_message("<span style=\"color:orange\"><b>[user]</b> looks through [src].</span>",\
					"<span style=\"color:orange\">You look through [src].</span>")
					var/list/availableCards = list()
					for (var/datum/playing_card/Card in src.cards)
						availableCards += "[Card.card_name]"
					boutput(user, "<span style=\"color:orange\">What card would you like to draw from [src]?</span>")
					...
					if (!cardToGive)
						return
					user.visible_message("<span style=\"color:orange\"><b>[usr]</b> draws a card from [src].</span>",\
					"<span style=\"color:orange\">You draw \an [chosenCard] from [src].</span>")
					src.draw_card(null, user, draw_face_up, cardToGive)
				else
					var/datum/playing_card/Card = src.cards[1]
					user.visible_message("<span style=\"color:orange\"><b>[usr]</b> draws [draw_face_up ? "\an [Card.card_name]" : "a card"] from [src].</span>",\
					"<span style=\"color:orange\">You draw [draw_face_up ? "\an [Card.card_name]" : "a card"] from [src].</span>")
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if (istype(W, /obj/item/playing_cards))
			...
			src.add_cards(C)
			user.visible_message("<span style=\"color:orange\"><b>[user]</b> adds [C] to the bottom of [src].</span>",\
			"<span style=\"color:orange\">You add [C] to the bottom of [src].</span>")
		else return ..()
	...
	attack_self(mob/user as mob)
		...
		switch (src.cards.len)
			if (1)
				...
				src.update_cards()
				user.visible_message("<span style=\"color:orange\"><b>[user]</b> flips the card [src.face_up ? "face up. It's \an [src.name]." : "face down."]</span>",\
				"<span style=\"color:orange\">You flip the card [src.face_up ? "face up. It's \an [src.name]." : "face down."]</span>")
			...
			if (2 to 10)
				...
				user.visible_message("<span style=\"color:orange\"><b>[user]</b> shows their hand: [final_shown_hand].</span>",\
				"<span style=\"color:orange\">You show your hand: [final_shown_hand].</span>")
				src.last_shown_off = world.time
			if (11 to INFINITY)
				...
				user.visible_message("<span style=\"color:orange\"><b>[user]</b> shuffles [src].</span>",\
				"<span style=\"color:orange\">You shuffle [src].</span>")
				src.last_shown_off = world.time
...
/obj/item/playing_cards/trading
	...
	proc/generate_human_image(var/mob/living/carbon/human/H)
		...
		var/skin_tone = H.bioHolder.mobAppearance.s_color
			...
			if (istype(W, /obj/item/playing_cards))
				...
				if (src.Cards)
					if (src.Cards.cards.len + C.cards.len > 60)
						...
						boutput(user, "<span style=\"color:orange\">You add [C] to the cards in [src].</span>")
