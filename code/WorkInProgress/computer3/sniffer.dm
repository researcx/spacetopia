...
/obj/item/device/net_sniffer
	...
	proc/interact(mob/user as mob)
		...
		var/dat = "<html><head><title>Packet Sniffer</title>[css_interfaces]</head><body>"
	...
	receive_signal(datum/signal/signal)
		...
		var/badcheck = 0
		for(var/check in src.sniff_filters)
			if(!(check in signal.data) || signal.data[check] != src.sniff_filters[check])
				badcheck = 1
				break
		if(badcheck)
			return
