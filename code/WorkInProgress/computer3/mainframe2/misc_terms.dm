...
/obj/machinery/networked/storage
	...
	attack_hand(mob/user as mob)
		...
		var/dat = "<html><head><title>Databank - \[[bank_id]]</title>[css_interfaces]</head><body>"

		dat += "<b>[capitalize(src.setup_tape_tag)]:</b> <a href='?src=\ref[src];tape=1'>[src.tape ? "Eject" : "--------"]</a><hr>"

		if (stat & NOPOWER)
			user << browse(dat,"window=databank;size=260x302")
...
/obj/machinery/networked/storage/bomb_tester
	...
	attack_hand(mob/user as mob)
		...
		var/dat = "<html><head><title>SimUnit - \[[bank_id]]</title>[css_interfaces]</head><body>"
		...
		user << browse(dat,"window=bombtester;size=260x302")
...
#define DISARM_CUTOFF 10 //Can't disarm past this point! OH NO!
	...
	attack_hand(mob/user as mob)
		...
		var/dat = "<html><head><title>Nuclear Charge</title>[css_interfaces]</head><body>"
		...
		user << browse(dat,"window=pnetnuke;size=260x302")
...
/obj/machinery/networked/radio
	...
	attack_hand(mob/user as mob)
		...
		var/dat = "<html><head><title>Network Radio</title>[css_interfaces]</head><body>"
		...
		user << browse(dat,"window=net_radio;size=260x302")
...
#define MAX_PRINTBUFFER_SIZE 10
	...
	attack_hand(mob/user as mob)
		...
		var/dat = "<html><head><title>Printer - \[[print_id]]</title>[css_interfaces]</head><body>"
		...
		user << browse(dat,"window=printer;size=260x302")
	...
	Topic(href, href_list)
		...
		if (href_list["unjam"])
			if(src.jam)
				...
				src.temp_msg = "PRINTER OK"
				src.updateUsrDialog()
				boutput(usr, "<span style=\"color:orange\">You clear the jam.</span>")
			else
...
/obj/machinery/networked/storage/scanner
	...
	attack_hand(mob/user as mob)
		...
		var/dat = "<html><head><title>Scanner - \[[copytext(bank_id,4)]]</title>[css_interfaces]</head><body>"
		...
		user << browse(dat,"window=scanner;size=260x302")
...
*/
	...
	attack_hand(mob/user as mob)
		...
		var/dat = "<html><head><title>IR Detector - \[[detector_id]]</title>[css_interfaces]</head><body>"
		...
		user << browse(dat,"window=secdetector;size=260x302")
...
*/
	...
	attack_hand(mob/user as mob)
		...
		var/dat = "<html><head><title>HEPT Emitter</title>[css_interfaces]</head><body><hr><center>Emission Crystals<br>"
	...
	Topic(href, href_list)
		...
		if (href_list["insert"])
			...
			if (istype(I, /obj/item/raw_material/telecrystal))
				...
				boutput(usr, "<span style=\"color:orange\">You insert [I] into the slot.</span>")
			...
			if (toEject)
				...
				boutput(usr, "<span style=\"color:orange\">You remove [toEject] from the slot.</span>")
...
/obj/machinery/networked/test_apparatus
	...
	attack_hand(mob/user as mob)
		...
		var/dat = "<html><head><title>[setup_device_name]</title>[css_interfaces]</head><body>"
