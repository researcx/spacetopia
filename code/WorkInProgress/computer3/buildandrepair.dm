	...
	switch(state)
		if(0)
			if(istype(P, /obj/item/wrench))
				...
				if(do_after(user, 20))
					boutput(user, "<span style=\"color:orange\">You wrench the frame into place.</span>")
					src.anchored = 1
					src.state = 1
			if(istype(P, /obj/item/weldingtool))
				playsound(src.loc, "sound/items/Welder.ogg", 50, 1)
				if(do_after(user, 20))
					boutput(user, "<span style=\"color:orange\">You deconstruct the frame.</span>")
		...
		if(1)
			if(istype(P, /obj/item/wrench))
				playsound(src.loc, "sound/items/Ratchet.ogg", 50, 1)
				if(do_after(user, 20))
					boutput(user, "<span style=\"color:orange\">You unfasten the frame.</span>")
					src.anchored = 0
					src.state = 0
			if(istype(P, /obj/item/motherboard) && !mainboard)
				playsound(src.loc, "sound/items/Deconstruct.ogg", 50, 1)
				boutput(user, "<span style=\"color:orange\">You place the mainboard inside the frame.</span>")
				src.icon_state = "1"
				src.mainboard = P
				user.drop_item()
				P.set_loc(src)
			if(istype(P, /obj/item/screwdriver) && mainboard)
				...
				boutput(user, "<span style=\"color:orange\">You screw the mainboard into place.</span>")
				src.state = 2
				src.icon_state = "2"
			if(istype(P, /obj/item/crowbar) && mainboard)
				playsound(src.loc, "sound/items/Crowbar.ogg", 50, 1)
				boutput(user, "<span style=\"color:orange\">You remove the mainboard.</span>")
				src.state = 1
				src.icon_state = "0"
		...
		if(2)
			if(istype(P, /obj/item/screwdriver) && mainboard && (!peripherals.len))
				playsound(src.loc, "sound/items/Screwdriver.ogg", 50, 1)
				boutput(user, "<span style=\"color:orange\">You unfasten the mainboard.</span>")
				src.state = 1
				src.icon_state = "1"
			...
			if(istype(P, /obj/item/crowbar) && src.peripherals.len)
				playsound(src.loc, "sound/items/Crowbar.ogg", 50, 1)
				boutput(user, "<span style=\"color:orange\">You remove the peripheral boards.</span>")
			...
			if(istype(P, /obj/item/cable_coil))
				if(P:amount >= 5)
					...
					if(do_after(user, 20))
						...
						P:amount -= 5
						if(!P:amount) qdel(P)
						boutput(user, "<span style=\"color:orange\">You add cables to the frame.</span>")
						src.state = 3
						src.icon_state = "3"
		if(3)
			if(istype(P, /obj/item/wirecutters))
				playsound(src.loc, "sound/items/Wirecutter.ogg", 50, 1)
				boutput(user, "<span style=\"color:orange\">You remove the cables.</span>")
				src.state = 2
				src.icon_state = "2"
			...
			if(istype(P, /obj/item/disk/data/fixed_disk) && !src.hd)
				...
				src.hd = P
				P.set_loc(src)
				boutput(user, "<span style=\"color:orange\">You connect the drive to the cabling.</span>")

			if(istype(P, /obj/item/crowbar) && src.hd)
				playsound(src.loc, "sound/items/Crowbar.ogg", 50, 1)
				boutput(user, "<span style=\"color:orange\">You remove the hard drive.</span>")
				src.hd.set_loc(src.loc)
				src.hd = null
				...
				if (S.material && S.material.material_flags & MATERIAL_CRYSTAL)
					if (S.amount >= src.glass_needed)
						...
						if(do_after(user, 20) && S)
							...
							if(S.amount < 1)
								qdel(S)
							boutput(user, "<span style=\"color:orange\">You put in the glass panel.</span>")
							src.state = 4
							src.icon_state = "4"
					else
		...
		if(4)
			if(istype(P, /obj/item/crowbar))
				playsound(src.loc, "sound/items/Crowbar.ogg", 50, 1)
				boutput(user, "<span style=\"color:orange\">You remove the glass panel.</span>")
				src.state = 3
				src.icon_state = "3"
			...
			if(istype(P, /obj/item/screwdriver))
				playsound(src.loc, "sound/items/Screwdriver.ogg", 50, 1)
				boutput(user, "<span style=\"color:orange\">You connect the monitor.</span>")
