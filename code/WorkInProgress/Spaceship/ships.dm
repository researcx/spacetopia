...
/obj/item/putt/frame_box
	...
	attack_self(mob/user as mob)
		boutput(user, "<span style=\"color:orange\">You dump out the box of parts onto the floor.</span>")
...
/obj/structure/puttframe/verb/deconstruct()
	...
	boutput(usr, "<span style=\"color:orange\">You deconstructed the MiniPutt frame.</span>")
	var/obj/O
	if (stage == 10)
	...
	switch(stage)
		...
		if(3)
			if(istype(W, /obj/item/cable_coil))
				if(W.amount < 2)
					boutput(user, "<span style=\"color:orange\">You need at least two lengths of cable.</span>")
					return
...
/obj/structure/podframe/verb/deconstruct()
	...
	boutput(usr, "<span style=\"color:orange\">You deconstructed the pod frame.</span>")
	var/obj/O
	if (stage == 10)
...
/obj/structure/podframe/attackby(obj/item/W as obj, mob/living/user as mob)
	switch(stage)
		...
		if(3)
			if(istype(W, /obj/item/cable_coil))
				if(W.amount < 4)
					boutput(user, "<span style=\"color:orange\">You need at least four lengths of cable.</span>")
					return
				boutput(user, "You begin to install the wiring...")
				playsound(src.loc, "sound/items/Deconstruct.ogg", 50, 1)
				if (!do_after(user, 30))
