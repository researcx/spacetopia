			...
			if (istype(W, /obj/item/ammo/bullets))
				...
				if (src.m_w_system)
					...
					if (may_load < ammo.amount_left)
						...
						boutput(user, "<span style=\"color:orange\">You load [may_load] ammunition from [ammo]. [ammo] now contains [ammo.amount_left] ammunition.</span>")
						...
						ammo.amount_left = 0
						boutput(user, "<span style=\"color:orange\">You load [ammo] into [m_w_system].</span>")
		...
		///////////////////////////////////////
			...
			// Added logs for atmos tanks and such here, because booby-trapping pods is becoming a trend (Convair880).
				...
				if (W && istype(W, /obj/item/tank))
					...
					boutput(usr, "<span style=\"color:orange\">You attach the [W.name] to [src.name]'s air supply valve.</span>")
					usr.drop_item()
					W.set_loc(src)
					src.atmostank = W
					src.updateDialog()
				...
				if (W && istype(W, /obj/item/tank))
					...
					boutput(usr, "<span style=\"color:orange\">You attach the [W.name] to [src.name]'s fuel supply valve.</span>")
					usr.drop_item()
					W.set_loc(src)
					src.fueltank = W
					src.updateDialog()
...
/obj/machinery/vehicle/proc/board_pod(var/mob/boarder)
	...
	boutput(M, "<span style=\"color:orange\">You can also use the Space Bar to fire!</span>")
...
/obj/machinery/vehicle/proc/open_parts_panel(mob/user as mob)
	...
	var/dat = "[css_interfaces]<TT><B>[src] Maintenance Panel</B><BR><HR><BR>"
	//Air and Fuel tanks
...
/obj/machinery/vehicle/proc/access_computer(mob/user as mob)
	...
	var/dat = "[css_interfaces]<TT><B>[src] Control Console</B><BR><HR><BR>"
