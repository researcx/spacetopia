...
//			H.w_uniform.add_fingerprint(src)
		...
		src.tri_message("<span style=\"color:orange\"><b>[src]</b> puts pressure on [src == H ? "[his_her]" : "[H]'s"] wounds, trying to stop the bleeding!</span>",\
		src, "<span style=\"color:orange\">You put pressure on [src == H ? "your" : "[H]'s"] wounds, trying to stop the bleeding!</span>",\
		H, "<span style=\"color:orange\">[H == src ? "You put" : "<b>[src]</b> puts"] pressure on your wounds, trying to stop the bleeding!</span>")
