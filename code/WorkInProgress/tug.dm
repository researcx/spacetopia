...
//	sealed_cabin = 0
	...
	eject_rider(var/crashed, var/selfdismount)
		...
		if (crashed)
			...
			return
		if (selfdismount)
			boutput(rider, "<span style=\"color:orange\">You dismount from [src].</span>")
			for (var/mob/C in AIviewers(src))
				if (C == rider)
					continue
	...
	MouseDrop_T(var/atom/movable/C, mob/user)
		...
		if (target == user && !user.stat)	// if drop self, then climbed in
			...
			boutput(user, "<span style=\"color:orange\">You climb onto [src].</span>")
		else if (target != user && !user.restrained())
			msg = "[user.name] helps [target.name] onto [src]!"
			boutput(user, "<span style=\"color:orange\">You help [target.name] onto [src]!</span>")
		else
			return
