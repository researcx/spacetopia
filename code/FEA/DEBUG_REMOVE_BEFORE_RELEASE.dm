	...
	verb
		parent_info()
			...
			if(parent)
				...
				boutput(usr, "<span style=\"color:orange\">[x],[y] has no parent air group.</span>")
	...
	valve
		verb
			...
			network_data()
				...
				boutput(world, "<span style=\"color:orange\">[x],[y]</span>")
...
turf/simulated
	...
	verb
		...
		air_status()
			...
			boutput(usr, "<span style=\"color:orange\">@[x],[y] ([GM.group_multiplier]): O:[GM.oxygen] T:[GM.toxins] N:[GM.nitrogen] C:[GM.carbon_dioxide] w [GM.temperature] Kelvin, [GM.return_pressure()] kPa [(active_hotspot)?("<span style=\"color:red\">BURNING</span>"):(null)]")
