	...
	proc/collide(atom/A as mob|obj|turf|area)
		...
		if (ismob(A))
			for (var/obj/item/cloaking_device/S in A.contents)
				if (S.active)
					S.deactivate(A)
					src.visible_message("<span style=\"color:orange\"><b>[A]'s cloak is disrupted!</b></span>")
			for (var/obj/item/device/disguiser/D in A.contents)
				if (D.on)
					D.disrupt(A)
					src.visible_message("<span style=\"color:orange\"><b>[A]'s disguiser is disrupted!</b></span>")
