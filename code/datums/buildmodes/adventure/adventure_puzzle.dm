/datum/adventure_submode/wizard
	...
	click_raw(var/atom/object, location, control, params)
		...
		if (pa.Find("ctrl") && pa.Find("shift") && pa.Find("right"))
			if (!pa.Find("alt"))
				if (istype(object, /obj/adventurepuzzle))
					boutput(usr, "<span style=\"color:orange\">Decreased layer by 0.1.</span>")
					var/obj/O = object
					O.layer -= 0.1
			else
				boutput(usr, "<span style=\"color:orange\">Reset the layers of every adventure object on that turf.</span>")
		...
		if (wizard.finished)
			qdel(wizard)
			wizard = null
			boutput(usr, "<span style=\"color:orange\">The wizard is finished.</span>")

	selected()
		boutput(usr, {"<span style=\"color:orange\">Right click the button to select the type of adventure wizard.<br>
