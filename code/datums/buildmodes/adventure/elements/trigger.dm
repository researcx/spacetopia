/datum/puzzlewizard/trigger
	...
	initialize()
		...
		boutput(usr, "<span style=\"color:orange\">Left click to place triggers, right click triggerables to (de)select them for automatic assignment to the triggers. Ctrl+click anywhere to finish.</span>")
		boutput(usr, "<span style=\"color:orange\">NOTE: Select stuff first, then make triggers for extra comfort!</span>")
