/datum/puzzlewizard/pressurepad
	...
	initialize()
		...
		boutput(usr, "<span style=\"color:orange\">Left click to place pressure pads, right click triggerables to (de)select them for automatic assignment to the pressure pads. Ctrl+click anywhere to finish.</span>")
		boutput(usr, "<span style=\"color:orange\">NOTE: Select stuff first, then make pressure pads for extra comfort!</span>")
