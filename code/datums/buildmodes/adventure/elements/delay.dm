/datum/puzzlewizard/delay
	...
	initialize()
		...
		boutput(usr, "<span style=\"color:orange\">Left click to place timers, right click triggerables to (de)select them for automatic assignment to the timers. Ctrl+click anywhere to finish.</span>")
		boutput(usr, "<span style=\"color:orange\">Right click delays to launch them immediately. (Useful for triggering periodic delays)</span>")
		boutput(usr, "<span style=\"color:orange\">NOTE: Select stuff first, then make buttons for extra comfort!</span>")
