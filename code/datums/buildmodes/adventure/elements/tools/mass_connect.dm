/datum/puzzlewizard/massconnecttool
	...
	initialize()
		...
		boutput(usr, "<span style=\"color:orange\">First, left click a triggerer to select it. Then left click an existing triggerable to select its type and trigger actions.</span>")
		boutput(usr, "<span style=\"color:orange\">Finally, use right click to select a rectangular area (as in wide spawn mode) to assign ALL triggerables of that type with the same name to the selected triggerer. Ctrl+click to finish.</span>")
		boutput(usr, "<span style=\"color:orange\">Right clicking a triggerer will clear all of its connections.</span>")
		boutput(usr, "<span style=\"color:orange\">Valid triggerers: trigger, button, pressure pad, key, remote control</span>")
		boutput(usr, "<span style=\"color:orange\">Valid triggerables: door, spawn location, light emitter, sliding wall, traps</span>")
	...
	build_click(var/mob/user, var/datum/buildmode_holder/holder, var/list/pa, var/atom/object)
		...
		if (pa.Find("left"))
			...
			if (A && T == A)
				...
				A.overlays -= selection
				A = null
				boutput(usr, "<span style=\"color:orange\">[count] objects added to [selected].</span>")
			else
				A = T
				A.overlays += selection
