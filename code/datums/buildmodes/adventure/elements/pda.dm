...
But if the document contains &lt;1&gt; &lt;2&gt; &lt;4&gt; then the number of action slots will be 2 instead of 4 due to the missing &lt;3&gt;."}
	...
	initialize()
		...
		if (is_custom == "Yes")
			...
			if (!html_file)
				boutput(usr, "<span style=\"color:orange\">File loading cancelled. Not using custom HTML.</span>")
				is_custom = "No"
			else
				...
				if (!html_content)
					...
					boutput(usr, "<span style=\"color:orange\">Verifying file sanity.</span>")
					...
					var/regex/jsep = new("on[a-zA-Z]+\\s*=", "i")
						...
						if (!action_slots)
							...
							boutput(usr, "<span style=\"color:orange\">File is probably sanitary. Allowed. Goodbye universe.</span>")
			boutput(usr, "<span style=\"color:orange\">STAGE 1: Select triggerables and associate actions with left click. Right Click to associate a name with this action group and start a new action group.</span>")
			boutput(usr, "<span style=\"color:orange\">All actions in a single group will be executed at once by the associated named button.</span>")
			boutput(usr, "<span style=\"color:orange\">CTRL+Right Click to end the action assignment phase and place handheld computers.</span>")
			boutput(usr, "<span style=\"color:orange\">You will not be able to assign new actions after you end this stage.</span>")
			boutput(usr, "<span style=\"color:orange\">If you loaded custom HTML, the number of action groups must match the amount of action slots defined by the HTML.</span>")
