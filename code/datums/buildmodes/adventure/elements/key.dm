/datum/puzzlewizard/key
	...
	initialize()
		...
		boutput(usr, "<span style=\"color:orange\">Left click to place keys, right click triggerables to (de)select them for automatic assignment to the keys. Ctrl+click anywhere to finish.</span>")
		boutput(usr, "<span style=\"color:orange\">NOTE: Select stuff first, then make keys for extra comfort!</span>")
	...
	afterattack(atom/target as mob|obj|turf|area, mob/user as mob)
		...
		if (istype(target, /obj/adventurepuzzle/triggerable))
			if (target in src.triggered)
				boutput(usr, "<span style=\"color:orange\">The key slides into [target]!</span>")
