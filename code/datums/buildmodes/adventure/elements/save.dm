...
/datum/puzzlewizard/save
	...
	initialize()
		...
		boutput(usr, "<span style=\"color:orange\">Use left clicks to mark two corners of the rectangular area to save. Saving will take a significant amount of time, and you should not modify the area until the saving is completed.</span>")
	...
	build_click(var/mob/user, var/datum/buildmode_holder/holder, var/list/pa, var/atom/object)
		if (pa.Find("left"))
			...
			if (T)
				if (!A)
					A = T
					boutput(usr, "<span style=\"color:orange\">Corner #1 set.</span>")
				else
					if (A.z != T.z)
					...
					A = null
					src = null
					boutput(usr, "<span style=\"color:orange\">Corner #2 set. Now beginning saving. Modifying the area may have unexpected results. DO NOT LOG OUT OR CHANGE MOB UNTIL THE SAVING IS FINISHED.</span>")
					...
					spawn(0)
						...
						if (usr && usr.client)
							...
							F.ExportText("/", target)
							boutput(usr, "<span style=\"color:orange\">Saving finished.</span>")
							usr << ftp(target)
							if (fexists(fname))
								fdel(fname)
						if (this)
