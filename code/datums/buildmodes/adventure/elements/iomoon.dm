...
/datum/puzzlewizard/iomoon_puzzle
	...
	initialize()
		boutput(usr, "<span style=\"color:orange\">Right click to select puzzle element, left click a turf to place the element, left click two elements to link them (activator -> target), ctrl+click anywhere to finish.</span>")
