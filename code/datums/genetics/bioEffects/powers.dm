	...
	cast(atom/target)
		...
		for (var/mob/living/L in T.contents)
			if (L == owner && linked_power.safety)
				continue
			boutput(L, "<span style=\"color:orange\">You are struck by a burst of ice cold air!</span>")
			if(L.burning)
				L.set_burning(0)
...
/datum/targetable/geneticsAbility/empath
	...
	cast(atom/target)
		...
		boutput(usr, "<span style=\"color:orange\">Mind Reading of [read.name]:</b></span>")
		...
		switch(pain_condition)
			if (81 to INFINITY)
				boutput(owner, "<span style=\"color:orange\"><b>Condition</b>: [read.name] feels good.</span>")
			if (61 to 80)
				boutput(owner, "<span style=\"color:orange\"><b>Condition</b>: [read.name] is suffering mild pain.</span>")
			if (41 to 60)
				boutput(owner, "<span style=\"color:orange\"><b>Condition</b>: [read.name] is suffering significant pain.</span>")
			if (21 to 40)
				boutput(owner, "<span style=\"color:orange\"><b>Condition</b>: [read.name] is suffering severe pain.</span>")
			else
				boutput(owner, "<span style=\"color:orange\"><b>Condition</b>: [read.name] is suffering excruciating pain.</span>")
				thoughts = "haunted by their own mortality"

		switch(read.a_intent)
			if (INTENT_HELP)
				boutput(owner, "<span style=\"color:orange\"><b>Mood</b>: You sense benevolent thoughts from [read.name].</span>")
			if (INTENT_DISARM)
				boutput(owner, "<span style=\"color:orange\"><b>Mood</b>: You sense cautious thoughts from [read.name].</span>")
			if (INTENT_GRAB)
				boutput(owner, "<span style=\"color:orange\"><b>Mood</b>: You sense hostile thoughts from [read.name].</span>")
			if (INTENT_HARM)
				boutput(owner, "<span style=\"color:orange\"><b>Mood</b>: You sense cruel thoughts from [read.name].</span>")
				...
				boutput(owner, "<span style=\"color:orange\"><b>Mood</b>: You sense strange thoughts from [read.name].</span>")

		if (istype(target,/mob/living/carbon/human))
			var/mob/living/carbon/human/H = read
			boutput(owner, "<span style=\"color:orange\"><b>Numbers</b>: You sense the number [H.pin] is important to [H.name].</span>")
		boutput(owner, "<span style=\"color:orange\"><b>Thoughts</b>: [read.name] is currently [thoughts].</span>")
...
/datum/targetable/geneticsAbility/adrenaline
	...
	cast()
		...
		if(linked_power.power)
			multiplier = 2
		if (owner.reagents)
			boutput(owner, "<span style=\"color:orange\">You get pumped up!</span>")
			owner.emote("scream")
...
/datum/targetable/geneticsAbility/healing_touch
	...
	cast(atom/target)
		...
		boutput(C, "<span style=\"color:orange\">You feel your pain fading away.</span>")
