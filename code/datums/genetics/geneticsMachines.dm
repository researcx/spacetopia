...
/obj/machinery/computer/genetics/Topic(href, href_list)
	...
	if(href_list["viewpool"])
		...
		if(istext(result))
			...
			boutput(usr, "<span style=\"color:orange\"><b>SCANNER ALERT:</b> Splice successful.</span>")
			src.saved_chromosomes -= C
			qdel(C)
			src.to_splice = null
		...
		if(bp.marker == "locked")
			boutput(usr, "<span style=\"color:orange\"><b>SCANNER ALERT:</b> Encryption is a [E.lockedDiff]-character code.</span>")
			var/characters = ""
			for(var/X in E.lockedChars)
				characters += "[X] "
			boutput(usr, "<span style=\"color:orange\">Possible characters in this code: [characters]</span>")
			if(genResearch.lock_breakers > 0)
				boutput(usr, "<span style=\"color:orange\">[genResearch.lock_breakers] auto-decryptions available. Enter UNLOCK as the code to expend one.</span>")
			...
			if (code == "UNLOCK")
				if(genResearch.lock_breakers > 0)
					...
					bp.bpp2 = bpc.bpp2
					bp.marker = "green"
					boutput(usr, "<span style=\"color:orange\"><b>SCANNER ALERT:</b> Base pair unlocked.</span>")
					if (E.dnaBlocks.sequenceCorrect())
						E.dnaBlocks.ChangeAllMarkers("white")
					usr << link("byond://?src=\ref[src];viewpool=\ref[E]")
					return
			...
			if (code == bp.lockcode)
				...
				bp.bpp2 = bpc.bpp2
				bp.marker = "green"
				boutput(usr, "<span style=\"color:orange\"><b>SCANNER ALERT:</b> Decryption successful. Base pair unlocked.</span>")
				if (E.dnaBlocks.sequenceCorrect())
					E.dnaBlocks.ChangeAllMarkers("white")
			else
				if (bp.locktries <= 1)
