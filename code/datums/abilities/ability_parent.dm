	...
	proc/actionKey(var/num)
		...
		for (var/datum/targetable/T in src.abilities)
			if(T.waiting_for_hotkey)
				...
				boutput(owner, "<span style=\"color:orange\">Bound [T.name] to [num].</span>")
				updateButtons()
				return 1
		...
		if(parameters["left"])
			...
			if (parameters["ctrl"])
				...
				if (owner == holder.ctrlPower)
					...
					boutput(user, "<span style=\"color:orange\"><b>[owner.name] has been unbound from Ctrl-Click.</b></span>")
					holder.updateButtons()
				else
					holder.ctrlPower = owner
					boutput(user, "<span style=\"color:orange\"><b>[owner.name] is now bound to Ctrl-Click.</b></span>")
				...
				if (owner == holder.altPower)
					...
					boutput(user, "<span style=\"color:orange\"><b>[owner.name] has been unbound from Alt-Click.</b></span>")
					holder.updateButtons()
				else
					holder.altPower = owner
					boutput(user, "<span style=\"color:orange\"><b>[owner.name] is now bound to Alt-Click.</b></span>")
				...
				if (owner == holder.shiftPower)
					...
					boutput(user, "<span style=\"color:orange\"><b>[owner.name] has been unbound from Shift-Click.</b></span>")
					holder.updateButtons()
				else
					holder.shiftPower = owner
					boutput(user, "<span style=\"color:orange\"><b>[owner.name] is now bound to Shift-Click.</b></span>")

			else
				if (holder.help_mode && owner.helpable)
					boutput(user, "<span style=\"color:orange\"><b>This is your [owner.name] ability.</b></span>")
					boutput(user, "<span style=\"color:orange\">[owner.desc]</span>")
					if (owner.holder.usesPoints)
						boutput(user, "<span style=\"color:orange\">Cost: <strong>[owner.pointCost]</strong></span>")
					if (owner.cooldown)
						boutput(user, "<span style=\"color:orange\">Cooldown: <strong>[owner.cooldown / 10] seconds</strong></span>")
				else
			...
			if(owner.waiting_for_hotkey)
				...
				boutput(usr, "<span style=\"color:orange\">Please press a number to bind this ability to...</span>")
