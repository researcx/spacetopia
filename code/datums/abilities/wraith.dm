...
/datum/targetable/wraithAbility/help
	...
	cast(atom/target)
		...
		if (holder.help_mode)
			...
			holder.help_mode = 1
			boutput(holder.owner, "<span style=\"color:orange\"><strong>Help Mode has been activated  To disable it, click on this button again.</strong></span>")
			boutput(holder.owner, "<span style=\"color:orange\">Hold down Shift, Ctrl or Alt while clicking the button to set it to that key.</span>")
			boutput(holder.owner, "<span style=\"color:orange\">You will then be able to use it freely by holding that button and left-clicking a tile.</span>")
			boutput(holder.owner, "<span style=\"color:orange\">Alternatively, you can click with your middle mouse button to use the ability on your current tile.</span>")
	...
	cast(atom/T)
		...
		if (ishuman(T))
			...
			if (H.bioHolder.HasEffect("training_chaplain"))
				...
				boutput(usr, "<span style=\"color:orange\">[pick("You sap [T]'s energy.", "You suck the breath out of [T].")]</span>")
			...
			if (O.emag_act(null, null))
				boutput(usr, "<span style=\"color:orange\">You alter the energy of [O].</span>")
				return 0
			else
...
/datum/targetable/wraithAbility/poltergeist
	...
	cast()
		...
		if (status == 0)
			...
			boutput(usr, "<span style=\"color:orange\">You prepare the area.</span>")
			return 0
		else
			if (casting)
				return
			...
			switch (effect)
				if (1)
					boutput(usr, "<span style=\"color:orange\">You flip some light switches near the designated location!!</span>")
					for (var/obj/machinery/light_switch/L in range(10, marker))
						L.attack_hand(holder.owner)
					return 0
				if (2)
					boutput(usr, "<span style=\"color:orange\">You cause a few lights to burn out near the designated location!.</span>")
					var/c_prob = 100
					...
					return 0
				if (3)
					boutput(usr, "<span style=\"color:orange\">Smoke rises in the designated location.</span>")
					...
					return 0
				if (4)
					boutput(usr, "<span style=\"color:orange\">Matter from your realm appears near the designated location!</span>")
					var/count = rand(5,9)
					var/turf/trgloc = get_turf(marker)
				...
				if (5)
					...
					if (!apc)
						...
						return 0
					boutput(usr, "<span style=\"color:orange\">You sap the power of the chamber's power source.</span>")
					var/obj/item/cell/cell = apc.cell
					if (cell)
						cell.use(sapped_amt)
					return 0
				if (6)
					boutput(usr, "<span style=\"color:orange\">Mysterious messages haunt PDAs near the designated location!</span>")
					...
					for (var/obj/item/device/pda2/pda in range(10, marker))
						src.haunt_pda(pda)
				if (7)
					boutput(usr, "<span style=\"color:orange\">Crates, lockers and doors mysteriously open and close in the designated area!</span>")
					var/c_prob = 100
