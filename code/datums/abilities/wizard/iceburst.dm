		...
		for (var/mob/living/M as mob in oview())
			...
			spawn(0)
				...
				for(i=0, i<20, i++)
					...
					step_to(A,M,0)
					if (get_dist(A,M) == 0)
						boutput(M, text("<span style=\"color:orange\">You are chilled by a burst of magical ice!</span>"))
...
/obj/icecube
	...
	attack_hand(mob/user as mob)
		user.visible_message("<span class='combat'><b>[user]</b> kicks [src]!</span>", "<span style=\"color:orange\">You kick [src].</span>")
