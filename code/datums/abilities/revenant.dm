...
/datum/bioEffect/hidden/revenant
	...
	proc/ghoulTouch(var/mob/living/carbon/human/poorSob, var/obj/item/affecting)
		if (poorSob.bioHolder.HasEffect("training_chaplain"))
			poorSob.visible_message("<span style=\"color:red\">[poorSob]'s faith shields them from [owner]'s ethereal force!", "<span style=\"color:orange\">Your faith protects you from [owner]'s ethereal force!</span>")
			return
		else
	...
	proc/wraithPossess(var/mob/wraith/W)
		...
		owner.visible_message("<span style=\"color:red\"><strong>[pick("[owner] suddenly rises from the floor!", "[owner] suddenly looks a lot less dead!", "A dark light shines from [owner]'s eyes!")]</strong></span>",\
			                  "<span style=\"color:orange\">[pick("You force your will into [owner]'s corpse.", "Your dark will forces [owner] to rise.", "You assume direct control of [owner].")]</span>")
...
/datum/targetable/revenantAbility/touchOfEvil
	...
	cast()
		if (istype(holder, /datum/abilityHolder/revenant))
			...
			holder.owner.visible_message("<span style=\"color:red\">[holder.owner] glows with ethereal power!</span>", "<span style=\"color:orange\">You feel ghostly strength pulsing through you.</span>")
			return 0
	...
	cast(atom/target)
		...
		if (ismob(target))
			...
			if (T.bioHolder && T.bioHolder.HasEffect("training_chaplain"))
				...
				holder.owner.show_message("<span style=\"color:orange\">You hurl [target] away from you!</span>")
...
/datum/targetable/revenantAbility/crush
	...
	cast(atom/target)
		...
		holder.owner.visible_message("<span style=\"color:red\">[holder.owner] reaches out towards [H], making a crushing motion.</span>", "<span style=\"color:orange\">You reach out towards [H].</span>")
		H.weakened += 2
