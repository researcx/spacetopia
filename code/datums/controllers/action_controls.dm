...
/datum/action/bar/blob_health // WOW HACK
	onUpdate()
		...
		bar.pixel_x = -nround( ((30 - (30 * complete)) / 2) )
/*
/datum/action/bar/bullethell colosseum and gauntlet removal
...
/datum/action/bar/icon/build
	...
	onStart()
		...
		owner.visible_message("<span style=\"color:orange\">[owner] begins assembling [objname]!</span>")

	onEnd()
		..()
		owner.visible_message("<span style=\"color:orange\">[owner] assembles [objname]!</span>")
...
/datum/action/bar/icon/cruiser_repair
	...
	onStart()
		..()
		owner.visible_message("<span style=\"color:orange\">[owner] begins repairing [repairing]!</span>")

	onEnd()
		..()
		owner.visible_message("<span style=\"color:orange\">[owner] successfully repairs [repairing]!</span>")
		...
		if(owner && ownerMob && target && cuffs && !target.handcuffed && cuffs == ownerMob.equipped() && get_dist(owner, target) <= 1)
			...
			if (issilicon(ownerMob))
				...
				if (cuffs.amount >= 2)
					...
					cuffs.amount--
					boutput(ownerMob, "<span style=\"color:orange\">The [cuffs.name] now has [cuffs.amount] lengths of [istype(cuffs, /obj/item/handcuffs/tape_roll) ? "tape" : "ziptie"] left.</span>")
...
/datum/action/bar/private/icon/handcuffRemoval //This is used when you try to resist out of handcuffs.
	...
	onEnd()
		...
		if(owner != null && istype(owner, /mob/living/carbon/human) && owner:handcuffed)
			...
			boutput(H, "<span style=\"color:orange\">You successfully remove your handcuffs.</span>")
