...
/datum/event_controller
	...
	var/minimum_population = 10 // Minimum amount of players connected for event to occur
	...
	proc/event_cycle()
		...
		if (events_enabled && (num_players >= minimum_population))
			...
			message_admins("<span style=\"color:orange\">A random event would have happened now, but they are disabled!</span>")
		...
		message_admins("<span style=\"color:orange\">Next event will occur at [round(next_event / 600)] minutes into the round.</span>")
		spawn(event_timer)
			event_cycle()
