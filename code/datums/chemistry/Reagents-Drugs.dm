...
/*		drug/bathsalts
			...
			reaction_mob(var/mob/M, var/method=TOUCH, var/volume)
				if(method == INGEST)
					...
					boutput(M, "<span style=\"color:orange\">You feel a bit more salty than usual.</span>")
				return
		...
		drug/crank
			...
			on_mob_life(var/mob/M)
				...
				if(prob(8))
					boutput(M, "<span style=\"color:orange\"><b>You feel great!</b></span>")
					M.reagents.add_reagent("methamphetamine", rand(1,2))
					M.emote(pick("laugh", "giggle"))
				if(prob(6))
					boutput(M, "<span style=\"color:orange\"><b>You feel warm.</b></span>")
...
//				if(prob(7)) M.emote(pick("twitch","drool","moan","giggle"))
		...
		drug/CBD
			name="cannabidiol"
			id="CBD"
			description="A non-psychoactive cannabinoid which has antipsychotic properties."
			reagent_state = LIQUID
			fluid_r = 184
			fluid_g = 184
			fluid_b = 0
			...
			on_mob_life(var/mob/M)
				...
				if(prob(3))
					switch (rand(1,3))
						...
						if(2)
							boutput(M, "<span style=\"color:red\"><b>Restarting world!</b> </span><span style=\"color:orange\">Initiated by Administrator!</span>")
...
/*		drug/krokodil
			...
			on_mob_life(var/mob/M)
				...
				if(prob(10))
					boutput(M, "<span style=\"color:orange\"><b>You feel pretty chill.</b></span>")
					M.bodytemperature--
					M.emote("smile")
				if(prob(5))
		...
		drug/catdrugs
			...
			on_mob_life(var/mob/M)
				...
				M.druggy = max(M.druggy, 15)
				if(prob(11))
					M.visible_message("<span style=\"color:orange\"><b>[M.name]</b> hisses!</span>")
					playsound(M.loc, "sound/effects/cat_hiss.ogg", 50, 1)
				if(prob(9))
					M.visible_message("<span style=\"color:orange\"><b>[M.name]</b> meows! What the fuck?</span>")
