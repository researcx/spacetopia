	...
	reagent
		nitroglycerin // Yes, this is a bad idea.
			...
			proc/explode(var/turf/T, expl_reason, del_holder=1)
				...
				if(holder && holder.my_atom) // Erik: Fix for Cannot read null.fingerprintshidden
					...
					context = "Fingerprints: [dd_list2text(fh)]"
		...
		aranesp
			...
			on_mob_life(var/mob/M)
				...
				if (prob(8)) boutput(M, "<span style=\"color:orange\">You feel [pick("really buff", "on top of the world","like you're made of steel", "energized", "invigorated", "full of energy")]!</span>")
				if (prob(5))
				...
				if (volume)
					if (method == TOUCH)
						if (istype(M, /mob/living/silicon/robot))
							...
							R.oil += volume * 2
							boutput(R, "<span style=\"color:orange\">Your joints and servos begin to run more smoothly.</span>")
...
/*		eyeofnewt
		...
		love
			...
			reaction_mob(var/mob/M)
				boutput(M, "<span style=\"color:orange\">You feel loved!</span>")
				return
			...
			on_mob_life(var/mob/M)
				...
				if (prob(8))
					...
					boutput(M, "<span style=\"color:orange\">You feel [.].</span>")
...
/*		fliptonium
			...
			reaction_mob(var/mob/M)
				...
				//DEBUG("<span style=\"color:orange\"><b>Spun [M]: [dir_temp], [speed_temp]</b></span>") // <- What's this?
...
/*			reaction_obj(var/obj/O, var/volume)
				if (volume >= 10)
					...
					//DEBUG("<span style=\"color:orange\"><b>Spun [O]: [dir_temp], [speed_temp]</b></span>")
*/
			on_add()
				if (istype(holder) && istype(holder.my_atom) && hascall(holder.my_atom,"add_stam_mod_regen"))
			...
			/*on_mob_life(var/mob/M) why
				for (var/datum/ailment_data/disease/virus in M.ailments)
					if (prob(10))
						...
						M.ailments -= virus
						boutput(M, "<span style=\"color:orange\">You feel better</span>")
				..(M)
				return*/
			reaction_turf(var/turf/T, var/volume)
				src = null
