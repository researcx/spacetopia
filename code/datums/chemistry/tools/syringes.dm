		...
		switch(mode)
			if (S_DRAW)
				if (istype(target, /mob/living))//Blood!
					...
					boutput(user, "<span style=\"color:orange\">You fill the syringe with 5 units of [target]'s blood.</span>")
					return
				...
				boutput(user, "<span style=\"color:orange\">You fill the syringe with 5 units of the solution.</span>")
			...
			if (S_INJECT)
				...
				if (istype(target, /obj/item/bloodslide))
					...
					boutput(user, "<span style=\"color:orange\">You fill the blood slide with [transferred] units of the solution.</span>")
					// contingency
				...
				if (istype(target,/obj/item/reagent_containers/patch))
					...
					boutput(user, "<span style=\"color:orange\">You fill [P].</span>")
					if (P.medical == 1)
						//break the seal
				...
				spawn (5)
					if (src && src.reagents && target && target.reagents)
						...
						boutput(user, "<span style=\"color:orange\">You inject 5 units of the solution. The syringe now contains [src.reagents.total_volume] units.</span>")
		return
