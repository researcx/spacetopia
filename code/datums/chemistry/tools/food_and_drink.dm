			...
			if (M == user)
				if (!bypass_utensils)
					...
					if (src.needspoon && !user.find_in_hands(/obj/item/kitchen/utensil/spoon))
						...
						M.visible_message("<span style=\"color:red\">[user] stares glumly at [src].</span>")
						return
				M.visible_message("<span style=\"color:orange\">[M] takes a bite of [src]!</span>",\
				"<span style=\"color:orange\">You take a bite of [src]!</span>")
						...
						if (doseed)
							...
							if (istype(stored) && !stored.isgrass)
								...
								if (stored.hybrid)
									...
									S.planttype = hybrid
								user.visible_message("<span style=\"color:orange\"><b>[user]</b> spits out a seed.</span>",\
								"<span style=\"color:orange\">You spit out a seed.</span>")
					user.u_equip(src)
					on_finish(M)
					qdel(src)
				return 1
...
/obj/item/reagent_containers/food/drinks
	...
	attack(mob/M as mob, mob/user as mob, def_zone)
		...
		if (iscarbon(M) || iscritter(M))
			if (M == user)
				M.visible_message("<span style=\"color:orange\">[M] takes a sip from [src].</span>")
			else
			...
			if (M.mind && M.mind.assigned_role == "Barman")
				...
				boutput(M, "<span style=\"color:orange\">Tastes like there might be some [reag_list] in this.</span>")
/*			else
				var/reag_list = ""
				...
				boutput(M, "<span style=\"color:orange\">You taste [reag_list]in this.</span>")
*/
...
/obj/item/reagent_containers/food/drinks/drinkingglass
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if (istype(W, /obj/item/raw_material/ice))
			...
			if (src.wedge)
				...
				return
			boutput(user, "<span style=\"color:orange\">You add [W] to the lip of [src].</span>")
			src.wedge = "orange"
			src.update_icon()
			qdel(W)
			...
			if (src.reagents.total_volume >= src.reagents.maximum_volume)
				boutput(user, "<span style=\"color:red\">[src] is full.</span>")
				return
			boutput(user, "<span style=\"color:orange\">You squeeze [W] into [src].</span>")
			...
			if (src.umbrella || src.in_glass)
				...
				return
			boutput(user, "<span style=\"color:orange\">You add [W] to [src].</span>")
			...
			if (S.shakes >= 15)
				...
				boutput(user, "<span style=\"color:orange\">You salt the rim of [src].</span>")
				src.salted = 1
				src.update_icon()
				S.shakes ++
				return
			...
			boutput(user, "<span style=\"color:orange\">You crack [W] into [src].</span>")
	...
	attack_self(var/mob/user as mob)
		...
		if (!ishuman(user))
			boutput(user, "<span style=\"color:orange\">You don't know what to do with the glass.</span>")
			return
		...
		if (!actions.len)
			boutput(user, "<span style=\"color:orange\">You can't think of anything to do with the glass.</span>")
			return
