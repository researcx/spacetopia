...
/obj/item/chem_grenade
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if (istype(W,/obj/item/grenade_fuse) && !stage)
			boutput(user, "<span style=\"color:orange\">You add [W] to the metal casing.</span>")
			...
			if (beakers.len)
				boutput(user, "<span style=\"color:orange\">You lock the assembly.</span>")
			...
			if (beakers.len == 2)
				...
				if (W.reagents.total_volume)
					boutput(user, "<span style=\"color:orange\">You add \the [W] to the assembly.</span>")
					user.drop_item()
					W.set_loc(src)
					beakers += W
				else
			...
			if (!S || !S:status)
				return
			boutput(user, "<span style=\"color:orange\">You attach the [src.name] to the [S.name]!</span>")
