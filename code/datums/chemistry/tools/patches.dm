		...
		if (iscarbon(M) || iscritter(M))
			if (M == user)
				...
				if (medical == 0)
					...
					user.visible_message("<span style=\"color:orange\"><b>[user]</b> stamps [src] on [M].</span>",\
					"<span style=\"color:orange\">You stamp [src] on [M].</span>")
					...
					user.visible_message("<span style=\"color:orange\"><b>[user]</b> applies [src] to [M].</span>",\
					"<span style=\"color:orange\">You apply [src] to [M].</span>")
...
/obj/item/patch_stack
	...
	attack_self(var/mob/user)
		if (patches.len)
			...
			patches -= P
			update_overlay()
			boutput(user, "<span style=\"color:orange\">You remove [P] from the stack.</span>")
		else
	...
	afterattack(var/atom/movable/target, var/mob/user)
		if (istype(target, /obj/item/reagent_containers/patch))
			if (target.loc != src && (!isrobot(user) || target.loc != user))
				...
				patches += target
				update_overlay()
				boutput(user, "<span style=\"color:orange\">You add [target] to the stack.</span>")
