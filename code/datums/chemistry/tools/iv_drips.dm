	...
	attack(mob/living/carbon/M as mob, mob/living/carbon/user as mob)
		...
		if (in_use && src.patient)
			if (src.patient != H)
				...
				H.tri_message("<span style=\"color:orange\"><b>[user]</b> removes [src]'s needle from [H == user ? "[H.gender == "male" ? "his" : "her"]" : "[H]'s"] arm.</span>",\
				user, "<span style=\"color:orange\">You remove [src]'s needle from [H == user ? "your" : "[H]'s"] arm.</span>",\
				H, "<span style=\"color:orange\">[H == user ? "You remove" : "<b>[user]</b> removes"] [src]'s needle from your arm.</span>")
			...
			H.tri_message("<span style=\"color:orange\"><b>[user]</b> begins inserting [src]'s needle into [H == user ? "[H.gender == "male" ? "his" : "her"]" : "[H]'s"] arm.</span>",\
			user, "<span style=\"color:orange\">You begin inserting [src]'s needle into [H == user ? "your" : "[H]'s"] arm.</span>",\
			H, "<span style=\"color:orange\">[H == user ? "You begin" : "<b>[user]</b> begins"] inserting [src]'s needle into your arm.</span>")
			...
			src.patient = H
			H.tri_message("<span style=\"color:orange\"><b>[user]</b> inserts [src]'s needle into [H == user ? "[H.gender == "male" ? "his" : "her"]" : "[H]'s"] arm.</span>",\
			user, "<span style=\"color:orange\">You insert [src]'s needle into [H == user ? "your" : "[H]'s"] arm.</span>",\
			H, "<span style=\"color:orange\">[H == user ? "You insert" : "<b>[user]</b> inserts"] [src]'s needle into your arm.</span>")
	...
	process(var/mob/living/carbon/human/H as mob)
		...
		if (src.mode == IV_INJECT)
			if (src.patient.reagents.is_full())
				src.patient.visible_message("<span style=\"color:orange\"><b>[src.patient]</b>'s transfusion finishes.</span>",\
				"<span style=\"color:orange\">Your transfusion finishes.</span>")
				src.stop_transfusion()
				return
			if (!src.reagents.total_volume)
			...
			if (src.reagents.is_full())
				src.patient.visible_message("<span style=\"color:orange\">[src] fills up and stops drawing blood from [src.patient].</span>",\
				"<span style=\"color:orange\">[src] fills up and stops drawing blood from you.</span>")
				src.stop_transfusion()
				return
			// Vampires can't use this trick to inflate their blood count, because they can't get more than ~30% of it back.
			// Also ignore that second container of blood entirely if it's a vampire (Convair880).
...
/obj/iv_stand
	...
	attackby(obj/item/W, mob/user)
		if (!src.IV && istype(W, /obj/item/reagent_containers/iv_drip))
			if (isrobot(user)) // are they a borg? it's probably a mediborg's IV then, don't take that!
				return
			user.visible_message("<span style=\"color:orange\">[user] hangs [W] on [src].</span>",\
			"<span style=\"color:orange\">You hang [W] on [src].</span>")
			user.u_equip(W)
			W.set_loc(src)
			src.IV = W
			W:stand = src
	...
	attack_hand(mob/user as mob)
		if (src.IV && !isrobot(user))
			...
			user.visible_message("<span style=\"color:orange\">[user] takes [oldIV] down from [src].</span>",\
			"<span style=\"color:orange\">You take [oldIV] down from [src].</span>")
