...
/obj/item/reagent_containers/hypospray
	...
	attack(mob/M as mob, mob/user as mob, def_zone)
		...
		if (ismob(M))
			...
			user.visible_message("<span style=\"color:orange\"><B>[user] injects [M] with [min(amt_prop, reagents.total_volume)] units of [src.reagents.get_master_reagent_name()].</B></span>",\
			"<span style=\"color:orange\">You inject [min(amt_prop, reagents.total_volume)] units of [src.reagents.get_master_reagent_name()]. [src] now contains [max(0,(src.reagents.total_volume-amt_prop))] units.</span>")
