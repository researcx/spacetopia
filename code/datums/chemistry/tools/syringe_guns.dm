...
/obj/item/gun/syringe
	...
	get_desc(dist)
		if (dist > 2)
			return
		. = "<br>[round(reagents.total_volume/15)] / [round(reagents.maximum_volume/15)] shots available.<br><span style=\"color:orange\">The internal reservoir contains:</span>"
		if (src.reagents.reagent_list.len)
			for (var/current_id in src.reagents.reagent_list)
				...
				. += "<br><span style=\"color:orange\">&emsp; [current_reagent.volume] units of [current_reagent.name]</span>"
		else
			. += "<br><span style=\"color:orange\">&emsp; Nothing</span>"
