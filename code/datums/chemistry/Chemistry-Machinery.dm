...
/obj/submachine/chef_sink/table
	name="table sink"
	icon = 'icons/obj/chemical.dmi'
	icon_state="table_sink"

/obj/submachine/chef_sink/alt
	density = 0
	icon = 'icons/obj/chemical.dmi'
...
/obj/machinery/chem_heater/
	...
	proc/roboactive(var/obj/item/reagent_containers/glass/B, var/mob/user, var/temperature)
		...
		boutput(user, "<span style=\"color:orange\">The temperature of [B] is now [ROB.total_temperature] degrees.</span>")
		...
		if(abs(ROB.total_temperature - temperature) <= 3)
			boutput(user, "<span style=\"color:orange\">The [src] has finished!</span>")
			src.power_usage = 50
			roboworking = 0
			return
