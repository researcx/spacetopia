...
/datum/chemicompiler_executor/proc/reservoirClick(resId)
	if(istype(reservoirs[resId], /obj/item/reagent_containers/glass))
		// Taking a res out
		if(!usr.equipped())
			boutput(usr, "<span style=\"color:orange\">You remove the [reservoirs[resId]] from the [src.holder].</span>")
		...
		if(istype(I, /obj/item/reagent_containers/glass))
			...
			boutput(usr, "<span style=\"color:orange\">You place the [I] into the [src.holder].</span>")
			usr.drop_item()
			I.set_loc(holder)
			reservoirs[resId] = I
