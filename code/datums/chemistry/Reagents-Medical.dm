			...
			reaction_mob(var/mob/M, var/method=TOUCH, var/volume_passed)
				...
				if(method == TOUCH)
					...
					if (istype(M, /mob/living/carbon/human))
						...
						if (H.bleeding)
							H.bleeding = 0
					boutput(M, "<span style=\"color:orange\">The synthetic flesh integrates itself into your wounds, healing you.</span>")
					M.UpdateDamageIcon()
					M.updatehealth()
		...
		medical/silver_sulfadiazine // COGWERKS CHEM REVISION PROJECT. marked for revision
			...
			reaction_mob(var/mob/M, var/method=TOUCH, var/volume_passed)
				...
				if (method == TOUCH)
					...
					boutput(M, "<span style=\"color:orange\">The silver sulfadiazine soothes your burns.</span>")
					M.UpdateDamageIcon()
					M.updatehealth()
		...
		medical/antihistamine
			...
			on_mob_life(var/mob/M)
				...
				if(prob(3))
					M.stunned += 2
					M.drowsyness++
					M.visible_message("<span style=\"color:orange\"><b>[M.name]<b> looks a bit dazed.</span>")
				..(M)
				return
		...
		medical/stypic_powder // // COGWERKS CHEM REVISION PROJECT. marked for revision
			...
			reaction_mob(var/mob/M, var/method=TOUCH, var/volume_passed)
				...
				if(method == TOUCH)
					...
					boutput(M, "<span style=\"color:orange\">The styptic powder stings like hell as it closes some of your wounds.</span>")
					M.emote("scream")
					M.UpdateDamageIcon()
					M.updatehealth()
				else if(method == INGEST)
