	...
	reagents
						...
						if (!old_reactions.Find(C))
							...
							if (my_atom)
								for(var/mob/living/M in AIviewers(4, get_turf(my_atom)) )	//Fuck you, ghosts
									if (C.mix_phrase) boutput(M, "<span style=\"color:orange\">[bicon(my_atom)] [C.mix_phrase]</span>")
		...
		proc/get_description(mob/user, rc_flags=0)
			...
			if(reagent_list.len)
				...
				. += "<span style=\"color:orange\">Nothing in it.</span>"
			return
		...
		proc/get_inexact_description(var/rc_flags=0)
			...
			if(full_text == "empty")
				if(rc_flags & (RC_SCALE | RC_VISIBLE | RC_FULLNESS) )
					desc = "<span style=\"color:orange\">It is empty.</span>"
				return desc
			...
			if(rc_flags & RC_VISIBLE)
				if(rc_flags & RC_SCALE)
					desc += "<span style=\"color:orange\">It contains [total_volume] units of \a [t]-colored [state_text].</span>"
				else
					desc += "<span style=\"color:orange\">It is [full_text] of \a [t]-colored [state_text].</span>"
			else
				if(rc_flags & RC_SCALE)
					desc += "<span style=\"color:orange\">It contains [total_volume] units.</span>"
				else
					if(rc_flags & RC_FULLNESS)
						desc += "<span style=\"color:orange\">It is [full_text].</span>"
