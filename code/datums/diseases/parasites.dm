...
/datum/ailment/parasite/spidereggs/surgery(var/mob/living/surgeon, var/mob/living/affected_mob, var/datum/ailment_data/D)
	...
	switch (outcome)
		...
		if (16 to 60)
			...
			var/success = 0
			if (prob(50))
				around_msg = "<span style=\"color:orange\"><b>[surgeon] cuts open [affected_mob] and removes some [name].</b></span>"
				self_msg = "<span style=\"color:orange\">You remove some [name] from [affected_mob]. You can still see some of it in there, though.</span>"
			else
				around_msg = "<span style=\"color:orange\"><b>[surgeon] cuts open [affected_mob] and removes the remaining [name].</b></span>"
				self_msg = "<span style=\"color:orange\">You remove the remaining [name] from [affected_mob].</span>"
				success = 1
			...
			return success
		if (61 to INFINITY)
			surgeon.visible_message("<span style=\"color:orange\"><b>[surgeon] cuts open [affected_mob] and removes all traces of [name]</b></span>", "<span style=\"color:orange\">You masterfully remove the [name] from [affected_mob].</span>")
			if (!numb)
				affected_mob.show_message("<span style=\"color:red\"><b>You feel a mild stabbing pain in your chest!</b></span>")
				affected_mob.TakeDamage("chest", 10, 0, 0, DAMAGE_STAB)
				affected_mob.updatehealth()
