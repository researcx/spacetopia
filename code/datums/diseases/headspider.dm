...
/datum/ailment/parasite/headspider/surgery(var/mob/living/surgeon, var/mob/living/affected_mob, var/datum/ailment_data/D)
	...
	switch (outcome)
		...
		if (16 to 60)
			...
			var/success = 0
			if (prob(50))
				around_msg = "<span style=\"color:orange\"><b>[surgeon] cuts open [affected_mob] and removes a part of the headspider.</b></span>"
				self_msg = "<span style=\"color:orange\">You remove some bits of the headspider from [affected_mob], but it quickly regrows them.</span>"
			else
				around_msg = "<span style=\"color:orange\"><b>[surgeon] cuts open [affected_mob] and removes the entire headspider.</b></span>"
				self_msg = "<span style=\"color:orange\">You remove the remaining headspider from [affected_mob].</span>"
				success = 1
			...
			return success
		if (61 to INFINITY)
			surgeon.visible_message("<span style=\"color:orange\"><b>[surgeon] cuts open [affected_mob] and removes all traces of the headspider.</b></span>", "<span style=\"color:orange\">You masterfully remove the headspider from [affected_mob].</span>")
			if (!numb)
				affected_mob.show_message("<span style=\"color:red\"><b>You feel a mild stabbing pain in your chest!</b></span>")
				affected_mob.TakeDamage("chest", 10, 0, 0, DAMAGE_STAB)
				affected_mob.updatehealth()
