...
/datum/ailment/addiction/stage_act(var/mob/living/affected_mob,var/datum/ailment_data/addiction/D)
	...
	if(prob(20) && (world.timeofday > (D.last_reagent_dose + D.withdrawal_duration)))
		boutput(affected_mob, "<span style=\"color:orange\">You no longer feel reliant on [D.associated_reagent]!</span>")
	...
	switch(D.stage)
		if(2)
			...
			if(prob(4))
				boutput(affected_mob, "<span style=\"color:orange\">You feel a dull headache.</span>")
		if(3)
			if(prob(8))
