...
/datum/blob_ability/help
	...
	onUse(var/turf/T)
		...
		if (owner.help_mode)
			...
			owner.help_mode = 1
			boutput(owner, "<span style=\"color:orange\">Help Mode has been activated  To disable it, click on this button again.</span>")
...
/datum/blob_upgrade/reinforce_spread
	...
	take_upgrade()
		...
		for (var/obj/O in deposits)
			qdel(O)
		boutput(usr, "<span style=\"color:orange\">Applying upgrade to the blob...</span>")
		spawn(0)
			var/wg = 0
			for (var/obj/blob/O in owner.blobs)
				...
				if (wg >= 20)
					sleep(1)
					wg = 0
			boutput(usr, "<span style=\"color:orange\">Finished applying material upgrade!</span>")
			taking = 0
		if (!(src in owner.upgrades))
			owner.upgrades += src
		return 0
