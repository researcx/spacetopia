...
/datum/objective/crew
	...
	headofpersonnel
		vanish
			...
			medal_name = "Unperson"
			check_completion()
				if(owner.current && owner.current.stat != 2 && owner.current.z != 1 && !istype(get_area(owner.current),/area/shuttle/escape/centcom)) return 1
				else return 0
	...
	staffassistant
		...
		promotion
			...
			check_completion()
				if(owner.current && owner.current.stat != 2 && ishuman(owner.current))
					var/mob/living/carbon/human/H = owner.current
					if(istype(get_area(H),/area/shuttle/escape/centcom) && H.wear_id && H.wear_id:registered == H.real_name && H.wear_id:assignment != ("Technical Assistant" || "Civilian" || "Medical Assistant")) return 1
					else return 0
		clown
		...
		promotion
			...
			check_completion()
				if(owner.current && owner.current.stat != 2 && istype(get_area(owner.current),/area/shuttle/escape/centcom)) //checking basic stuff - they escaped alive and have an ID
					var/mob/living/carbon/human/H = owner.current
					if(H.wear_id && H.wear_id:registered == H.real_name && H.wear_id:assignment != ("Technical Assistant" || "Civilian" || "Medical Assistant")) return 1
					else return 0
		spacesuit
			explanation_text = "Get your grubby hands on a spacesuit."
			medal_name = "Vacuum Sealed"
