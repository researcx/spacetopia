...
//			src.current_projectile_num = 1
		...
		boutput(user, "<span style=\"color:orange\">you set the output to [src.current_projectile.sname].</span>")
	return
...
/obj/item/pixel_gun/energy
	...
	attackby(obj/item/b as obj, mob/user as mob)
		if(istype(b, /obj/item/ammo/power_cell) && !rechargeable)
			if(src.cell)
				if(b:swap(src))
					boutput(user, "<span style=\"color:orange\">You change the cell in the [src]!</span>")
			else
				src.cell = b
				user.drop_item()
				b.set_loc(src)
				boutput(user, "<span style=\"color:orange\">You load the [src]!</span>")
		else
			..()
