...
/obj/machinery/computer/ATM
	...
	attackby(var/obj/item/I as obj, user as mob)
		...
		if(istype(I, /obj/item/card/id))
			boutput(user, "<span style=\"color:orange\">You swipe your ID card in the ATM.</span>")
			src.scan = I
		if(istype(I, /obj/item/spacecash/))
			if (src.accessed_record)
				boutput(user, "<span style=\"color:orange\">You insert the cash into the ATM.</span>")
				if (src.doTransaction(usr.client.ckey,"[I.amount]","ATM Deposit") == 1)
					src.accessed_record.fields["current_money"] += I.amount
					I.amount = 0
					qdel(I)
				else
					boutput(user, "<span style=\"color:orange\">The ATM rejects your cash.</span>")
			else boutput(user, "<span style=\"color:red\">You need to log in before depositing cash!</span>")
		if(istype(I, /obj/item/lotteryTicket))
			if (src.accessed_record)
				boutput(user, "<span style=\"color:orange\">You insert the lottery ticket into the ATM.</span>")
				if(I:winner)
					boutput(user, "<span style=\"color:orange\">Congratulations, this ticket is a winner netting you [I:winner] credits</span>")
	...
	attack_hand(var/mob/user as mob)
		...
		user.machine = src
		var/dat = "<head><title>Automated Teller Machine</title>[css_interfaces]</head><body>"
		...
		switch(src.state)
			...
			if(STATE_LOGGEDIN)
				if(!src.accessed_record)
					...
					if (src.scan)
						dat += "<BR>Your bank balance is: &ETH; [src.accessed_record.fields["current_money"]]."
						dat += "<BR>Your balance on your card is: &ETH; [src.scan.money]"
	...
	proc/TryToFindRecord()
		for(var/datum/data/record/B in data_core.bank)
			if(src.scan && (B.fields["name"] == user.real_name) )
		...
		switch(href_list["operation"])
			...
			if("withdraw")
				...
				if(amount > src.accessed_record.fields["current_money"])
					...
					if (src.doTransaction(usr.client.ckey,"-[amount]","ATM Withdrawal") == 1)
						src.scan.money += amount
						src.accessed_record.fields["current_money"] -= amount
					else
						boutput(usr, "<span style=\"color:red\">Failed to complete transfer. Possible problem with banking system.</span>")
			...
			if("deposit")
				...
				if(amount > src.scan.money)
					...
					if (src.doTransaction(usr.client.ckey,"[amount]","ATM Deposit") == 1)
						src.scan.money -= amount
						src.accessed_record.fields["current_money"] += amount
					else
						boutput(usr, "<span style=\"color:red\">Failed to complete transfer. Possible problem with banking system.</span>")
...
/obj/machinery/computer/bank_data
	...
	attack_hand(mob/user as mob)
			...
			if (src.authenticated)
				switch(src.screen)
					if(1.0)
						...
						dat += {"
						<u><b>Total Station Funds:</b> &ETH;[num2text(totalfunds,50)]</u>
						<BR>
						<BR><b>Current Payroll Budget:</b> &ETH;[num2text(wagesystem.station_budget,50)]
						<BR><b>Current Research Budget:</b> &ETH;[num2text(wagesystem.research_budget,50)]
						<BR><b>Current Shipping Budget:</b> &ETH;[num2text(wagesystem.shipping_budget,50)]
						<BR>
						<b>Current Payroll Cost:</b> &ETH;[payroll]
						<BR>
		...
		user << browse(text("<HEAD><TITLE>Bank Records</TITLE>[css_interfaces]</head><TT>[]</TT>", dat), "window=secure_bank")
		...
		if ((usr.contents.Find(src) || (in_range(src, usr) && istype(src.loc, /turf))) || (istype(usr, /mob/living/silicon)))
			...
			if (src.authenticated)
				if (href_list["list"])
					...
					if (t1 > 10000)
						t1 = 10000
						boutput(usr, "<span style=\"color:red\">Maximum wage is &ETH;10,000.</span>")
					R.fields["wage"] = t1
					...
					if (t2 == "Withdraw")
						...
						boutput(usr, "<span style=\"color:orange\">&ETH;[t1] added to station budget from [R.fields["name"]]'s account.</span>")
					else if (t2 == "Deposit")
						...
						R.fields["current_money"] += t1
						wagesystem.station_budget -= t1
						boutput(usr, "<span style=\"color:orange\">&ETH;[t1] added to [R.fields["name"]]'s account from station budget.</span>")
...
/obj/submachine/ATM
	...
	attack_hand(var/mob/user as mob)
		...
		user.machine = src
		var/dat = "<head><title>ATM</title>[css_interfaces]</head><body style=\"text-align: center;\">"
		dat += "<div class=\"atm\">"

		switch(src.state)
			if(STATE_LOGGEDOFF)
				dat += "<BR><BR><BR><A HREF='?src=\ref[src];operation=enterpin'>Scan your fingerprint to begin</A>"

			if(STATE_LOGGEDIN)
				dat += "<BR><BR><b>Account Balance:</b> &ETH;[src.accessed_record.fields["current_money"]]<BR>"
				//dat += "<BR>Your balance on your card is: &ETH;[src.scan.money]"
				//dat += "<BR><BR><A HREF='?src=\ref[src];operation=withdraw'>Withdraw to Card</A>"
				dat += "<BR><A HREF='?src=\ref[src];operation=withdrawcash'>Withdraw Cash</A>"
		...
		dat += "<BR><A HREF='?action=mach_close&window=atm'>Close</A>"
		dat += "</div>"
		user << browse(dat, "window=atm;size=400x300")
		onclose(user, "atm")


	proc/TryToFindRecord()
		for(var/datum/data/record/B in data_core.bank)
			world << "usrname : [usr.real_name]"
			if(B.fields["name"] == usr.real_name)
				src.accessed_record = B
				return 1
		return 0
	...
	Topic(href, href_list)
		...
		switch(href_list["operation"])
			...
			if ("enterpin")
				if(TryToFindRecord(usr))
					//world.log << "[usr.real_name] + [TryToFindRecord(usr)]"
					src.state = STATE_LOGGEDIN
				else
					boutput(usr, "<span style=\"color:red\">No bank record found.</span>")
			...
			if("withdraw")
				if (usr.real_name in FrozenAccounts)
