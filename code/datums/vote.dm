...
/mob/verb/vote()
	...
	var/text = "<HTML><HEAD><TITLE>Voting</TITLE>[css_interfaces]</head><BODY scroll=no>"
	...
	if(vote.voting)
		...
		if(shuttlecoming == 1)
			boutput(usr, "<span style=\"color:orange\">Cannot start Vote - Shuttle has been called.</span>")
			return
