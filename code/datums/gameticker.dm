...
/datum/controller/gameticker/proc/pregame()
	pregame_timeleft = 1 // raisekd from 120 to accomodate the v500 ads
...
//	else
	...
	if (map_setting == "SPACETOPIA")
		ooc_allowed = 0
		boutput(world, "<B>OOC has been automatically disabled.</B>")
	...
	spawn(0)
		...
		if (istype(random_events,/datum/event_controller/))
			spawn(random_events.minor_events_begin)
				message_admins("<span style=\"color:orange\">Minor Event cycle has been started.</span>")
				random_events.minor_event_cycle()
			spawn(random_events.events_begin)
				message_admins("<span style=\"color:orange\">Random Event cycle has been started.</span>")
		...
		//boutput(world, "<B><FONT color='lime'>The game has started!</FONT></B>")
		//boutput(world, "<span style=\"color:orange\"><b>Tip:</b> [pick(tips)]</span>")
...
/datum/controller/gameticker
	...
	proc/process()
		...
		if(mode.check_finished())
			...
			spawn(50)
				boutput(world, "<span style=\"color:orange\"><B>Restarting soon</B></span>")
