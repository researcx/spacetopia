...
/client/proc/call_proc_all(var/typename as null|text)
	...
	if (thetype)
		...
		boutput(usr, "<span style=\"color:orange\">'[procname]' called on [counter] instances of '[typename]'</span>")
...
/proc/doCallProc(target = null)
	...
	for(var/i=0, i<argnum, i++)
		...
		if (listargs == null) return
	if (target)
		boutput(usr, "<span style=\"color:orange\">Calling '[procname]' with [listargs.len] arguments on '[target]'</span>")
		if(listargs.len)
			returnval = call(target,procname)(arglist(listargs))
		else
			returnval = call(target,procname)()
	else
		boutput(usr, "<span style=\"color:orange\">Calling '[procname]' with [listargs.len] arguments</span>")
		if(listargs.len)
			returnval = call(procname)(arglist(listargs))
		else
			returnval = call(procname)(arglist(listargs))

	boutput(usr, "<span style=\"color:orange\">Proc returned: [returnval ? returnval : "null"]</span>")
	return
...
/client/proc/cmd_admin_makeai(var/mob/M in world)
	...
	if(ishuman(M))
		...
		if ((istype(S, /obj/landmark/start) && istype(S.loc, /turf)))
			boutput(M, "<span style=\"color:orange\"><B>You have been teleported to your new starting location!</B></span>")
			M.set_loc(S.loc)
			M.buckled = null
...
/client/proc/print_instance(var/atom/theinstance)
	if (isarea(theinstance))
		...
		if (!T)
			boutput(usr, "<span style=\"color:orange\">[theinstance] (no turfs in area).</span>")
		else
			boutput(usr, "<span style=\"color:orange\">[theinstance] including [showMyCoords(T.x, T.y, T.z)].</span>")
	else if (isturf(theinstance))
		boutput(usr, "<span style=\"color:orange\">[theinstance] at [showMyCoords(theinstance.x, theinstance.y, theinstance.z)].</span>")
	else
		...
		while (Q && Q != T)
			in_text += " in [Q]"
			Q = Q.loc
		boutput(usr, "<span style=\"color:orange\">[theinstance][in_text] at [showMyCoords(T.x, T.y, T.z)]</span>")
...
/client/proc/find_one_of(var/typename as text)
	...
	if (thetype)
		...
		if (!theinstance)
			...
			return
		boutput(usr, "<span style=\"color:orange\"><b>Found instance of [thetype]:</b></span>")
...
/client/proc/find_all_of(var/typename as text)
	...
	if (thetype)
		var/counter = 0
		boutput(usr, "<span style=\"color:orange\"><b>All instances of [thetype]: </b></span>")
		for (var/atom/theinstance in world)
			if (!istype(theinstance, thetype))
				continue
			counter++
			print_instance(theinstance)
		boutput(usr, "<span style=\"color:orange\">Found [counter] instances total.</span>")
	else
		boutput(usr, "No type matches for [typename].")
		return
...
/client/proc/find_thing(var/atom/A as null|anything in world)
	...
	boutput(usr, "<span style=\"color:orange\"><b>Located [A] ([A.type]): </b></span>")
	print_instance(A)
	...
	if (thetype)
		...
		for (var/atom/theinstance in world)
			if (!istype(theinstance, thetype))
				continue
			counter++
		boutput(usr, "<span style=\"color:orange\">There are <b>[counter]</b> instances total of [thetype].</span>")
	else
...
/client/proc/toggle_camera_network_reciprocity()
	...
	boutput(usr, "<span style=\"color:orange\">Toggled camera network reciprocity [camera_network_reciprocity ? "on" : "off"]</span>")
...
/client/proc/cmd_randomize_handwriting()
	...
	if (src.mob && src.mob.mind)
		...
		boutput(usr, "<span style=\"color:orange\">Handwriting style is now: [src.mob.mind.handwriting]</span>")
