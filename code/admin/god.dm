...
/client/proc/become_god(godName as text)
	...
	if(!(usr.client in god_list))
		if(!length(god_list))
			...
			boutput(world, "<span style=\"color:orange\">You may now pray in the Chapel.</span>")
...
/client/proc/revoke_god()
	...
	if(usr.client in god_list)
		...
		if(!length(god_list))
			...
			world << sound('sound/effects/thunder.ogg', volume = 80)
			boutput(world, "<span style=\"color:orange\">You cannot pray anymore</span>")
...
/mob/living/verb/pray(message as text)
	...
	for(var/god in god_list)
		boutput(god, "<span style=\"color:orange\"><B>PRAYER [usr.name]/[usr.key]</B>: [message]</span>")