	...
	for (var/mob/M in mobs)
		if (M.client && M.client.holder)
			if (M.client.player_mode && !M.client.player_mode_ahelp)
				...
				boutput(M, "<span style=\"color:orange\"><font size='3'><b><span style='color: red'>HELP: </span>[key_name(src,0,0)][(src.real_name ? "/"+src.real_name : "")] <A HREF='?src=\ref[M.client.holder];action=adminplayeropts;targetckey=[src.client.ckey]' class='popt'><i class='icon-info-sign'></i></A></b>: [msg]</font></span>")

#ifdef DATALOGGER
	game_stats.Increment("adminhelps")
	game_stats.ScanText(msg)
#endif
	boutput(usr, "<span style=\"color:orange\"><font size='3'><b><span style='color: red'>HELP: </span> You</b>: [msg]</font></span>")
	...
	for (var/mob/M in mobs)
		if (M.client && M.client.holder)
			if (!M.client.holder.hear_prayers)
				...
				boutput(M, "<span style=\"color:orange\"><B>PRAYER: </B><a href='?src=\ref[M.client.holder];action=subtlemsg&targetckey=[usr.ckey]'>[usr.key]</a> / [usr.real_name ? usr.real_name : usr.name] <A HREF='?src=\ref[M.client.holder];action=adminplayeropts;targetckey=[src.client.ckey]' class='popt'><i class='icon-info-sign'>: <I>[msg]</I></span>")