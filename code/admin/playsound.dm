...
/client/proc/play_sound(S as sound, var/vol as num)
	...
	if (src.holder.rank == "Host" || src.holder.rank == "Coder" || src.holder.rank == "Shit Person")
		...
		if (src.djmode)
			if (src.stealth) boutput(world, "<span class=\"medal\"><b>Now Playing:</b></span> <span style=\"color:orange\">[S]</span>")
			else boutput(world, "<span class=\"medal\"><b>[src.alt_key ? "[src.fakekey]" : "[src.key]"] played:</b></span> <span style=\"color:orange\">[S]</span>")
	else
		if (usr.client.canplaysound)
			...
			if (src.djmode)
				...
				boutput(world, "<span class=\"medal\"><b>[admin_key(src, 1)] Played:</b></span> <span style=\"color:orange\">[S]</span>")
		else
	...
	if (src.djmode)
		if (src.stealth) boutput(world, "<span class=\"medal\"><b>Now Playing (your volume: [vol]):</b></span> <span style=\"color:orange\">[S]</span>")
		else boutput(world, "<span class=\"medal\"><b>[src.alt_key ? "[src.fakekey]" : "[src.key]"] played (your volume: [vol]):</b></span> <span style=\"color:orange\">[S]</span>")
#endif
		...
		for (var/client/C in clients)
			...
			if (vol == 0)
				if (src.djmode || src.non_admin_dj)
					boutput(C, "<span class=\"medal\"><b>[src.key] played (your volume: 0):</b></span> <span style=\"color:orange\">[S]</span>")
				continue
			C << sound(uploaded_sound,wait=0,channel=admin_sound_channel,volume=vol)
			//////DEBUG("Playing sound for [C] on channel [admin_sound_channel]")
			if (src.djmode || src.non_admin_dj)
				boutput(C, "<span class=\"medal\"><b>[src.key] played (your volume: [vol]):</b></span> <span style=\"color:orange\">[S]</span>")
			sleep(1)
		move_admin_sound_channel()
...
/client/proc/non_admin_dj(S as sound)
	...
	if (src.play_music_real(S))
		boutput(src, "<span style=\"color:orange\">Loading music [S]...</span>")
