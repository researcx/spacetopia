...
/proc/addBan(step = 1, data)
	...
	if (step == 2 || !centralConn)
		...
		if (targetC)
			...
			boutput(targetC, "<span style=\"color:red\">To try to resolve this matter head to IRC at https://spacetopia.pw</span>")
		else
		...
		if (expiry == 0)
			...
			var/adminMsg = "<span style=\"color:orange\">"
...
/proc/editBan(step = 1, data)
	...
	if (step == 2 || !centralConn)
		...
		message_admins("<span style=\"color:orange\">[key_name(adminC)] edited [target]'s ban. Reason: [row["reason"]] Duration: [(expiry == 0 ? "Permanent": "[expiry]")]</span>")
...
/proc/deleteBan(step = 1, data)
	...
	if (step == 2 || !centralConn)
		...
		if (expired)
			...
			message_admins("<span style=\"color:orange\">Ban expired for [target]</span>")
		else
			logTheThing("admin", adminC, null, "unbanned [row["ckey"]]")
			logTheThing("diary", adminC, null, "unbanned [row["ckey"]]", "admin")
			message_admins("<span style=\"color:orange\">[key_name(adminC)] unbanned [target]</span>")
...
/proc/forceUpdateLocalBans(latestLocalID, latestRemoteID)
	...
	for (var/row in data)
		...
		if (returnData["error"]) //Error returned from the local db jeeeeez aint nothing going our way
			...
			if (returnData["showAdmins"])
				message_admins("<span style=\"color:orange\"><b>Failed for route [type]BanApiFallback</b>: [returnData["error"]]</span>")
	...
	for (var/e = 1, e <= data.len, e++) //each ban
		...
		if (returnData["error"]) //Error returned from the local db jeeeeez aint nothing going our way
			...
			logTheThing("diary", null, null, "<b>Local API Error</b> - Callback failed in [type]BanApiFallback with message: [returnData["error"]]", "debug")
			if (returnData["showAdmins"])
				message_admins("<span style=\"color:orange\"><b>Failed for route [type]BanApiFallback</b>: [returnData[</span>"error"]]")
