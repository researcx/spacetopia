...
/client/proc/cmd_add_to_screen(var/atom/A as obj|mob|turf in world)
	...
	if (chosen == "everyone")
		...
		C.screen += A
		boutput(usr, "<span style=\"color:orange\">Successful.</span>")
...
*/
	...
	if(!istype(src.mob, /mob/dead/observer) && !istype(src.mob, /mob/dead/target_observer))
		src.mob.ghostize()
		boutput(src, "<span style=\"color:orange\">You are now observing</span>")
	else
		boutput(src, "<span style=\"color:orange\">You are already observing!</span>")
...
/client/proc/admin_play()
	...
	if(istype(src.mob, /mob/dead/observer))
		...
		boutput(src, "<span style=\"color:orange\">You are now playing</span>")
	else
		boutput(src, "<span style=\"color:orange\">You are already playing!</span>")
...
/client/proc/play_cytube()
	set name = "Play SyncTube"
	set desc = "Opens a global media player!"
	admin_only

	for(var/mob/M in world)
		if(M.client)
			M << link("https://cytu.be/r/spacetopia-jukebox")
...
/client/proc/warn(var/mob/M in world)
	...
	if(!M.client.warned)
		...
		M.client.warned = 1
		message_admins("<span style=\"color:orange\">[src.ckey] warned [M.ckey].</span>")
	else
		var/addData[] = new()
		addData["ckey"] = M.ckey
...
/client/proc/cmd_admin_grantfullaccess(var/mob/M in world)
	...
	if (istype(M, /mob/living/carbon/human))
		...
		if (H.wear_id)
			...
			message_admins("[key_name(src)] has granted [M.key] full access.")
			id:access = get_access("Captain")
		else
...
/client/proc/view_fingerprints(obj/O as obj in world)
	...
	set category = "Admin"
	admin_only
	if(!islist(O.fingerprintshidden) || !O.fingerprintshidden.len)
