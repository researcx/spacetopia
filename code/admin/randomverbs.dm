...
/client/proc/cmd_admin_prison_unprison(mob/M as mob in world)
	...
	if (M && ismob(M))
		...
		if (A && istype(A, /area/prison/cell_block/wards))
			...
			message_admins("<span style=\"color:orange\">[key_name(usr)] sent [key_name(M)] to the prison zone.</span>")
...
/client/proc/cmd_admin_subtle_message(mob/M as mob in world)
	...
	message_admins("<span style=\"color:orange\"><b>SubtleMessage: [key_name(usr)] <i class='icon-arrow-right'></i> [key_name(M)] : [msg]</b></span>")
...
/client/proc/cmd_admin_plain_message(mob/M as mob in world)
	...
	message_admins("<span style=\"color:orange\"><b>PlainMSG: [key_name(usr)] <i class='icon-arrow-right'></i> [key_name(M)] : [msg]</b></span>")
...
/client/proc/cmd_admin_plain_message_all()
	...
	message_admins("<span style=\"color:orange\">[key_name(usr)] showed a plain message to all</span>")
...
/client/proc/cmd_admin_pm(mob/M as mob in world)
	...
	if(M)
		...
		if (usr.client && usr.client.holder)
			...
			boutput(usr, "<span style=\"color:orange\">Admin PM to-<b>[key_name(M, usr, 1)]</b>: [t]</span>")
		else
			if (M.client && M.client.holder)
				boutput(M, "<span style=\"color:orange\"><b>Reply PM from-<i>[key_name(usr, M, 1)]</i>: [t]</b></span>")
			else
				boutput(M, "<span style=\"color:red\"><b>Reply PM from-<i>[key_name(usr, M, 0)]</i>: [t]</b></span>")
			boutput(usr, "<span style=\"color:orange\">Reply PM to-<b>[key_name(M, usr, 0)]</b>: [t]</span>")
...
/datum/admins/proc/playeropt(mob/M)
	...
	//	if (istype(M, /mob/living/carbon/human/machoman)) sofia macho removal
...
/client/proc/cmd_admin_add_freeform_ai_law()
	...
	for (var/mob/living/silicon/O in mobs)
		if (isghostdrone(O))
			continue
		boutput(O, "<h3><span style=\"color:orange\">New law uploaded by Centcom: [input]</span></h3>")
...
/client/proc/cmd_admin_reset_ai()
	...
	if (alert(src, "Are you sure you want to reset the AI's laws?", "Confirmation", "Yes", "No") == "Yes")
		...
		for(var/mob/living/silicon/O in mobs)
			...
			boutput(O, "<h3><span style=\"color:orange\">Behavior safety chip activated. Laws reset.</span></h3>")
			O.show_laws()
...
/client/proc/flip_view()
	...
	message_admins("<span style=\"color:orange\">[key_name(usr)] set [key_name(selection)]'s viewport orientation to [rotation].</span>")
...
/client/proc/cmd_whois(target as text)
	...
	var/msg = "<span style='color:orange'>"
...
/client/proc/cmd_admin_check_reagents(var/atom/target as null|mob|obj|turf in world)
	...
	if (!target.reagents || !target.reagents.total_volume)
		boutput(usr, "<span style=\"color:orange\"><b>[target] contains no reagents.</b></span>")
		return
	...
	boutput(usr, "<span style=\"color:orange\"><b>[target]'s contents: <i>[final_log]</i>. Temp: <i>[target.reagents.total_temperature] K</i></b></span>") // Added temperature (Convair880).
...
/client/proc/modify_organs(var/mob/living/carbon/human/H as mob in list_humans())
	...
	switch (what2do)
		if ("Replace")
			...
			boutput(usr, "<span style=\"color:orange\">[H]'s [lowertext(organ)] replaced with [created_organ].</span>")
		...
		if ("Drop")
			if (alert(usr, "Are you sure you want [H] to drop their [lowertext(organ)]?", "Confirmation", "Yes", "No") == "Yes")
				H.organHolder.drop_organ(organ)
				boutput(usr, "<span style=\"color:orange\">[H]'s [lowertext(organ)] dropped.</span>")
		...
		if ("Delete")
			if (alert(usr, "Are you sure you want to delete [H]'s [lowertext(organ)]?", "Confirmation", "Yes", "No") == "Yes")
				var/organ2del = H.organHolder.drop_organ(organ)
				qdel(organ2del)
				boutput(usr, "<span style=\"color:orange\">[H]'s [lowertext(organ)] deleted.</span>")
	...
	if (src.holder.level >= LEVEL_MOD)
		if (ismob(M))
			if (alert(usr, "Boot [M]?", "Confirmation", "Yes", "No") == "Yes")
				...
				message_admins("<span style=\"color:orange\">[key_name(usr)] booted [key_name(M)].</span>")
				del(M.client)
	else
