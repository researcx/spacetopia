...
/datum/admins/Topic(href, href_list)
	...
	if (usr.client != src.owner)
		message_admins("<span style=\"color:orange\">[key_name(usr)] has attempted to override the admin panel!</span>")
	...
	switch(href_list["action"])
		...
		if ("toggle_auto_stealth")
			if (src.level >= LEVEL_SA)
				...
				boutput(usr, "<span style=\"color:orange\">Auto Stealth [src.auto_stealth ? "enabled" : "disabled"].</span>")
				if (src.auto_stealth)
					if (src.auto_alt_key)
						src.auto_alt_key = 0
					...
					if (!usr.client.stealth && !isnull(src.auto_stealth_name))
						...
						if (!new_key)
							...
							boutput(usr, "<span style=\"color:orange\">Auto Stealth name removed.</span>")
		...
		if ("toggle_auto_alt_key")
			if (src.level >= LEVEL_SA)
				...
				boutput(usr, "<span style=\"color:orange\">Auto Alt Key [src.auto_alt_key ? "enabled" : "disabled"].</span>")
				if (src.auto_alt_key)
					if (src.auto_stealth)
						src.auto_stealth = 0
					...
					if (!usr.client.alt_key && !isnull(src.auto_alt_key_name))
						...
						if (!new_key)
							...
							boutput(usr, "<span style=\"color:orange\">Auto Alt Key removed.</span>")
							return src.show_pref_window(usr)
			...
			if (src.level >= LEVEL_SA)
				switch(href_list["type"])
					if("1")
						...
						boutput(world, "<span style=\"color:orange\"><B>Alert: The emergency shuttle has been called. It will arrive in [round(emergency_shuttle.timeleft()/60)] minutes.</B></span>")
						logTheThing("admin", usr, null,  "called the Emergency Shuttle")
						logTheThing("diary", usr, null, "called the Emergency Shuttle", "admin")
						message_admins("<span style=\"color:orange\">[key_name(usr)] called the Emergency Shuttle to the station</span>")
					...
					if("2")
						...
						switch(emergency_shuttle.direction)
							...
							if(1)
								...
								boutput(world, "<span style=\"color:orange\"><B>Alert: The shuttle is going back!</B></span>")
								...
								message_admins("<span style=\"color:orange\">[key_name(usr)] recalled the Emergency Shuttle</span>")
			else
		...
		if("edit_shuttle_time")
			if (src.level >= LEVEL_SHITGUY)
				...
				message_admins("<span style=\"color:orange\">[key_name(usr)] edited the Emergency Shuttle's timeleft to [emergency_shuttle.timeleft(, 1)]</span>")
			else
			...
			switch(href_list["type"])
				if("del")
					...
					if(href_list["id"])
						if(alert("Delete This Note?",,"Yes","No") == "No")
							...
							message_admins("<span style=\"color:orange\">[key_name(usr)] deleted note [href_list["id"]] belonging to <A href='?src=%admin_ref%;action=notes&target=[player]'>[player]</A>.</span>")
				...
				if("add")
					...
					if (new_note["note"])
						...
						message_admins("<span style=\"color:orange\">[key_name(usr)] added a note for <A href='?src=%admin_ref%;action=notes&target=[player]'>[player]</A>.</span>")
	...
	//		var/list/alljobs = get_all_jobs()
		...
		if("jobban")
			if (src.level >= LEVEL_SA)
				...
				if (jobban_isbanned(M, job))
					...
					message_admins("<span style=\"color:orange\">[key_name(usr)] unbanned [key_name(M)] from [job]</span>")
					...
					message_admins("<span style=\"color:orange\">[key_name(usr)] banned [key_name(M)] from [job]</span>")
		...
		if ("removejobban")
			if (src.level >= LEVEL_CODER)
				...
				if(t)
					...
					message_admins("<span style=\"color:orange\">[key_name(usr)] removed [t]</span>")
					jobban_remove(t)
			else
		...
		if ("mute")
			if (src.level >= LEVEL_MOD)
				...
				if (ismob(M) && M.client)
					...
					message_admins("<span style=\"color:orange\">[key_name(usr)] has [(muted ? "permanently muted" : "unmuted")] [key_name(M)].</span>")
		...
		if ("tempmute")
			if (src.level >= LEVEL_MOD)
				...
				if (ismob(M))
					...
					message_admins("<span style=\"color:orange\">[key_name(usr)] has [(muted ? "temporarily muted" : "unmuted")] [key_name(M)].</span>")
		...
		if ("banooc")
			if (src.level >= LEVEL_MOD)
				...
				if (ismob(M) && M.client)
					...
					message_admins("<span style=\"color:orange\">[key_name(usr)] has [(oocbanned ? "OOC Banned" : "OOC Unbanned")] [key_name(M)].</span>")
		...
		if ("c_mode_current")
			if (src.level >= LEVEL_SA)
				...
				if (requestedMode in valid_modes)
					...
					message_admins("<span style=\"color:orange\">[key_name(usr)] set the mode as [requestedMode].</span>")
					world.save_mode(requestedMode)
					master_mode = requestedMode
					if (alert("Declare mode change to all players?","Mode Change","Yes","No") == "Yes")
						boutput(world, "<span style=\"color:orange\"><b>The mode is now: [requestedMode]</b></span>")
				else
		...
		if ("c_mode_next")
			if (src.level >= LEVEL_SA)
				...
				message_admins("<span style=\"color:orange\">[key_name(usr)] set the next round's mode as [newmode].</span>")
				world.save_mode(newmode)
				if (alert("Declare mode change to all players?","Mode Change","Yes","No") == "Yes")
					boutput(world, "<span style=\"color:orange\"><b>The next round's mode will be: [newmode]</b></span>")
			else
				alert("You need to be at least a Secondary Adminstrator to change the game mode.")
		...
		if ("monkeyone")
			if (src.level >= LEVEL_SA)
				...
				if(istype(M, /mob/living/carbon/human))
					...
					message_admins("<span style=\"color:orange\">[key_name(usr)] attempting to monkeyize [key_name(M)]</span>")
					N.monkeyize()
				else
		...
		if ("forcespeech")
			if (src.level >= LEVEL_PA)
				...
				if (ismob(M))
					...
					message_admins("<span style=\"color:orange\">[key_name(usr)] forced [key_name(M)] to say: [speech]</span>")
			else
		...
		if ("tdome")
			if (src.level >= LEVEL_SA)
				...
				boutput(M, "<span style=\"color:orange\"><b>You have been sent to the Thunderdome. You are on [team].</b></span>")
				boutput(M, "<span style=\"color:orange\"><b>Prepare for combat. If you are not let out of the preparation area within a few minutes, please adminhelp. (F1 key)</b></span>")
				...
				if (which == "cancel")
					...
					if (kind == "normal arm")
						if (which == "left arm" || which == "both arms")
							...
							limb.holder = M
							limb.remove_stage = 0
							M.show_message("<span style=\"color:orange\"><b>Your left arm magically regrows!</b></span>")
						...
						if (which == "right arm" || which == "both arms")
							...
							limb.holder = M
							limb.remove_stage = 0
							M.show_message("<span style=\"color:orange\"><b>Your right arm magically regrows!</b></span>")
					...
					if (kind == "synth arm")
						if (which == "left arm" || which == "both arms")
							...
							limb.holder = M
							limb.remove_stage = 0
							M.show_message("<span style=\"color:orange\"><b>Your left arm magically becomes a synth arm!</b></span>")
						...
						if (which == "right arm" || which == "both arms")
							...
							limb.holder = M
							limb.remove_stage = 0
							M.show_message("<span style=\"color:orange\"><b>Your right arm magically becomes a synth arm!</b></span>")
					...
					if (kind == "blooming synth arm")
						if (which == "left arm" || which == "both arms")
							...
							limb.holder = M
							limb.remove_stage = 0
							M.show_message("<span style=\"color:orange\"><b>Your left arm magically becomes a blooming synth arm!</b></span>")
						...
						if (which == "right arm" || which == "both arms")
							...
							limb.holder = M
							limb.remove_stage = 0
							M.show_message("<span style=\"color:orange\"><b>Your right arm magically becomes a blooming synth arm!</b></span>")
					...
					if (kind == "cyborg arm")
						if (which == "left arm" || which == "both arms")
							...
							limb.holder = M
							limb.remove_stage = 0
							M.show_message("<span style=\"color:orange\"><b>Your left arm magically becomes a cyborg arm!</b></span>")
						...
						if (which == "right arm" || which == "both arms")
							...
							limb.holder = M
							limb.remove_stage = 0
							M.show_message("<span style=\"color:orange\"><b>Your right arm magically becomes a cyborg arm!</b></span>")
					...
					if (kind == "light cyborg arm")
						if (which == "left arm" || which == "both arms")
							...
							limb.holder = M
							limb.remove_stage = 0
							M.show_message("<span style=\"color:orange\"><b>Your left arm magically becomes a light cyborg arm!</b></span>")
						...
						if (which == "right arm" || which == "both arms")
							...
							limb.holder = M
							limb.remove_stage = 0
							M.show_message("<span style=\"color:orange\"><b>Your right arm magically becomes a light cyborg arm!</b></span>")
					...
					if (kind == "wendigo arm")
						if (which == "left arm" || which == "both arms")
							...
							limb.holder = M
							limb.remove_stage = 0
							M.show_message("<span style=\"color:orange\"><b>Your left arm eeriely becomes a wendigo arm!</b></span>")
						...
						if (which == "right arm" || which == "both arms")
							...
							limb.holder = M
							limb.remove_stage = 0
							M.show_message("<span style=\"color:orange\"><b>Your right arm eeriely becomes a wendigo arm!</b></span>")
					...
					if (kind == "bear arm")
						if (which == "left arm" || which == "both arms")
							...
							limb.holder = M
							limb.remove_stage = 0
							M.show_message("<span style=\"color:orange\"><b>Your left arm eeriely becomes a bear arm!</b></span>")
						...
						if (which == "right arm" || which == "both arms")
							...
							limb.holder = M
							limb.remove_stage = 0
							M.show_message("<span style=\"color:orange\"><b>Your right arm eeriely becomes a bear arm!</b></span>")
					...
					if (kind == "abomination arm")
						if (which == "left arm" || which == "both arms")
							...
							limb.holder = M
							limb.remove_stage = 0
							M.show_message("<span style=\"color:orange\"><b>Your left arm becomes a chitinous tendril!</b></span>")
						...
						if (which == "right arm" || which == "both arms")
							...
							limb.holder = M
							limb.remove_stage = 0
							M.show_message("<span style=\"color:orange\"><b>Your right arm becomes a chitinous tendril!</b></span>")
					...
					if (kind == "werewolf arm")
						if (which == "left arm" || which == "both arms")
							...
							limb.holder = M
							limb.remove_stage = 0
							M.show_message("<span style=\"color:orange\"><b>Your left arm becomes a werewolf arm!</b></span>")
						...
						if (which == "right arm" || which == "both arms")
							...
							limb.holder = M
							limb.remove_stage = 0
							M.show_message("<span style=\"color:orange\"><b>Your right arm becomes a werewolf arm!</b></span>")
					...
					if (kind == "predator arm")
						if (which == "left arm" || which == "both arms")
							...
							limb.holder = M
							limb.remove_stage = 0
							M.show_message("<span style=\"color:orange\"><b>Your left arm becomes a predator arm!</b></span>")
						...
						if (which == "right arm" || which == "both arms")
							...
							limb.holder = M
							limb.remove_stage = 0
							M.show_message("<span style=\"color:orange\"><b>Your right arm becomes a predator arm!</b></span>")
						...
						if (which == "left arm" || which == "both arms")
							...
							limb.remove_stage = 0
							limb:set_item(item)
							M.show_message("<span style=\"color:orange\"><b>Your left arm [pick("magically ", "weirdly ", "suddenly ", "unwillingly ", "grodily ", "")]becomes [item]!</b></span>")
						...
						if (which == "right arm" || which == "both arms")
							...
							limb.remove_stage = 0
							limb:set_item(item)
							M.show_message("<span style=\"color:orange\"><b>Your right arm [pick("magically ", "weirdly ", "suddenly ", "unwillingly ", "grodily ", "")]becomes [item]!</b></span>")
					...
					if (type == "sever")
						...
						if (which == "left leg" || which == "both legs")
							...
							limb:holder = M
							limb:remove_stage = 0
							M.show_message("<span style=\"color:orange\"><b>Your left leg becomes a brand new [limb]!</b></span>")
						...
						if (which == "right leg" || which == "both legs")
							...
							limb:holder = M
							limb:remove_stage = 0
							M.show_message("<span style=\"color:orange\"><b>Your right leg becomes a brand new [limb]!</b></span>")
			...
			if(( src.level >= LEVEL_PA ) || ((src.level >= LEVEL_SA) ))
				...
				switch(which)
					...
					if("Ghostdrone")
						droneize(H, 0)
				if(.)
					message_admins("<span style=\"color:orange\">[key_name(usr)] transformed [H.real_name] into a [which].</span>")
			else
...
/*		if ("makemacho") sofia machoman removal
			if( src.level < LEVEL_PA )
		...
		if ("object_list")
			if (src.level >= LEVEL_PA)
				if (config.allow_admin_spawning && (src.state == 2 || src.level >= LEVEL_PA))
					...
					if (number > 5)
						alert("NO MORE THAN 5 AT A TIME ASSHOLE")
						return
					if (number == 1)
		...
		if("giveantagtoken") //Gives player a token they can redeem to guarantee an antagonist role
			if (src.level >= LEVEL_SA)
				...
				if (newtokens <= 0)
					...
					message_admins("<span style=\"color:orange\">[key_name(usr)] removed all antag tokens from [key_name(M)]</span>")
					removedall = 1
				...
				if (!removedall)
					...
					message_admins("<span style=\"color:orange\">[key_name(usr)] [(tokens >= 1) ? "added" : "removed"] [abs(tokens)]  Antag tokens [(tokens >= 1) ? "to" : "from"] [key_name(M)].</span>")
				...
				switch(href_list["type"])
						...
						for(var/mob/living/carbon/human/H in mobs)
							switch(which)
								...
								if("Flashman")
									H.set_mutantrace(/datum/mutantrace/flashy)
							sleep(-1)
						message_admins("<span style=\"color:orange\">[key_name(usr)] transformed everyone into a [which].</span>")
					...
					if("prisonwarp")
						if(!ticker)
							...
							return
						message_admins("<span style=\"color:orange\">[key_name(usr)] teleported all players to the prison zone.</span>")
					...
					if("traitor_all")
						if (src.level >= LEVEL_SA)
							...
							message_admins("<span style=\"color:orange\">[key_name(usr)] made everyone a[hardmode ? " hard-mode" : ""] [which_traitor]. Objective is [custom_objective]</span>")
					...
					if("flicklights")
							...
							for(var/mob/M in mobs)
								if(M.client && M.stat != 2 && prob(25))
									...
									if(AffectedArea.name != "Space" && AffectedArea.name != "Engine Walls" && AffectedArea.name != "Chemical Lab Test Chamber" && AffectedArea.name != "Escape Shuttle" && AffectedArea.name != "Arrival Area" && AffectedArea.name != "Arrival Shuttle" && AffectedArea.name != "start area" && AffectedArea.name != "Engine Combustion Chamber")
										...
										switch(Message)
											if(1)
												M.show_message(text("<span style=\"color:orange\">You shudder as if cold...</span>"), 1)
											if(2)
												M.show_message(text("<span style=\"color:orange\">You feel something gliding across your back...</span>"), 1)
											if(3)
												M.show_message(text("<span style=\"color:orange\">Your eyes twitch, you feel like something you can't see is here...</span>"), 1)
											if(4)
												M.show_message(text("<span style=\"color:orange\">You notice something moving out of the corner of your eye, but nothing is there...</span>"), 1)
						...
						for(var/mob/M in mobs)
							if(M.client && M.stat != 2)
								M.show_message(text("<span style=\"color:orange\">The chilling wind suddenly stops...</span>"), 1)
							sleep(-1)
					if("retardify")
						if (src.level >= LEVEL_SHITGUY)
		...
		if ("secretsdebug")
			if (src.level >= LEVEL_CODER)
				switch(href_list["type"])
					...
					/*if("gauntlet")
						src.owner:debug_variables(gauntlet_controller) colosseum and gauntlet removal sofia
					if("colosseum")
...
/datum/admins/proc/restart()
	...
	if(confirm == "Cancel")
		return
	if(confirm == "Yes")
		boutput(world, "<span style=\"color:red\"><b>Restarting world!</b></span> <span style=\"color:orange\">Initiated by [admin_key(usr.client, 1)]!</span>")
...
/datum/admins/proc/announce()
	...
	if (message)
		...
		boutput(world, "<span style=\"color:orange\"><b>[admin_key(usr.client, 1)] Announces:</b><br>&emsp; [message]</span>")
...
/datum/proc/give_objective(mob/M as mob, var/traitor_type, var/special = null, var/mass_traitor_obj = null, var/mass_traitor_esc = null)
	if (!M || !traitor_type)
	...
	if (!M.client)
		...
		return
	if(checktraitor(M))
		boutput(usr, "<span style=\"color:red\">You already have a job.</span>")
		return
	...
	if(mass_traitor_obj)
		...
		if (ishuman(M) || iscritter(M))
			eligible_objectives = typesof(/datum/objective/regular/)
		else if (issilicon(M))
			eligible_objectives = list(/datum/objective/regular/assassinate,
			/datum/objective/regular/force_evac_time,/datum/objective/regular/gimmick,/datum/objective/escape/hijack,
	...
	if(!mass_traitor_obj)
		logTheThing("admin", usr, M, "has become a[special ? " [special]" : ""] [traitor_type].")
		logTheThing("diary", usr, M, "has become a[special ? " [special]" : ""] [traitor_type].", "admin")
		message_admins("<span style=\"color:orange\">[key_name(usr)] has become a[special ? " [special]" : ""] [traitor_type].</span>")
	return
...
/datum/admins/proc/evilize(mob/M as mob, var/traitor_type, var/special = null, var/mass_traitor_obj = null, var/mass_traitor_esc = null)
	...
	if(!mass_traitor_obj)
		...
		message_admins("<span style=\"color:orange\">[key_name(usr)] has made [key_name(M)] a[special ? " [special]" : ""] [traitor_type].</span>")
	return
