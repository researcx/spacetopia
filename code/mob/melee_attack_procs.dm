...
/mob/proc/shake_awake(var/mob/living/target)
	...
	src.visible_message("<span style=\"color:orange\">[src] shakes [target], trying to wake them up!</span>")
	target.hit_twitch()
...
/mob/living/carbon/human/administer_CPR(var/mob/living/carbon/human/target)
	...
	if (target.head && (target.head.c_flags & 4))
		boutput(src, "<span style=\"color:orange\">You need to take off their headgear before you can give CPR!</span>")
		return

	if (target.wear_mask && !(target.wear_mask.c_flags & 32))
		boutput(src, "<span style=\"color:orange\">You need to take off their facemask before you can give CPR!</span>")
		return

	if (src.head && (src.head.c_flags & 4))
		boutput(src, "<span style=\"color:orange\">You need to take off your headgear before you can give CPR!</span>")
		return

	if (src.wear_mask && !(src.wear_mask.c_flags & 32))
		boutput(src, "<span style=\"color:orange\">You need to take off your facemask before you can give CPR!</span>")
		return
