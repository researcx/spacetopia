...
/mob/living/carbon/human/update_clothing(var/loop_blocker)
	...
	// Bottom
	if (src.bottom)
		wear_sanity_check(src.bottom)
		src.bottom.wear_image.icon_state = "[src.bottom.icon_state]"//[(!( src.lying ) ? null : "2")]"
		src.bottom.wear_image.layer = MOB_CLOTHING_LAYER-0.1
		src.bottom.wear_image.color = src.bottom.color
		src.bottom.wear_image.alpha = src.bottom.alpha

		UpdateOverlays(src.bottom.wear_image, "wear_bottom")
	else
		UpdateOverlays(null, "wear_bottom")
	...
	if (src.socks)
		...
		src.socks.wear_image.layer = MOB_CLOTHING_LAYER-0.2
		if (src.limbs && src.limbs.l_leg && !istype(src.limbs.l_leg, /obj/item/parts/robot_parts))
			src.socks.wear_image.icon_state = "left_[src.socks.icon_state]"
			src.socks.wear_image.color = src.socks.color
			UpdateOverlays(src.socks.wear_image, "wear_socks_l")

		if (src.limbs && src.limbs.r_leg && !istype(src.limbs.r_leg, /obj/item/parts/robot_parts))
			src.socks.wear_image.icon_state = "right_[src.socks.icon_state]"//[!( src.lying ) ? null : "2"]"
			src.socks.wear_image.color = src.socks.color
			src.socks.wear_image.alpha = src.socks.alpha
			UpdateOverlays(src.socks.wear_image, "wear_socks_r")
	else
		UpdateOverlays(null, "wear_socks_l")
		UpdateOverlays(null, "wear_socks_r")

	// Underwear
	if (src.underwear)
		wear_sanity_check(src.underwear)
		src.underwear.wear_image.icon_state = "[src.underwear.icon_state]"//[(!( src.lying ) ? null : "2")]"
		src.underwear.wear_image.layer = MOB_CLOTHING_LAYER-0.3
		src.underwear.wear_image.color = src.underwear.color
		src.underwear.wear_image.alpha = src.underwear.alpha
		UpdateOverlays(src.underwear.wear_image, "wear_underwear")
	else
		UpdateOverlays(null, "wear_underwear")
...
/mob/living/carbon/human/update_face()
	...
	if (!m_race || !m_race.override_eyes)
		...
		//below need layer = MOB_HAIR_LAYER2 set but it's an invalid arg (runtime error: bad arg name 'layer')
	if(aH.customization_first && aH.customization_first != "None")
		if (!m_race || !m_race.override_hair)
			image_cust_one = new/image(file("icons/custom_icons/" + d_hhairs[aH.customization_first]["icon"]), layer = MOB_HAIR_LAYER2)
			//image_cust_one.color = aH.customization_first_color
			if(d_hhairs[aH.customization_first]["options"] == "1")
				image_cust_one.color = aH.customization_first_color
			if(d_hhairs[aH.customization_first]["options"] == "2")
				var/icon/hairicon = icon(file("icons/custom_icons/" + d_hhairs[aH.customization_first]["icon"]))
				hairicon.MapColors(aH.customization_first_color, aH.customization_first_color_2, null, null)
				image_cust_one = new/image(hairicon, layer = MOB_HAIR_LAYER2)

	if(aH.customization_third && aH.customization_third != "None")
		if (!m_race || !m_race.override_detail)
			image_cust_three = new/image(file("icons/custom_icons/" + d_fhairs[aH.customization_third]["icon"]), layer = MOB_HAIR_LAYER2)
			//image_cust_three.color = aH.customization_third_color
			if(d_fhairs[aH.customization_third]["options"] == "1")
				image_cust_three.color = aH.customization_third_color
			if(d_fhairs[aH.customization_third]["options"] == "2")
				var/icon/hairicon = icon(file("icons/custom_icons/" + d_fhairs[aH.customization_third]["icon"]))
				hairicon.MapColors(aH.customization_third_color, aH.customization_third_color_2, null, null)
				image_cust_three = new/image(hairicon, layer = MOB_HAIR_LAYER2)

	if (aH.ears && aH.ears != "None")
		image_cust_two = new/image(file("icons/custom_icons/" + d_ears[aH.ears]["icon"]), layer = MOB_HAIR_LAYER2)
		if(d_ears[aH.ears]["options"] == "1")
			image_cust_two.color = aH.ear_color
		if(d_ears[aH.ears]["options"] == "2")
			var/icon/earsicon = icon(file("icons/custom_icons/" + d_ears[aH.ears]["icon"]))
			earsicon.MapColors(aH.ear_color, aH.ear_detail_color, null, null)
			image_cust_two = new/image(earsicon, layer = MOB_HAIR_LAYER2)
...
/mob/living/carbon/human/update_body()
	...
	if (!src.mutantrace)
		if ((src.bioHolder && !src.bioHolder.HasEffect("fat")) || src.decomp_stage)
			...
			var/skin_tone = src.bioHolder.mobAppearance.s_color
			...
			if (!src.decomp_stage)
				...
				if (src.organHolder && src.organHolder.head)
					...
					else if (src.organHolder.head.donor_appearance && src.organHolder.head.donor_appearance.s_color != skin_tone)
						var/h_skin_tone = src.organHolder.head.donor_appearance.s_color
			...
			if (src.organHolder && src.organHolder.heart)
			 ...
			 //furry shit
			if (src.bioHolder.mobAppearance.tail && src.bioHolder.mobAppearance.tail != "None")
				tail_detail = new/icon("icon" = file("icons/custom_icons/" + d_tails[src.bioHolder.mobAppearance.tail]["icon"]))
				tail_detail.Blend(hex2rgb(src.bioHolder.mobAppearance.tail_color), ICON_MULTIPLY)
				src.body_standing.overlays += tail_detail

			if (src.bioHolder.mobAppearance.chest_detail && src.bioHolder.mobAppearance.chest_detail != "None")
				chest_detail = new/icon("icon" = file("icons/custom_icons/" + d_chests[src.bioHolder.mobAppearance.chest_detail]["icon"]))
				chest_detail.Blend(hex2rgb(src.bioHolder.mobAppearance.chest_detail_color), ICON_MULTIPLY)
				src.body_standing.overlays += chest_detail

			if (src.bioHolder.mobAppearance.face_detail && src.bioHolder.mobAppearance.face_detail != "None")
				face_detail = new/icon("icon" = file("icons/custom_icons/" + d_faces[src.bioHolder.mobAppearance.face_detail]["icon"]))
				face_detail.Blend(hex2rgb(src.bioHolder.mobAppearance.face_detail_color), ICON_MULTIPLY)
				src.body_standing.overlays += face_detail

			if (src.bioHolder.mobAppearance.leg_detail && src.bioHolder.mobAppearance.leg_detail != "None")
				leg_detail_l = new/icon("icon" = file("icons/custom_icons/" + d_legs[src.bioHolder.mobAppearance.leg_detail]["icon"]), "icon_state" = "left")
				leg_detail_l.Blend(hex2rgb(src.bioHolder.mobAppearance.leg_detail_color), ICON_MULTIPLY)

				leg_detail_r = new/icon("icon" = file("icons/custom_icons/" + d_legs[src.bioHolder.mobAppearance.leg_detail]["icon"]), "icon_state" = "right")
				leg_detail_r.Blend(hex2rgb(src.bioHolder.mobAppearance.leg_detail_color), ICON_MULTIPLY)

				src.body_standing.overlays += leg_detail_l
				src.body_standing.overlays += leg_detail_r

			if (src.bioHolder.mobAppearance.arm_detail && src.bioHolder.mobAppearance.arm_detail != "None")
				arm_detail_l = new/icon("icon" = file("icons/custom_icons/" + d_arms[src.bioHolder.mobAppearance.arm_detail]["icon"]), "icon_state" = "left")
				arm_detail_l.Blend(hex2rgb(src.bioHolder.mobAppearance.arm_detail_color), ICON_MULTIPLY)

				arm_detail_r = new/icon("icon" = file("icons/custom_icons/" + d_arms[src.bioHolder.mobAppearance.arm_detail]["icon"]), "icon_state" = "right")
				arm_detail_r.Blend(hex2rgb(src.bioHolder.mobAppearance.arm_detail_color), ICON_MULTIPLY)

				src.body_standing.overlays += arm_detail_l
				src.body_standing.overlays += arm_detail_r
