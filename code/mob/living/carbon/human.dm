...
/mob/living/carbon/human
	...
	var/obj/item/clothing/bottom/bottom = null
//	var/obj/item/device/radio/w_radio = null
	var/obj/item/clothing/shoes/shoes = null
	var/obj/item/clothing/socks/socks = null
	var/obj/item/clothing/underwear/underwear = null
	...
	//furry shit
	var/icon/ears_detail
	var/icon/tail_detail

	var/icon/face_detail
	var/icon/chest_detail

	var/icon/leg_detail_l
	var/icon/leg_detail_r

	var/icon/arm_detail_l
	var/icon/arm_detail_r
...
/mob/living/carbon/human/New()
	...
	//image_cust_two = image('icons/mob/human_hair.dmi', layer = MOB_HAIR_LAYER2) //unused!
...
/datum/human_limbs
	...
	proc/mend(var/howmany = 4)
		...
		if (!l_arm && howmany > 0)
			...
			l_arm.holder = holder
			boutput(holder, "<span style=\"color:orange\">Your left arm regrows!</span>")
		...
		if (!r_arm && howmany > 0)
			...
			r_arm.holder = holder
			boutput(holder, "<span style=\"color:orange\">Your right arm regrows!</span>")
		...
		if (!l_leg && howmany > 0)
			...
			l_leg.holder = holder
			boutput(holder, "<span style=\"color:orange\">Your left leg regrows!</span>")
		...
		if (!r_leg && howmany > 0)
			...
			r_leg.holder = holder
			boutput(holder, "<span style=\"color:orange\">Your right leg regrows!</span>")
	...
	proc/replace_with(var/target, var/new_type, var/mob/user)
		...
		if (istext(target) && ispath(new_type))
			if (target == "both_arms" || target == "l_arm")
				...
				src.holder.show_message("<span style=\"color:orange\"><b>Your left arm [pick("magically ", "weirdly ", "suddenly ", "grodily ", "")]becomes [src.l_arm]!</b></span>")
				if (user)
			...
			if (target == "both_arms" || target == "r_arm")
				...
				src.holder.show_message("<span style=\"color:orange\"><b>Your right arm [pick("magically ", "weirdly ", "suddenly ", "grodily ", "")]becomes [src.r_arm]!</b></span>")
				if (user)
			...
			if (target == "both_legs" || target == "l_leg")
				if (ispath(new_type, /obj/item/parts/human_parts/leg) || ispath(new_type, /obj/item/parts/robot_parts/leg))
					...
					src.holder.show_message("<span style=\"color:orange\"><b>Your left leg [pick("magically ", "weirdly ", "suddenly ", "grodily ", "")]becomes [src.l_leg]!</b></span>")
					if (user)
			...
			if (target == "both_legs" || target == "r_leg")
				if (ispath(new_type, /obj/item/parts/human_parts/leg) || ispath(new_type, /obj/item/parts/robot_parts/leg))
					...
					src.holder.show_message("<span style=\"color:orange\"><b>Your right leg [pick("magically ", "weirdly ", "suddenly ", "grodily ", "")]becomes [src.r_leg]!</b></span>")
					if (user)
...
//		if (!src.mutantrace) // wow fucking racist (this is amazing -- sofia)
...
/mob/living/carbon/human/proc/unkillable_respawn(var/antag_removal = 0)
	...
	if (!antag_removal && src.spell_soulguard)
		boutput(src, "<span style=\"color:orange\">Your Soulguard enchantment activates and saves you...</span>")
						...
						if (!src.restrained())
							...
							if (src.reagents)
								if (src.reagents.has_reagent("ants") && src.reagents.has_reagent("mutagen"))
									...
									boutput(src, "<span style=\"color:orange\">The ants arachnify.</span>")
				...
				if (src.emote_check(voluntary))
					if ((src.charges >= 1) && (!muzzled))
						...
						for (var/mob/M in oview(1))
							...
							s.set_up(3, 1, src)
							s.start()
							boutput(M, "<span style=\"color:orange\">BZZZZZZZZZZZT!</span>")
					...
					if (src.organHolder && !src.organHolder.butt)
						...
						if (!fart_on_other)
							switch(rand(1, 42))
								...
								if (39) message = "<B>[src] <span style=\"color:red\">f</span><span style=\"color:orange\">a</span>r<span style=\"color:red\">t</span><span style=\"color:orange\">s</span>!</B>"
			...
			if ("pee", "piss", "urinate")
				if (src.emote_check(voluntary))
					if (sims)
						...
						if (bladder > 75)
							boutput(src, "<span style=\"color:orange\">You don't need to go right now.</span>")
							return
						else if (bladder > 50)
							if (!toilet)
						...
						for (var/obj/item/storage/toilet/T in src.loc)
							message = pick("<B>[src]</B> unzips their pants and pees in the toilet.", "<B>[src]</B> empties their bladder.", "<span style=\"color:orange\">Ahhh, sweet relief.</span>")
							src.urine = 0
							T.clogged += 0.10
							break
					else
...
/mob/living/carbon/human/get_desc()
	...
	if (usr.stat == 0)
		. += "<br><span style=\"color:orange\">You look closely at <B>[src.name]</B>.</span>"
	...
	if (src.writtengender)
		. += "<br><span style=\"color:orange\">[src.name] is <b>[src.writtengender]</b>.</span>"
	...
	if (found)
		if (!ishuman(usr))
			...
			. += "<br><span style=\"color:orange\">A servant of His Grace...</span>"
	...
	if (src.w_uniform)
		if (src.w_uniform.blood_DNA)
			...
			. += "<br><span style=\"color:orange\">[src.name] is wearing a [bicon(src.w_uniform)] [src.w_uniform.name].</span>"

	if (src.handcuffed)
		. +=  "<br><span style=\"color:orange\">[src.name] is [bicon(src.handcuffed)] handcuffed!</span>"
	...
	if (src.wear_suit)
		if (src.wear_suit.blood_DNA)
			...
			. += "<br><span style=\"color:orange\">[src.name] has a [bicon(src.wear_suit)] [src.wear_suit.name] on.</span>"

	if (src.ears)
		. += "<br><span style=\"color:orange\">[src.name] has a [bicon(src.ears)] [src.ears.name] by [t_his] mouth.</span>"
	...
	if (src.head)
		if (src.head.blood_DNA)
			...
			. += "<br><span style=\"color:orange\">[src.name] has a [bicon(src.head)] [src.head.name] on [t_his] head.</span>"
	...
	if (src.wear_mask)
		if (src.wear_mask.blood_DNA)
			...
			. += "<br><span style=\"color:orange\">[src.name] has a [bicon(src.wear_mask)] [src.wear_mask.name] on [t_his] face.</span>"
	...
	if (src.glasses)
		if (((src.wear_mask && src.wear_mask.see_face) || !src.wear_mask) && ((src.head && src.head.see_face) || !src.head))
			if (src.glasses.blood_DNA)
				...
				. += "<br><span style=\"color:orange\">[src.name] has a [bicon(src.glasses)] [src.glasses.name] on [t_his] face.</span>"
	...
	if (src.l_hand)
		if (src.l_hand.blood_DNA)
			...
			. += "<br><span style=\"color:orange\">[src.name] has a [bicon(src.l_hand)] [src.l_hand.name] in [t_his] left hand.</span>"
	...
	if (src.r_hand)
		if (src.r_hand.blood_DNA)
			...
			. += "<br><span style=\"color:orange\">[src.name] has a [bicon(src.r_hand)] [src.r_hand.name] in [t_his] right hand.</span>"
	...
	if (src.belt)
		if (src.belt.blood_DNA)
			...
			. += "<br><span style=\"color:orange\">[src.name] has a [bicon(src.belt)] [src.belt.name] on [t_his] belt.</span>"

	if (src.gloves)
		if (src.gloves.blood_DNA)
			. += "<br><span style=\"color:red\">[src.name] has bloody [bicon(src.gloves)] [src.gloves.name] on [t_his] hands!</span>"
		else
			. += "<br><span style=\"color:orange\">[src.name] has [bicon(src.gloves)] [src.gloves.name] on [t_his] hands.</span>"
	else if (src.blood_DNA)
		. += "<br><span style=\"color:red\">[src.name] has[src.blood_DNA ? " bloody " : " "]hands!</span>"

	if (src.back)
		. += "<br><span style=\"color:orange\">[src.name] has a [bicon(src.back)] [src.back.name] on [t_his] back.</span>"
	...
	if (src.wear_id)
		if (istype(src.wear_id, /obj/item/card/id))
			if (src.wear_id:registered != src.real_name && in_range(src, usr) && prob(10))
				...
				. += "<br><span style=\"color:orange\">[src.name] is wearing [bicon(src.wear_id)] [src.wear_id.name].</span>"
			...
			if (src.wear_id:ID_card:registered != src.real_name && in_range(src, usr) && prob(10))
				...
				. += "<br><span style=\"color:orange\">[src.name] is wearing [bicon(src.wear_id)] [src.wear_id.name] with [bicon(src.wear_id:ID_card)] [src.wear_id:ID_card:name] in it.</span>"
		...
		if (src.limbs)
			if (!src.limbs.l_arm)
				...
				if (src.limbs.l_arm:remove_object)
					. += "<br><span style=\"color:orange\">[src.name] has [src.limbs.l_arm.remove_object] attached as a left arm!</span>"
			if (!src.limbs.r_arm)
				...
				if (src.limbs.r_arm:remove_object)
					. += "<br><span style=\"color:orange\">[src.name] has [src.limbs.r_arm.remove_object] attached as a right arm!</span>"
			if (!src.limbs.l_leg)
	...
	if(usr.traitHolder && (usr.traitHolder.hasTrait("observant") || istype(usr, /mob/dead/observer)))
		if(src.traitHolder && src.traitHolder.traits.len)
			. += "<br><span style=\"color:orange\">[src] has the following traits:</span>"
			for(var/X in src.traitHolder.traits)
				var/obj/trait/T = getTraitById(X)
				. += "<br><span style=\"color:orange\">[T.cleanName]</span>"
		else
			. += "<br><span style=\"color:orange\">[src] does not appear to possess any special traits.</span>"
	...
	if (src.juggling())
		...
		. += "<br><span style=\"color:orange\">[src] is juggling [items]!</span>"
...
/mob/living/carbon/human/movement_delay()
	...
	if (src.wear_suit)
		...
		if(src.wear_suit.type == /obj/item/clothing/suit/space)
			if(!istype(src.loc, /turf/space))
				tally += 0.7
		if(src.wear_suit.type == /obj/item/clothing/suit/space/emerg)
			if (!istype(src.loc, /turf/space))
				tally += 2.3
...
/mob/living/carbon/human/proc/throw_item(atom/target)
	...
	//if(stamina < 170)
	//	boutput(src, "<span style=\"color:red\"><b>You are too tired throw!</b></span>")
	//	return
...
/mob/living/carbon/human/find_in_equipment(var/eqtype)
	...
	if (istype(shoes, eqtype))
		return shoes
	if (istype(socks, eqtype))
		return socks
	if (istype(underwear, eqtype))
		return underwear
	if (istype(bottom, eqtype))
		return bottom
		...
		switch(src.place)
			...
			if ("shoes")
				...
				//SN src = null
					...
					return
			if ("socks")
				if (!( src.target.socks ) || !src.target.socks.handle_other_remove(src.source, src.target))
				//SN src = null
					qdel(src)
					return
			if ("underwear")
				if (!( src.target.underwear ) || !src.target.underwear.handle_other_remove(src.source, src.target))
				//SN src = null
					qdel(src)
					return
			if ("bottom")
				if (!( src.target.bottom ) || !src.target.bottom.handle_other_remove(src.source, src.target))
				//SN src = null
					qdel(src)
					return
				/* TODO - DONE
					...
					if (src.place == "drink")
						...
						if (src.place == "dnainjector")
							...
							switch(src.place)
								...
								if ("socks")
									message = "<span style=\"color:red\"><B>[src.source] is trying to take off the [src.target.socks] from [src.target]'s feet!</B></span>"
								if ("underwear")
									message = "<span style=\"color:red\"><B>[src.source] is trying to take off the [src.target.underwear] from [src.target]'s body!</B></span>"
								if ("bottom")
									message = "<span style=\"color:red\"><B>[src.source] is trying to take off the [src.target.bottom] from [src.target]'s legs!</B></span>"
								if ("belt")
...
/obj/equip_e/human/done()
	...
	switch(src.place)
		...
		if ("socks")
			equip_e_slot(slot_socks)
		if ("underwear")
			equip_e_slot(slot_underwear)
		if ("bottom")
			equip_e_slot(slot_bottom)
		if ("l_hand")
...
/mob/living/carbon/human/proc/show_inv(mob/user as mob)
	...
	<BR><B>Socks:</B> <A href='?src=\ref[src];varname=socks;slot=[src.slot_socks];item=socks'>[(src.socks ? src.socks : "Nothing")]</A>
	<BR><B>Underwear:</B> <A href='?src=\ref[src];varname=underwear;slot=[src.slot_underwear];item=underwear'>[(src.underwear ? src.underwear : "Nothing")]</A>
	...
	<BR><B>Trousers:</B> <A href='?src=\ref[src];varname=bottom;slot=[src.slot_bottom];item=bottom'>[(src.bottom ? src.bottom : "Nothing")]</A>
...
//		blood_volume = max(blood_volume, 0)
	...
	/*	else if (istype(loc, /obj/machinery/colosseum_putt)) colosseum removal
			loc_temp = T20C */
		else
			loc_temp = environment.temperature
	...
	proc/handle_blood() // hopefully this won't cause too much lag?
		...
		if (src.bleeding)
			...
			if (src.bleeding <= 3 && prob(2)) // blood does clot and all, but we want bleeding to maybe not stop entirely on its own TOO easily
				src.bleeding --
				boutput(src, "<span style=\"color:orange\">Your wounds feel [fluff].</span>")
		...
		for (var/obj/item/implant/I in src.implant)
			if (istype(I, /obj/item/implant/robust))
				...
				if (src.health < 0)
					...
					if (src.mind) boutput(src, "<span style=\"color:orange\">Your Robusttec-Implant uses all of its remaining energy to save you and deactivates.</span>")
					src.implant -= I
...
/mob/living/carbon/human/whisper(message as text)
	...
	if (src.client && !src.client.holder && url_regex && url_regex.Find(message))
		boutput(src, "<span style=\"color:orange\"><b>Web/BYOND links are not allowed in ingame chat.</b></span>")
...
/mob/living/carbon/human/proc/force_equip(obj/item/I, slot)
	...
	switch(slot)
		if (slot_back)
			if (!src.back)
				src.back = I
				hud.add_other_object(I, ui_back)
				I.equipped(src, "back")
				equipped = 1
		if (slot_wear_mask)
		...
		if (slot_wear_id)
			if (!src.wear_id)
				src.wear_id = I
				hud.add_object(I, HUD_LAYER+2, ui_id)
				I.equipped(src, "id")
				equipped = 1
		if (slot_ears)
		...
		if (slot_socks)
			if (!src.socks)
				src.socks = I
				hud.add_other_object(I, ui_socks)
				I.equipped(src, "socks")
				equipped = 1
		if (slot_underwear)
			if (!src.underwear)
				src.underwear = I
				hud.add_other_object(I, ui_underwear)
				I.equipped(src, "underwear")
				equipped = 1
		if (slot_bottom)
			if (!src.bottom)
				src.bottom = I
				hud.add_other_object(I, ui_bottom)
				I.equipped(src, "bottom")
				equipped = 1
		if (slot_wear_suit)
			if (!src.wear_suit)
				src.wear_suit = I
	...
	switch (slot)
		...
		if (slot_shoes)
			if (istype(I, /obj/item/clothing/shoes))
				...
				if ((src.mutantrace && !src.mutantrace.uses_human_clothes && !SH.compatible_species.Find(src.mutantrace.name)) || (!ismonkey(src) && SH.monkey_clothes))
					...
					return 1
		if (slot_socks)
			if (istype(I, /obj/item/clothing/socks))
				var/obj/item/clothing/SC = I
				if ((src.mutantrace && !src.mutantrace.uses_human_clothes && !SC.compatible_species.Find(src.mutantrace.name)) || (!ismonkey(src) && SC.monkey_clothes))
					////DEBUG("[src] can't wear [I].")
					return 0
				else
					return 1
		if (slot_underwear)
			if (istype(I, /obj/item/clothing/underwear))
				var/obj/item/clothing/UW = I
				if ((src.mutantrace && !src.mutantrace.uses_human_clothes && !UW.compatible_species.Find(src.mutantrace.name)) || (!ismonkey(src) && UW.monkey_clothes))
					////DEBUG("[src] can't wear [I].")
					return 0
				else
					return 1
		if (slot_bottom)
			if (istype(I, /obj/item/clothing/bottom))
				var/obj/item/clothing/SB = I
				if ((src.mutantrace && !src.mutantrace.uses_human_clothes && !SB.compatible_species.Find(src.mutantrace.name)) || (!ismonkey(src) && SB.monkey_clothes))
					////DEBUG("[src] can't wear [I].")
					return 0
				else
					return 1
...
/mob/living/carbon/human/shock(var/atom/origin, var/wattage, var/zone = "chest", var/stun_multiplier = 1, var/ignore_gloves = 0)
	...
	if (src.bioHolder.HasEffect("resist_electric") == 2)
		...
		boutput(src, "<span style=\"color:orange\">You absorb the electrical shock, healing your body!</span>")
		return 0
	else if (src.bioHolder.HasEffect("resist_electric") == 1)
		boutput(src, "<span style=\"color:orange\">You feel electricity course through you harmlessly!</span>")
		return 0
...
/mob/living/carbon/human/proc/resist()
	...
	if (!src.stat && !src.restrained() && (src.shoes && src.shoes.chained))
		if (ishuman(src))
			...
			if (ischangeling(src))
				...
				src.visible_message("<span style=\"color:red\">[src] rips apart the shackles with pure brute strength!</b></span>", "<span style=\"color:orange\">You rip apart the shackles.</span>")
				...
				src.visible_message("<span style=\"color:red\">[src] rips apart the shackles with pure machine-like strength!</b></span>", "<span style=\"color:orange\">You rip apart the shackles.</span>")
	...
	if (src.handcuffed)
		if (ishuman(src))
			if (src.is_changeling())
				boutput(src, "<span style=\"color:orange\">You briefly shrink your hands to remove your handcuffs.</span>")
			...
			if (ispredator(src) || iswerewolf(src))
				...
				boutput(src, "<span style=\"color:orange\">You rip apart your handcuffs.</span>")
...
/mob/living/carbon/human/get_equipped_items()
	...
	if (src.socks) . += src.socks
	if (src.underwear) . += src.underwear
	if (src.bottom) . += src.bottom
