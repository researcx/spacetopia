	...
	if (istype(our_door, /obj/machinery/door/airlock/))
		...
		if (A.canAIControl())
			if (alert("This door is located in [get_area(A)]. Open it?","Airlock: \"[A.name]\"","Yes","No") == "Yes")
				...
				if (get_dist(A, target_name) > 3) // Let's be a bit lenient.
					...
					return
				if (A.open())
					boutput(usr, "<span style=\"color:orange\">[A.name] opened successfully.</span>")
				else
		...
		if (alert("This door is located in [get_area(our_door)]. Open it?","Airlock: \"[our_door.name]\"","Yes","No") == "Yes")
			...
			if (get_dist(our_door, target_name) > 3) // Let's be a bit lenient.
				...
				return
			if (our_door.open())
				boutput(usr, "<span style=\"color:orange\">[our_door.name] opened successfully.</span>")
			else
...
td {
...
</style>[css_interfaces]</head><body><h2>Module editor</h2><h3>Current items</h3><table style='width:100%'><tr><td style='width:80%'><b>Module</b></td><td style='width:10%'>&nbsp;</td><td style='width:10%'>&nbsp;</td></tr>"}
	...
	Topic(href, href_list)
		...
		if (href_list["addcurr"])
			...
			current.loc = D
			current = null
			boutput(usr, "<span style=\"color:orange\">Added item to module!</span>")
		show_interface(usr.client, D)
