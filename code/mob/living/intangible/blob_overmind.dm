...
/mob/living/intangible/blob_overmind
	...
	Life(datum/controller/process/mobs/parent)
		...
		if (blobs.len >= next_evo_point)
			...
			evo_points++
			boutput(src, "<span style=\"color:orange\"><b>You have expanded enough to earn one evo point! You will be granted another at size [next_evo_point]. Good luck!</b></span>")

		if (blobs.len >= next_extra_nucleus)
			next_extra_nucleus += initial(next_extra_nucleus)
			extra_nuclei++
			boutput(src, "<span style=\"color:orange\"><b>You have expanded enough to earn one extra nucleus! You will be granted another at size [next_extra_nucleus]. Good luck!</b></span>")
	...
	clicked(parameters)
		...
		if (upgrade)
			if ((parameters["shift"] || !user.help_mode) && upgrade.check_requirements())
				...
				if (upgrade.check_requirements())
					boutput(user, "<span style=\"color:orange\">Shift-click on this icon to take the upgrade.</span>")
				else
		...
		if (parameters["ctrl"] && ability.targeted)
			...
			if (ability == user.ctrl_power)
				...
				boutput(user, "<span style=\"color:orange\"><b>[ability.name] has been unbound from Ctrl-Click.</b></span>")
				user.update_buttons()
			else
				user.ctrl_power = ability
				boutput(user, "<span style=\"color:orange\"><b>[ability.name] is now bound to Ctrl-Click.</b></span>")
			...
			if (ability == user.alt_power)
				user.alt_power = null
				boutput(user, "<span style=\"color:orange\"><b>[ability.name] has been unbound from Alt-Click.</b></span>")
				user.update_buttons()
			else
				user.alt_power = ability
				boutput(user, "<span style=\"color:orange\"><b>[ability.name] is now bound to Alt-Click.</b></span>")
			...
			if (ability == user.shift_power)
				...
				boutput(user, "<span style=\"color:orange\"><b>[ability.name] has been unbound from Shift-Click.</b></span>")
				user.update_buttons()
			else
				user.shift_power = ability
				boutput(user, "<span style=\"color:orange\"><b>[ability.name] is now bound to Shift-Click.</b></span>")

		else
			if (user.help_mode)
				boutput(user, "<span style=\"color:orange\"><b>This is your [ability.name] ability.</b></span>")
				boutput(user, "<span style=\"color:orange\">It costs [ability.bio_point_cost] bio points to use.</span>")
				if (istype(ability,/datum/blob_ability/build))
					var/datum/blob_ability/build/AB = ability
					boutput(user, "<span style=\"color:orange\">This is a building ability - you need to use it on a regular blob tile.</span>")
					if (AB.gen_rate_invest > 0)
						boutput(user, "<span style=\"color:orange\">This ability requires you to invest [AB.gen_rate_invest] of your BP generation rate in it. It will be returned when the cell is destroyed.</span>")
				boutput(user, "<span style=\"color:orange\">[ability.desc]</span>")
				boutput(user, "<span style=\"color:orange\">Hold down Shift, Ctrl or Alt while clicking the button to set it to that key.</span>")
				boutput(user, "<span style=\"color:orange\">You will then be able to use it freely by holding that button and left-clicking a tile.</span>")
				boutput(user, "<span style=\"color:orange\">Alternatively, you can click with your middle mouse button to use the ability on your current tile.</span>")
				boutput(user, "<span style=\"color:orange\">If you want to swap the places of two buttons on this bar, click and drag one to the position you want it to occupy.</span>")
			else
				ability.onUse()
