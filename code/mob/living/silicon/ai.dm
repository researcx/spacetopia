...
/mob/living/silicon/ai/examine()
	...
	if (isghostdrone(usr))
		return
	boutput(usr, "<span style=\"color:orange\">*---------*</span>")
	boutput(usr, "<span style=\"color:orange\">This is [bicon(src)] <B>[src.name]</B>!</span>")
	if (src.stat == 2)
		...
		if (-1)
			if (istype(src.cell,/obj/item/cell/))
				if (src.cell.charge >= 100)
					boutput(src, "<span style=\"color:orange\"><b>ALERT: Internal power cell has regained sufficient charge to operate. Rebooting.</b></span>")
					src.set_power_mode(1)
				...
				if (istype(APC.cell,/obj/item/cell/))
					if (APC.operating && (APC.equipment != 0))
						if (APC.cell.charge > 100)
							...
							boutput(src, "<span style=\"color:orange\"><b>Local power restored successfully. Location: [APC.area].</b></span>")
						else
...
/mob/living/silicon/ai/proc/ai_view_crew_manifest()
	...
	usr << browse("<head><title>Crew Manifest</title>[css_interfaces]</head><body><tt><b>Crew Manifest:</b><hr>[crew]</tt></body>", "window=aimanifest")
...
/mob/living/silicon/ai/proc/ai_custom_arrival_alert()
	...
	boutput(src, "<span style=\"color:orange\">Arrival alert set to '[newalert]'</span>")
...
/mob/living/silicon/ai/verb/change_network()
	...
	if (src.network == "SS13")
		...
		src.network = "SS13"
	boutput(src, "<span style=\"color:orange\">Switched to [src.network] camera network.</span>")
