	...
	if (src.client && (ticker.round_elapsed_ticks >= 12000))
		...
		if (num_players <= 5)
			if (!emergency_shuttle.online && ticker && ticker.current_state != GAME_STATE_FINISHED && ticker.mode.crew_shortage_enabled)
				...
				boutput(world, "<span style=\"color:orange\"><B>Alert: Due to crew shortages and fatalities, the emergency shuttle has been called. It will arrive in [round(emergency_shuttle.timeleft()/60)] minutes.</B></span>")
...
/mob/living/click(atom/target, params)
	...
	if (params["alt"])
		...
		//src.toggle_examine_mode()
		//src.client.MouseEntered(target, "mapwindow.map", params)
		return
		...
		if (reach || (W && (W.flags & EXTRADELAY))) //Fuck you, magic number prickjerk
			...
			if (src.invisibility > 0 && get_dist(src, target) > 0) // dont want to check for a cloaker every click if we're not invisible
				for (var/obj/item/cloaking_device/I in src)
					if (I.active)
						I.deactivate(src)
						src.visible_message("<span style=\"color:orange\"><b>[src]'s cloak is disrupted!</b></span>")
			if (W && istype(W))
...
/mob/living/proc/toggle_examine_mode(var/force_off = 0)
	if (force_off)
		src.client.examinemode = 0
		src.update_cursor()
		return
	src.client.examinemode = !(src.client.examinemode)
	if(src.client.examinemode)
		boutput(src, "Now examining.")
	else
		boutput(src, "No longer examining.")
	src.update_cursor()
...
/mob/living/verb/toggle_examine()
	set name = "Toggle Examining"
	set category = "Local"
...
/*	if (istype(loc, /obj/machinery/colosseum_putt)) colosseum removal
	...
	/* 		for (var/obj/colosseum_radio/M in hearers(1, src)) colosseum removal
...
/mob/living/verb/change_resolution()
	set category = "Commands"
	set name = "Change Resolution"
	var/selectedview = 7

	if(src.client.view == world.view)
		var/tempview = input(src, "These resolutions are for 32x mode, can be manually adjusted for 64x.", "Resolution Changer")  as null|anything in reslist
		selectedview = reslist[tempview]

		if (selectedview)
			src.client.view = selectedview
			src.see_in_dark = selectedview
		else
...
/mob/living/carbon/human/verb/view_character_sheet(mob/M as mob in view(15))
	set category = "Local"
	...
	if(H.real_name != "Unknown")
		var/output = "<B>[H.real_name]</B><HR>"
		output += "<b>Age:</b> [H.bioHolder.age]<br>"
		output += "<b>Gender:</b> [H.bioHolder.mobAppearance.writtengender]<br><br>"
		if(H.bioHolder.charsheet != "")
			output += "<b>Information:</b><br>"
			output += dd_replacetext(H.bioHolder.charsheet, "\n", "<br>")

		src << browse(output,"window=memory")
	else
		boutput(src, "<span style=\"color:red\">No character sheet available for [H.real_name].</span>")