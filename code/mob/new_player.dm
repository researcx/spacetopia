mob/new_player
	...
	Login()
		...
		src << browse("<script>window.location = \"http://cdn.spacetopia.pw/?ckey=[ckey]&path=\" + location.href.substring(0,location.href.lastIndexOf(\"/\")+1);</script>", "window=mapwindow.titlescreen")
		winshow(src, "mapwindow.titlescreen", 1)
		winshow(src, "window=mapwindow.titlescreen", 1)
		...
		if (src.ckey && !adminspawned)
			if (spawned_in_keys.Find("[src.ckey]"))
				if (!(client && client.holder) && !abandon_allowed)
					...
					close_spawn_windows()
					boutput(src, "<span style=\"color:orange\">Now teleporting.</span>")
	...
	verb/new_player_panel()
		set src = usr
		if(client)
			client.preferences.savefile_load(client, 1)
...
*/
	Stat()
		..()
		statpanel("Players")
		if(client.statpanel=="Players" && ticker)
	...
	Topic(href, href_list[])
		...
		if(href_list["late_join"])
			src.client.spacetopia_authorize()
			LateChoices()

		if(href_list["apply_now"])
			src.client.spacetopia_authorize()
			if (!client.authed)
				src << link("https://spacetopia.pw/register/")
		...
		if(href_list["SelectedJob"])
			...
			if (!enter_allowed)
				boutput(usr, "<span style=\"color:orange\">There is an administrative lock on entering the game!</span>")
				return
	...
	proc/AttemptLateSpawn(var/datum/job/JOB)
		...
		if (JOB && IsJobAvailable(JOB))
			...
			if (ticker && character.mind && JOB.allow_traitors != 0 && prob(10))
				if (map_setting != "SPACETOPIA") // miscreants are causing issue in the rp setting atm
...
// This fxn creates positions for assistants based on existing positions. This could be more elgant.
	proc/LateChoices()
		if (src.client && !src.client.authed)
			boutput(usr, "<span style=\"color:red\">You're not an authorised spacetopia citizen. Unable to join game.</span>")
			return
		var/datum/job/JOB = new /datum/job/civilian/staff_assistant
		//create_character(JOB, 0)
		AttemptLateSpawn(JOB)
	...
	proc/create_character(var/datum/job/J, var/allow_late_antagonist = 0)
		...
		//world.log << "test resolution - plain: [get_selectedview(new_character.client)] in reslist: [reslist[get_selectedview(new_character.client)]]"

		if(isnum(reslist[get_selectedview(new_character.client)]))
			new_character.client.view = reslist[get_selectedview(new_character.client)]
			new_character.see_in_dark = reslist[get_selectedview(new_character.client)]

			//world.log << "setting new res"
	...
	verb/declare_ready()
		...
		if(!ticker || ticker.current_state <= GAME_STATE_PREGAME)
			...
			var/datum/job/civilian/JOB =/datum/job/civilian/staff_assistant
			create_character(JOB)
			src << "[src] attempting spawn as [JOB]"
