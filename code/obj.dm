...
/obj/lattice
	...
	attackby(obj/item/C as obj, mob/user as mob)
		...
		if (istype(C, /obj/item/weldingtool) && C:welding)
			boutput(user, "<span style=\"color:orange\">Slicing lattice joints ...</span>")
			C:eyecheck(user)
		...
		if (istype(C, /obj/item/rods))
			...
			if (R.amount >= 2)
				R.amount -= 2
				boutput(user, "<span style=\"color:orange\">You assemble a barricade from the lattice and rods.</span>")
...
/obj/lattice/barricade
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if (istype(W, /obj/item/weldingtool))
			...
			if(WELD.welding)
				boutput(user, "<span style=\"color:orange\">You disassemble the barricade.</span>")
				WELD.eyecheck(user)
			...
			if (R.amount > difference)
				R.amount -= difference
				src.strength = 5
				boutput(user, "<span style=\"color:orange\">You reinforce the barricade.</span>")
				boutput(user, "<span style=\"color:orange\">The barricade is now fully reinforced!</span>") // seperate line for consistency's sake i guess
				return
			else if (R.amount <= difference)
				R.amount -= difference
				src.strength = 5
				boutput(user, "<span style=\"color:orange\">You use up the last of your rods to reinforce the barricade.</span>")
				if (src.strength >= 5) boutput(user, "<span style=\"color:orange\">The barricade is now fully reinforced!</span>")
				if (R.amount < 1)
					user.u_equip(W)
					qdel(W)
				return
