/obj/critter/gunbot/drone
	...
	seek_target()
		...
		/*if (A == colosseum_controller.colosseum) colosseum removal
	...
	CritterDeath()
		...
		spawn(20)
			/*if (get_area(src) != colosseum_controller.colosseum || must_drop_loot) colosseum removal
				if (prob(25))
		...
		if(task == "following path")
			...
			for(var/obj/machinery/cruiser/CR in range(10, src))
				if(CR)
					...
					break
		//	if (!waking)
				//if (get_area(src) == colosseum_controller.colosseum) colosseum removal
				//	waking = 1
			...
			spawn(rand(1,3)) // so it might miss, sometimes, maybe
				...
				for(var/obj/O in affected)
					...
					for(var/turf/T in src_turf)
						if(T == O) continue
						T.meteorhit(O)
				/*	for(var/obj/machinery/colosseum_putt/A in src_turf) colosseum removal
						if (A == O || A == user) continue
						A.meteorhit(O)*/
...
/obj/critter/gunbot/drone/iridium // the worstest jerk, even worse than the previous worst jerk.
	...
	/*	for (var/obj/machinery/colosseum_putt/poorPod in range(src, 5)) colosseum removal sofia
			lineObjs += DrawLine(src, poorPod, /obj/line_obj/elec, 'icons/obj/projectiles.dmi',"WholeLghtn",1,1,"HalfStartLghtn","HalfEndLghtn",FLY_LAYER,1,PreloadedIcon='icons/effects/LghtLine.dmi')
