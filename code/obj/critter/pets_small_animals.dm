...
/obj/critter/opossum
	...
	on_revive()
		..()
		src.visible_message("<span style=\"color:orange\"><b>[src]</b> stops playing dead and gets back up!</span>")
		src.alive = 1
		src.density = 1
...
*/
	...
	attack_hand(mob/user as mob)
		if (src.alive && (user.a_intent != INTENT_HARM))
			...
			if(prob(30))
				...
				for(var/mob/O in hearers(src, null))
					O.show_message("<span style=\"color:orange\"><B>[src]</B> flops on his back! Scratch that belly!</span>",2)
				spawn(30)
	...
	CritterDeath()
		...
		spawn(600)
			for(var/mob/O in hearers(src, null))
				O.show_message("<span style=\"color:orange\"><b>[src]</b> wags his tail and gets back up!</span>")
			src.alive = 1
			density = 1
			src.health = 100
			src.icon_state = "[src.doggy]"
...
/obj/critter/parrot // if you didn't want me to make a billion dumb parrot things you shouldn't have let me anywhere near the code so this is YOUR FAULT NOT MINE - Haine
	...
	proc/take_stuff()
		if (src.treasure)
			if (prob(2))
				src.visible_message("<span style=\"color:orange\"><b>[src]</b> drops its [src.treasure]!</span>")
	...
	proc/fussle()
		...
		if (src.treasure && prob(10))
			if (!src.muted)
				src.visible_message("<span style=\"color:orange\"><b>[src]</b> [pick("fusses with", "picks at", "pecks at", "throws around", "waves around", "nibbles on", "chews on", "tries to pry open")] [src.treasure].</span>")
			if (prob(5))
				src.visible_message("<span style=\"color:orange\"><b>[src]</b> drops its [src.treasure]!</span>")
				src.treasure.set_loc(src.loc)
				src.treasure = null
	...
	ai_think()
		if (task == "thinking" || task == "wandering")
			...
			if (prob(src.chatter_chance) && !src.muted)
				src.chatter()
			if (prob(5) && !src.muted)
				src.visible_message("<span style=\"color:orange\"><b>[src]</b> [pick("chatters", "chirps", "squawks", "mutters", "cackles", "mumbles", "fusses", "preens", "clicks its beak", "fluffs up", "poofs up")]!</span>")
			if (prob(15))
	...
	attack_hand(mob/user as mob)
		if (src.alive)
			if (user.a_intent == INTENT_HARM)
				...
				if(prob(15))
					src.visible_message("<span style=\"color:orange\"><b>[src]</b> chirps happily!</span>")
				return
		else
			..()
		return
...
*/
	proc/dance_response()
		if (!src.alive || src.sleeping)
			return
		if (prob(20))
			src.visible_message("<span style=\"color:orange\"><b>[src]</b> responds with a dance of its own!</span>")
			src.dance()
		else
			src.visible_message("<span style=\"color:orange\"><b>[src]</b> flaps and bobs [pick("to the beat", "in tune", "approvingly", "happily")].</span>")
