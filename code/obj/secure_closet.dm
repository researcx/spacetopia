...
/obj/secure_closet/personal/emag_act(var/mob/user, var/obj/item/card/emag/E) //FUCK YOU
	if(!src.broken)
		...
		src.update_overlays()
		if (user)
			user.visible_message("<span style=\"color:orange\">The locker has been broken by [user] with an electromagnetic card!</span>")
		return 1
	return 0
...
/obj/secure_closet/relaymove(mob/user as mob)
	...
	if (!( src.locked ))
		...
		if (!src.jiggled)
			src.jiggled = 1
			boutput(user, "<span style=\"color:orange\">It's welded shut!</span>")
...
/obj/secure_closet/verb/move_inside()
	...
	if (src.opened)
		...
		if (src.locked && src.allowed(usr))
			src.locked = !src.locked
			src.visible_message("<span style=\"color:orange\">The locker has been [src.locked ? null : "un"]locked by [usr].</span>")
			src.update_overlays()
