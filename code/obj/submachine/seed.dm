/obj/submachine/seed_manipulator/
		...
		if(href_list["page"])
			...
			if (istype(I,/obj/item/reagent_containers/food/snacks/plant/))
				...
				if (!give)
					...
					boutput(usr, "<span style=\"color:orange\">Extracted [give] seeds from [I].</span>")
					while (give > 0)
						var/obj/item/seed/S
				...
				if (src.inserted.reagents.total_volume < 10)
					...
					if (!usable_reagents.len)
						...
						switch(S.HYPinfusionS(R.id,src))
							...
							else boutput(usr, "<span style=\"color:orange\">Infusion of [R.name] successful.</span>")
			...
			if (prob(splice_chance)) // We're good, so start splicing!
				...
				boutput(usr, "<span style=\"color:orange\">Splice successful.</span>")
				if (!src.seedoutput) src.seeds.Add(S)
				else S.set_loc(src.loc)
	...
	attackby(var/obj/item/W as obj, var/mob/user as mob)
		if(istype(W, /obj/item/reagent_containers/glass/) || istype(W, /obj/item/reagent_containers/food/drinks/))
			...
			user.drop_item()
			W.set_loc(src)
			boutput(user, "<span style=\"color:orange\">You add [W] to the machine!</span>")
			src.updateUsrDialog()
			...
			if (select != "Never Mind")
				...
				for (var/obj/item/I in S.contents)
					...
					if (istype(I,/obj/item/reagent_containers/food/snacks/plant/) && (select == "Everything" || select == "Fruit Only"))
						...
						continue
				if (loadcount)
					boutput(user, "<span style=\"color:orange\">[loadcount] items were loaded from the satchel!</span>")
				else
	...
	MouseDrop_T(atom/movable/O as mob|obj, mob/user as mob)
		...
		if (istype(O, /obj/item/reagent_containers/food/snacks/plant/) || istype(O, /obj/item/seed/))
			user.visible_message("<span style=\"color:orange\">[user] begins quickly stuffing [O.name] into [src]!</span>")
			...
			boutput(user, "<span style=\"color:orange\">You finish stuffing items into [src]!</span>")
		else ..()
	...
	Topic(href, href_list)
		...
		if(href_list["page"])
			...
			if (istype(I))
				...
				I.set_loc(src.loc)
				boutput(usr, "<span style=\"color:orange\">You eject [I] from the machine!</span>")
				src.update_icon()
			src.updateUsrDialog()
...
/obj/submachine/seed_vendor
	...
	emag_act(var/mob/user, var/obj/item/card/emag/E)
		if (!src.hacked)
			if(user)
				boutput(user, "<span style=\"color:orange\">You disable the [src]'s product locks!</span>")
			src.hacked = 1
