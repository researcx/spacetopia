/obj/submachine/chef_sink
	...
	attackby(obj/item/W as obj, mob/user as mob)
		...
		if (istype(W, /obj/item/reagent_containers/food/snacks/ingredient/flour))
			...
			user.visible_message("<span style=\"color:orange\">[user] cleans [W].</span>")
	...
	attack_hand(var/mob/user as mob)
		if (ishuman(user))
			...
			playsound(src.loc, "sound/effects/slosh.ogg", 100, 1)
			if (H.gloves)
				user.visible_message("<span style=\"color:orange\">[user] cleans [his_or_her(user)] gloves.</span>")
				H.gloves.clean_forensic() // Ditto (Convair880).
				H.set_clothing_icon_dirty()
			else
				user.visible_message("<span style=\"color:orange\">[user] washes [his_or_her(user)] hands.</span>")
				if (H.sims)
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if (istype(W, /obj/item/reagent_containers/food/snacks/ice_cream_cone))
			if(src.cone)
				...
				W.set_loc(src)
				src.cone = W
				boutput(user, "<span style=\"color:orange\">You load the cone into [src].</span>")
...
/obj/submachine/chef_oven
	...
	attackby(obj/item/W as obj, mob/user as mob)
		...
		if (!proceed)
			...
			return
		user.visible_message("<span style=\"color:orange\">[user] loads [W] into [src].</span>")
		user.u_equip(W)
		W.set_loc(src)
		W.dropped()
		src.updateUsrDialog()
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if (istype(W, /obj/item/satchel/))
			...
			if (S.contents.len < 1) boutput(usr, "<span style=\"color:red\">There's nothing in the satchel!</span>")
				...
				user.visible_message("<span style=\"color:orange\">[user] loads [S]'s contents into [src]!</span>")
				var/amtload = 0
				...
				boutput(user, "<span style=\"color:orange\">[amtload] items loaded from satchel!</span>")
			...
			if (!proceed)
				...
				return
			user.visible_message("<span style=\"color:orange\">[user] loads [W] into the [src].</span>")
			user.u_equip(W)
			W.set_loc(src)
			W.dropped()
			return
	...
	MouseDrop(over_object, src_location, over_location)
		...
		if (over_object == usr && (in_range(src, usr) || usr.contents.Find(src)))
			...
			for(var/mob/O in AIviewers(usr, null))
				O.show_message("<span style=\"color:orange\">[usr] empties the [src].</span>")
			return
	...
	MouseDrop_T(atom/movable/O as mob|obj, mob/user as mob)
		...
		if (istype(O, /obj/storage))
			...
			for (var/obj/item/plant/P in O.contents)
				P.set_loc(src)
				amtload++
			if (amtload) boutput(user, "<span style=\"color:orange\">[amtload] items of food loaded from [O]!</span>")
			...
			user.visible_message("<span style=\"color:orange\">[user] begins quickly stuffing food into [src]!</span>")
			...
			for(var/obj/item/plant/P in view(1,user))
				...
				sleep(3)
				if (user.loc != staystill) break
			boutput(user, "<span style=\"color:orange\">You finish stuffing food into [src]!</span>")
		else ..()
		src.updateUsrDialog()
...
/obj/submachine/mixer
	...
	attackby(obj/item/W as obj, mob/user as mob)
		...
		if (!proceed)
			...
			return
		user.visible_message("<span style=\"color:orange\">[user] puts [W] into the [src].</span>")
		user.u_equip(W)
		W.set_loc(src)
		W.dropped()
