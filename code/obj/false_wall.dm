/turf/simulated/wall/false_wall
	...
	attack_hand(mob/user as mob)
		...
		if (src.density)
			...
			if (known)
				if (open())
					boutput(user, "<span style=\"color:orange\">The wall slides open.</span>")
			...
			if (close())
				boutput(user, "<span style=\"color:orange\">The wall slides shut.</span>")
		return
		...
		if (istype(S, /obj/item/screwdriver))
			...
			if (!src.density || prob(prob_opens))
				...
				if (src.density && !known) //if it was closed, let them know that they did something
					boutput(user, "<span style=\"color:orange\">It was a false wall!</span>")
				//disassemble it
				boutput(user, "<span style=\"color:orange\">Now dismantling false wall.</span>")
