...
/obj/machinery/port_a_brig
	...
	MouseDrop(over_object, src_location, over_location)
		...
		if (alert("Set selected turf as home location?",,"Yes","No") == "Yes")
			...
			usr.visible_message("<span style=\"color:orange\"><b>[usr.name]</b> changes the [src.name]'s home turf.</span>", "<span style=\"color:orange\">New home turf selected: [get_area(src.homeloc)].</span>")
	...
	attackby(obj/item/W, mob/user as mob)
		...
		if (istype(W, /obj/item/card/id))
			...
			var/turf/T = user.loc
			boutput(user, "<span style=\"color:orange\">Prying door open.</span>")
			playsound(src.loc, "sound/items/Crowbar.ogg", 100, 1)
			sleep(100)
			if ((user.loc == T && user.equipped() == W))
				src.locked = 0
				boutput(user, "<span style=\"color:orange\">You pried the door open.</span>")
