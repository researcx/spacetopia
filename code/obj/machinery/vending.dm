...
/obj/machinery/vending/fortune
	...
	prevend_effect()
		if(src.seconds_electrified)
			src.visible_message("<span style=\"color:orange\">[src] wakes up!</span>")
			...
			speak("F*!@$*(9HZZZZ9**###!")
			sleep(25)
			src.visible_message("<span style=\"color:orange\">[src] spasms violently!</span>")
			playsound(src.loc, pick(sounds_broken), 40, 1)
			if (src.icon_vend)
				flick(src.icon_vend,src)
			sleep(10)
			src.visible_message("<span style=\"color:orange\">[src] makes an obscene gesture!</b></span>")
			playsound(src.loc, pick(sounds_broken), 40, 1)
			if (src.icon_vend)
				flick(src.icon_vend,src)
			sleep(15)
			...
			sleep(25)
			src.visible_message("<span style=\"color:orange\">[src] rocks back and forth!</span>")
			playsound(src.loc, pick(sounds_working), 40, 1)
			if (src.icon_vend)
				flick(src.icon_vend,src)
			sleep(10)
			src.visible_message("<span style=\"color:orange\">[src] makes a mystical gesture!</b></span>")
			playsound(src.loc, pick(sounds_working), 40, 1)
			if (src.icon_vend)
				flick(src.icon_vend,src)
			sleep(15)
...
/obj/machinery/vending/proc/scan_card(var/mob/user as mob)
	if (!user || !src.acceptcard)
		return
	boutput(user, "<span style=\"color:orange\">You scan your fingerprint.</span>")
	var/datum/data/record/account = null
	account = FindBankAccountByName(user.real_name)
	if (!account)
		/*var/enterpin = input(user, "Please enter your PIN number.", "Enter PIN", 0) as null|num
		if (enterpin == card.pin)
			boutput(user, "<span style=\"color:orange\">Card authorized.</span>")
			src.scan = card
		else
			boutput(user, "<span style=\"color:red\">Pin number incorrect.</span>")
			src.scan = null
	else*/
		boutput(user, "<span style=\"color:red\">No bank account associated with your fingerprint found.</span>")
		src.scan = null
	else
		boutput(user, "<span style=\"color:orange\">Fingerprint for [user.real_name] authorized.</span>")
		src.scan = 1
...
/obj/machinery/vending/proc/generate_vending_HTML()
	src.vending_HTML = "[css_interfaces]<style>body,html{text-align: center;}</style><TT>"
	...
	if (src.pay && src.acceptcard)
		if (src.paying_for && !src.scan)
			...
			src.vending_HTML += "&emsp;<font color = '[src.paying_for.product_display_color]' size='4'><b>[src.paying_for.product_name]</b></font><br>"
			src.vending_HTML += "<br>Please scan your fingerprint to authorize payment.<br><br>"
		else if (src.scan)
			if (src.paying_for)
				...
				src.vending_HTML += "<br><B>Please scan your fingerprint to authorize payment.</b><br><br>"
			var/datum/data/record/account = null
			account = FindBankAccountByName(src.real_name)
			src.vending_HTML += "<B>Credits on Account: &ETH;[account.fields["current_money"]]</B> <BR>"

	if (src.product_list.len == 0)
		src.vending_HTML += "<font color = 'red'>No product loaded!</font>"

	else if (src.paying_for)
		src.vending_HTML += "<a href='byond://?src=\ref[src];vend=\ref[src.paying_for]'><u><b>Continue</b></u></a><br/>"
		...
		for (var/datum/data/vending_product/R in src.product_list)
			...
			src.vending_HTML += " ([R.product_amount])</font><br>"

			if (R.product_amount > 0)
				src.vending_HTML += "<a href='byond://?src=\ref[src];vend=\ref[R]' class='toggle-on vending'>Vend[(src.pay && R.product_cost) ? " ($[R.product_cost])" : null]</A>"
			else
				src.vending_HTML += "<a href='' class='toggle-off vending'>Vend[(src.pay && R.product_cost) ? " ($[R.product_cost])" : null]</A>"
			src.vending_HTML += "<br><br>"

		if (src.pay)
			src.vending_HTML += "<BR><B>Available Credits:</B>  &ETH;[src.credit]<br><a href='byond://?src=\ref[src];return_credits=1'>Return Credits</A>"
			if (!src.acceptcard)
...
/obj/machinery/vending/proc/generate_wire_HTML()
	src.vendwires = list("Violet" = 1,\
		...
		"Goldenrod" = 3,\
		"Green" = 4,)
	src.wire_HTML = "[css_interfaces]<TT><B>The Access Panel is open:</B><br>"
...
/obj/machinery/vending/attackby(obj/item/W as obj, mob/user as mob)
	if (istype(W, /obj/item/spacecash))
		if (src.pay)
			src.credit += W.amount
			W.amount = 0
			boutput(user, "<span style=\"color:orange\">You insert [W].</span>")
			user.u_equip(W)
			W.dropped()
			qdel( W )
			src.generate_HTML(1)
	...
	if (istype(W, /obj/item/card/id))
		if (src.acceptcard)
			...
			src.credit += amount
			W:money -= amount
			boutput(user, "<span style=\"color:orange\">You deposit [amount] credits. [W] now has [W:money] credits.</span>")
			src.updateUsrDialog()
			return()*/
		else
		...
		if (src.seconds_electrified != 0)
			...
			if (stat & NOPOWER)
				...
				boutput(user, "<span style=\"color:orange\">[bicon(W)] Regular electrical response received from access panel.</span>")
		return
		...
		if ((href_list["vend"]) && (src.vend_ready))
			...
			if (src.pay)
				if (src.acceptcard && src.scan)
					account = FindBankAccountByName(usr.real_name)
					if (!account)
				...
				if (!pay || (src.credit >= R.product_cost) || (account && account.fields["current_money"] >= R.product_cost)) //Conor12: Prevents credit hitting negative numbers if multiple items are bought at once.
					...
					if (src.pay)
						if (src.acceptcard && src.scan && account)
							if (src.doTransaction(usr.client.ckey,"-[R.product_cost]","[src.name]") == 1)
								account.fields["current_money"] -= R.product_cost
							else
								boutput(usr, "<span style=\"color:red\">Unable to connect to Spacetopia Banking.</span>")
								return
						else
		...
		if (href_list["return_credits"])
			spawn(src.vend_delay)
				if (src.credit > 0)
					...
					src.credit = 0
					boutput(usr, "<span style=\"color:orange\">You receive [returned].</span>")
					src.generate_HTML(1)
