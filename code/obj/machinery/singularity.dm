...
/obj/machinery/emitter/verb/rotate_ccw()
	set src in view(1)
	if (src.state != 0)
...
/obj/machinery/emitter/verb/rotate_cw()
	set src in view(1)
	if (src.state != 0)
		boutput(usr, "<span style=\"color:red\">It's secured to the floor and can't be rotated.</span>")
		return
...
/obj/machinery/power/collector_control/attackby(obj/item/W, mob/user)
	if(istype(W, /obj/item/device/analyzer))
		boutput(user, "<span style=\"color:orange\">The analyzer detects that [lastpower]W are being produced.</span>")
