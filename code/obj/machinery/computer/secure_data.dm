...
/obj/machinery/computer/secure_data/attack_hand(mob/user as mob)
	...
	if (src.temp)
		...
		dat = text("Confirm Identity<br> <A href='?src=\ref[];scan=1'>[]</A><HR>", src, (src.scan ? text("[]", src.scan.name) : "----------"))
	...
	user << browse(text("<HEAD><TITLE>Security Records</TITLE>[css_interfaces]</head><TT>[]</TT>", dat), "window=secure_rec")
