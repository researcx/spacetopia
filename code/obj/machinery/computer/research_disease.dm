...
"}
	...
	user << browse(text("<HEAD><TITLE>Disease Research</TITLE>[css_interfaces]</head><TT>[]</TT>", dat), "window=dis_res")
	...
	if ((usr.contents.Find(src) || (in_range(src, usr) && istype(src.loc, /turf))) || (istype(usr, /mob/living/silicon)))
		...
		if(href_list["res"])
			...
			if(disease_research.start_research(disease_research.tier*1000, researched_item))
				boutput(usr, "<span style=\"color:orange\">Commencing research</span>")
			else
				boutput(usr, "<span style=\"color:orange\">Could not start research</span>")
