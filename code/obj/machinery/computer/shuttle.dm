...
/obj/machinery/computer/shuttle/emag_act(var/mob/user, var/obj/item/card/emag/E)
	...
	if (user)
		...
		switch(choice)
			if("Launch")
				boutput(world, "<span style=\"color:orange\"><B>Alert: Shuttle launch time shortened to 10 seconds!</B></span>")
		...
		switch(choice)
			if("Authorize")
				...
				if (src.auth_need - src.authorized.len > 0)
					boutput(world, text("<span style=\"color:orange\"><B>Alert: [] authorizations needed until shuttle is launched early</B></span>", src.auth_need - src.authorized.len))
				else
					boutput(world, "<span style=\"color:orange\"><B>Alert: Shuttle launch time shortened to 60 seconds!</B></span>")
			...
			if("Abort")
				boutput(world, "<span style=\"color:orange\"><B>All authorizations to shorting time for shuttle launch have been revoked!</B></span>")
