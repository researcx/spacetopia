	...
	switch(state)
		...
		if (1)
			...
			if (istype(P, /obj/item/circuitboard) && !circuit)
				...
				boutput(user, "<span style=\"color:orange\">You place the circuit board inside the frame.</span>")
				src.icon_state = "1"
				src.circuit = P
				user.drop_item()
				P.set_loc(src)
			if (istype(P, /obj/item/screwdriver) && circuit)
				...
				boutput(user, "<span style=\"color:orange\">You screw the circuit board into place.</span>")
				src.state = 2
				src.icon_state = "2"
			if (istype(P, /obj/item/crowbar) && circuit)
				playsound(src.loc, "sound/items/Crowbar.ogg", 50, 1)
				boutput(user, "<span style=\"color:orange\">You remove the circuit board.</span>")
				src.state = 1
				src.icon_state = "0"
		...
		if (2)
			if (istype(P, /obj/item/screwdriver) && circuit)
				playsound(src.loc, "sound/items/Screwdriver.ogg", 50, 1)
				boutput(user, "<span style=\"color:orange\">You unfasten the circuit board.</span>")
				src.state = 1
				src.icon_state = "1"
