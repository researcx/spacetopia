...
a.updated {
	...
	var/dat = "<html><head><title>[station_name()] Stock Exchange</title>[css][css_interfaces]</head><body><h2>Stock Exchange</h2>"
...
/obj/machinery/computer/stockexchange/attackby(var/obj/item/I as obj, user as mob)
	if (istype(I, /obj/item/card/id))
		...
		boutput(user, "<span style=\"color:orange\">You swipe the ID card.</span>")
...
/obj/machinery/computer/stockexchange/proc/sell_some_shares(var/datum/stock/S, var/mob/user)
	...
	if (!S.sellShares(logged_in, amt))
		...
		return
	boutput(user, "<span style='color:orange'>Sold [amt] shares of [S.name] for [total] credits.</span>")
...
/obj/machinery/computer/stockexchange/proc/buy_some_shares(var/datum/stock/S, var/mob/user)
	...
	if (!S.buyShares(logged_in, amt))
		boutput(user, "<span style='color:red'>Could not complete transaction.</span>")
		return
	boutput(user, "<span style='color:orange'>Bought [amt] shares of [S.name] for [total] credits.</span>")

/obj/machinery/computer/stockexchange/proc/do_borrowing_deal(var/datum/borrow/B, var/mob/user)
	if (B.stock.borrow(B, logged_in))
		boutput(user, "<span style='color:orange'>You successfully borrowed [B.share_amount] shares. Deposit: [B.deposit].</span>")
	else
...
/obj/machinery/computer/stockexchange/Topic(href, href_list)
	...
	if (href_list["archive"])
		...
		var/dat = "<html><head><title>News feed for [S.name]</title>[css_interfaces]</head><body><h2>News feed for [S.name]</h2><div><a href='?src=\ref[src];archive=\ref[S]'>Refresh</a></div>"
