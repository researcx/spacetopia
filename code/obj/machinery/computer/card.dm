...
/obj/machinery/computer/card/attack_hand(var/mob/user as mob)
	...
	if (src.mode) // accessing crew manifest
		...
		dat = "[css_interfaces]<b>Crew Manifest:</b><br>Please use security record computer to modify entries.<br>[crew]<a href='?src=\ref[src];print=1'>Print</a><br><br><a href='?src=\ref[src];mode=0'>Access ID modification console.</a><br>"
	else
		...
		if (src.authenticated && src.modify)
			...
			var/list/civilianjobs = list("Civilian", "Barman", "Chef", "Botanist", "Chaplain", "Janitor", "Clown")
			...
			var/civilian_access = "<br>Staff:<br><br>"
			var/list/civilian_access_list = list(6, 12, 22, 23, 25, 26, 27, 28, 35)
			var/engineering_access = "<br>Engineering:<br><br>"
			...
			var/supply_access = "<br>Supply:<br><br>"
			var/list/supply_access_list = list(30, 31, 47, 50, 51)
			var/research_access = "<br>Science and Medical:<br><br>"
			var/list/research_access_list = list(5, 7, 8, 9, 10, 24, 29, 33)
			var/security_access = "<br>Security:<br><br>"
			var/list/security_access_list = list(1, 2, 4, 37, 38, 39)
			var/command_access = "<br>Command:<br><br>"
		...
		dat = "[css_interfaces]<TT>[header][body]<hr><a href='?src=\ref[src];mode=1'>Access Crew Manifest</a><br></tt>"
...
/obj/machinery/computer/card/attackby(obj/item/I as obj, mob/user as mob)
	if(istype(I, /obj/item/screwdriver))
		...
		if (!src.scan)
			boutput(user, "<span style=\"color:orange\">You insert [I] into the authentication card slot.</span>")
			user.drop_item()
			I.set_loc(src)
			src.scan = I
		else if (!src.modify)
			boutput(user, "<span style=\"color:orange\">You insert [I] into the target card slot.</span>")
			user.drop_item()
			I.set_loc(src)
			src.modify = I
		src.updateUsrDialog()
