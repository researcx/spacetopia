...
/obj/machinery/computer/announcement
	...
	attack_hand(mob/user)
		if(..()) return
		user.machine = src
		var/dat = {"[css_interfaces]
			<body>
				<h1>Announcement Computer</h1>
				<hr>
				Status: [status]<BR>
				Card:<br> <a href='?src=\ref[src];card=1'>[src.ID ? src.ID.name : "--------"]</a><br>
				Broadcast delay: [nice_timer()]<br>
				<br>
				Message:<br> <a href='?src=\ref[src];edit_message=1'>[src.message ? src.message : "___________"]</a> <a href='?src=\ref[src];clear_message=1'>Clear</a><br>
				<br>
				<b><a href='?src=\ref[src];send_message=1'>Transmit</a></b>
			"}
		if (src.announces_arrivals)
			dat += "<hr><h2>Arrivals Announcements</h2>"
			dat += "[src.arrival_announcements_enabled ? "Arrival Announcement Message: \"[src.arrivalalert]\"<br><br><b><a href='?src=\ref[src];set_arrival_message=1'>Change</a></b> <b><a href='?src=\ref[src];toggle_arrival_message=1'>Disable</a></b>" : "Arrival Announcements Disabled<br><br><b><a href='?src=\ref[src];toggle_arrival_message=1'>Enable</a></b>"]"
		dat += "</body>"
