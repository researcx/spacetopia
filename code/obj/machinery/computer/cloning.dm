...
/obj/machinery/computer/cloning/attackby(obj/item/W as obj, mob/user as mob)
	if (wagesystem.clones_for_cash && istype(W, /obj/item/spacecash))
		...
		cash.amount = 0
		user.show_text("<span style=\"color:orange\">You add [cash] to the credit in [src].</span>")
		user.u_equip(W)
		qdel(W)
...
/obj/machinery/clone_scanner
	...
	verb/move_inside()
		...
		if (src.occupant)
			boutput(usr, "<span style=\"color:orange\"><B>The scanner is already occupied!</B></span>")
			return
	...
	attackby(var/obj/item/grab/G as obj, user as mob)
		...
		if (src.occupant)
			boutput(user, "<span style=\"color:orange\"><B>The scanner is already occupied!</B></span>")
			return
