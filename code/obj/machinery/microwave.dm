...
obj/machinery/microwave/attackby(var/obj/item/O as obj, var/mob/user as mob)
	if (istype(O,/obj/item/electronics/scanner))
		user.visible_message("<span style=\"color:red\"><B>[user] scans \the [src] with [O]!</B></span>")
		return

	if(src.broken > 0)
		if(src.broken == 2 && istype(O, /obj/item/screwdriver)) // If it's broken and they're using a screwdriver
			src.visible_message("<span style=\"color:orange\">[user] starts to fix part of the microwave.</span>")
			sleep(20)
			src.visible_message("<span style=\"color:orange\">[user] fixes part of the microwave.</span>")
			src.broken = 1 // Fix it a bit
		else if(src.broken == 1 && istype(O, /obj/item/wrench)) // If it's broken and they're doing the wrench
			src.visible_message("<span style=\"color:orange\">[user] starts to fix part of the microwave.</span>")
			sleep(20)
			src.visible_message("<span style=\"color:orange\">[user] fixes the microwave!</span>")
			src.icon_state = "mw"
			src.broken = 0 // Fix it!
		else
		...
		if(istype(O, /obj/item/spraybottle)) // If they're trying to clean it then let them
			src.visible_message("<span style=\"color:orange\">[user] starts to clean the microwave.</span>")
			sleep(20)
			src.visible_message("<span style=\"color:orange\">[user] has cleaned the microwave!</span>")
		...
		if(src.egg_amount < 5)
			src.visible_message("<span style=\"color:orange\">[user] adds an egg to the microwave.</span>")
			src.egg_amount++
			qdel(O)
	else if(istype(O, /obj/item/reagent_containers/food/snacks/ingredient/flour)) // If flour is used, add it
		if(src.flour_amount < 5)
			src.visible_message("<span style=\"color:orange\">[user] adds some flour to the microwave.</span>")
			src.flour_amount++
			qdel(O)
	else if(istype(O, /obj/item/reagent_containers/food/snacks/ingredient/meat/monkeymeat))
		if(src.monkeymeat_amount < 5)
			src.visible_message("<span style=\"color:orange\">[user] adds some meat to the microwave.</span>")
		...
		if(src.donkpocket_amount < 2)
			src.visible_message("<span style=\"color:orange\">[user] adds a donk-pocket to the microwave.</span>")
		...
		if(!istype(extra_item, /obj/item)) //Allow one non food item to be added!
			...
			O.set_loc(src)
			O.dropped(user)
			src.visible_message("<span style=\"color:orange\">[user] adds [O] to the microwave.</span>")
		else
...
"}
	...
	user << browse("<HEAD><TITLE>Microwave Controls</TITLE>[css_interfaces]</head><TT>[dat]</TT>", "window=microwave")
	...
	if(href_list["cook"])
		if(!src.operating)
			...
			if(operation == 1) // If cook was pressed
				src.visible_message("<span style=\"color:orange\">The microwave turns on.</span>")
			...
			if(!isnull(cooking))
				src.visible_message("<span style=\"color:orange\">The microwave begins cooking something!</span>")
