/obj/machinery/glass_recycler
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if(istype(W, /obj/item/raw_material/shard))
			glass_amt += 1
			user.visible_message("<span style=\"color:orange\">[user] inserts [W] into [src].</span>")
			user.u_equip(W)
			qdel(W)
		else
	...
	proc/create(var/object)
		...
		if(object != "plate")
			src.visible_message("<span style=\"color:orange\">[src] manufactures a [object] glass!</span>")
			return
		else
			src.visible_message("<span style=\"color:orange\">[src] manufactures a glass plate!</span>")
			return