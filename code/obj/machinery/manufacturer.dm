...
#define WIRE_SHOCK 4
			...
			if (src.timeleft < 1)
				...
				spawn(0)
					if (src.queue.len < 1)
						...
						src.visible_message("<span style=\"color:orange\">[src] finishes its production queue.</span>")
						src.mode = "ready"
						src.build_icon()
	...
	emag_act(var/mob/user, var/obj/item/card/emag/E)
		if (!src.hacked)
			src.hacked = 1
			if(user)
				boutput(user, "<span style=\"color:orange\">You remove the [src]'s product locks!</span>")
			return 1
		return 0
	...
	attackby(obj/item/W as obj, mob/user as mob)
		...
		if (istype(W, /obj/item/paper/manufacturer_blueprint))
			...
			playsound(src.loc, src.sound_happy, 50, 1)
			boutput(user, "<span style=\"color:orange\">The manufacturer accepts and scans the blueprint.</span>")
			qdel(BP)
			return

		else if (istype(W, /obj/item/satchel))
			user.visible_message("<span style=\"color:orange\">[user] uses [src]'s automatic loader on [W]!</span>", "<span style=\"color:orange\">You use [src]'s automatic loader on [W].</span>")
			var/amtload = 0
			...
			if (amtload) boutput(user, "<span style=\"color:orange\">[amtload] materials loaded from [W]!</span>")
			...
			if (istype(WELD,src.base_material_class) && src.accept_loading(user))
				if (alert(user,"What do you want to do with [WELD]?","[src.name]","Repair","Load it in") == "Load it in")
					do_action = 1
			if (do_action == 1)
				user.visible_message("<span style=\"color:orange\">[user] loads [WELD] into the [src].</span>", "<span style=\"color:orange\">You load [WELD] into the [src].</span>")
				...
				if (src.health < 50)
					...
					if (WELD.get_fuel() > 1)
						...
						if (src.health == 100)
							boutput(user, "<span style=\"color:orange\"><b>[src] looks fully repaired!</b></span>")
					else
			...
			if (istype(C,src.base_material_class) && src.accept_loading(user))
				if (alert(user,"What do you want to do with [C]?","[src.name]","Repair","Load it in") == "Load it in")
					do_action = 1
			if (do_action == 1)
				user.visible_message("<span style=\"color:orange\">[user] loads [C] into the [src].</span>", "<span style=\"color:orange\">You load [C] into the [src].</span>")
				...
				if (src.health >= 50)
					...
					if (src.health >= 50)
						boutput(user, "<span style=\"color:orange\">The wiring is fully repaired. Now you need to weld the external plating.</span>")
			...
			if (istype(W,src.base_material_class) && src.accept_loading(user))
				if (alert(user,"What do you want to do with [W]?","[src.name]","Dismantle/Construct","Load it in") == "Load it in")
					do_action = 1
			if (do_action == 1)
				user.visible_message("<span style=\"color:orange\">[user] loads [W] into the [src].</span>", "<span style=\"color:orange\">You load [W] into the [src].</span>")
	...
	MouseDrop(over_object, src_location, over_location)
		...
		if (istype(over_object,/obj/storage/crate/))
			...
			if (C.locked || C.welded)
				...
				boutput(usr, "<span style=\"color:orange\">You set the manufacturer to output to [over_object]!</span>")
			...
			boutput(usr, "<span style=\"color:orange\">You set the manufacturer to output on top of [O]!</span>")
	...
	MouseDrop_T(atom/movable/O as mob|obj, mob/user as mob)
		...
		if (istype(O, /obj/item/paper/manufacturer_blueprint))
			...
			user.visible_message("<span style=\"color:orange\">[user] uses [src]'s automatic loader on [O]!</span>", "<span style=\"color:orange\">You use [src]'s automatic loader on [O].</span>")
			var/amtload = 0
			for (var/obj/item/M in O.contents)
				...
				src.load_item(M)
				amtload++
			if (amtload) boutput(user, "<span style=\"color:orange\">[amtload] materials loaded from [O]!</span>")
			else boutput(user, "<span style=\"color:red\">No material loaded!</span>")

		else if (istype(O, /obj/item/) && src.accept_loading(user,1))
			user.visible_message("<span style=\"color:orange\">[user] begins quickly stuffing materials into [src]!</span>")
			...
			boutput(user, "<span style=\"color:orange\">You finish stuffing materials into [src]!</span>")
