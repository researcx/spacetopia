...
/obj/machinery/plantpot
	...
	attackby(obj/item/W as obj, mob/user as mob)
		...
		if (istype(W, /obj/item/wrench) || istype(W, /obj/item/screwdriver))
			...
			if (src.current)
				...
				return
			user.visible_message("<span style=\"color:orange\">[user] plants a seed in the [src].</span>")
			user.u_equip(SEED)
			SEED.set_loc(src)
			if (SEED.planttype)
			...
			if (!W.reagents.total_volume)
				...
				user.visible_message("<span style=\"color:orange\">[user] pours [W:amount_per_transfer_from_this] units of [W]'s contents into [src].</span>")
			...
			user.visible_message("<span style=\"color:orange\">[user] plants [W] in the tray.</span>")
	...
	attack_hand(var/mob/user as mob)
		...
		if (src.current)
			...
			if (src.dead)
				boutput(user, "<span style=\"color:orange\">You clear the dead plant out of the tray.</span>")
				HYPdestroyplant()
				return
			...
			if (HYPcheck_if_harvestable())
				...
				if (src.health >= growing.starthealth * 4) boutput(user, "<span style=\"color:orange\">The plant is flourishing!</span>")
				else if (src.health >= growing.starthealth * 2) boutput(user, "<span style=\"color:orange\">The plant looks very healthy.</span>")
	...
	proc/HYPharvesting(var/mob/living/user,var/obj/item/satchel/SA)
		...
		if (src.health >= growing.starthealth * 2 && prob(30))
			boutput(user, "<span style=\"color:orange\">This looks like a good harvest!</span>")
		...
		if (src.health >= growing.starthealth * 4 && prob(30))
			boutput(user, "<span style=\"color:orange\">It's a bumper crop!</span>")
		...
		if (getamount < 1)
			...
			if (seedcount)
				boutput(user, "<span style=\"color:orange\">You harvested [cropcount] items and [seedcount] seeds.</span>")
			else
				boutput(user, "<span style=\"color:orange\">You harvested [cropcount] items.</span>")
		...
		if (!HYPCheckCommut(DNA,/datum/plant_gene_strain/immortal))
			...
			if (src.health >= growing.starthealth * 4)
				...
				if (prob(extra_harvest_chance))
					boutput(user, "<span style=\"color:orange\">The plant glistens with good health!</span>")
