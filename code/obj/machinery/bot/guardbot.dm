...
/obj/machinery/bot/guardbot
	...
	attackby(obj/item/W as obj, mob/user as mob)
		...
		*/
			...
			if (src.health < initial(health))
				...
				src.visible_message("<span style=\"color:orange\">[user] repairs [src]!</span>", "<span style=\"color:orange\">You repair [src].</span>")
	...
	get_desc(dist)
		..()
		if (src.on && src.idle)
			. = "<br><span style=\"color:orange\">[src] appears to be sleeping.</span>"
		if (src.health < initial(health))
			if (src.health > 10)
	...
	attack_hand(mob/user as mob)
		...
		if(user.a_intent == "help" && user.machine != src && (get_dist(user,src) <= 1))
			...
			user.visible_message("<span style=\"color:orange\">[user] [affection]s [src]!</span>","<span style=\"color:orange\">You [affection] [src]!</span>")
			if(src.task)
...
*/
		interact(mob/user as mob)
			var/dat = "[css_interfaces]<TT><B>PR-6S Guardbuddy v1.4</B></tt><br><br>"
			...
			user << browse("<head><title>Guardbuddy v1.4 controls</title>[css_interfaces]</head>[dat]", "window=guardbot")
		...
		task_input(var/input)
			...
			if(input == "hugged")
				switch(master.emotion)
					...
					if("joy","love")
						if (prob(25))
							master.visible_message("<span style=\"color:orange\">[master.name] reciprocates the hug!</span>")
				return 1
...
/obj/machinery/guardbot_dock
	...
	attack_hand(mob/user as mob)
		...
		var/dat = "<html><head><title>PR-6S Docking Station</title>[css_interfaces]</head><body>"
...
/obj/machinery/computer/tour_console
	...
	attack_hand(mob/user as mob)
		...
		user << browse("<head><title>Tour Monitor</title>[css_interfaces]</head>[dat]", "window=tourconsole;size=302x245")
...
/obj/machinery/bot/guardbot/old
	...
	interact(mob/user as mob)
		var/dat = "[css_interfaces]<TT><B>PR-4 Robuddy v0.8</B></tt><br><br>"
		...
		user << browse("<head><title>Robuddy v0.8 controls</title>[css_interfaces]</head>[dat]", "window=guardbot")
		onclose(user, "guardbot")
		return
