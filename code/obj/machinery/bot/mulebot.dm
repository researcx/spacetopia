...
/obj/machinery/bot/mulebot
	...
	emag_act(var/mob/user, var/obj/item/card/emag/E)
		locked = !locked
		if(user)
			boutput(user, "<span style=\"color:orange\">You [locked ? "lock" : "unlock"] the mulebot's controls!</span>")
	...
	attackby(var/obj/item/I, var/mob/user)
		if(istype(I,/obj/item/cell) && open && !cell)
			...
			if(locked)
				boutput(user, "<span style=\"color:orange\">The maintenance hatch cannot be opened or closed while the controls are locked.</span>")
				return

			open = !open
			if(open)
				src.visible_message("[user] opens the maintenance hatch of [src]", "<span style=\"color:orange\">You open [src]'s maintenance hatch.</span>")
				on = 0
				icon_state="mulebot-hatch"
			else
				src.visible_message("[user] closes the maintenance hatch of [src]", "<span style=\"color:orange\">You close [src]'s maintenance hatch.</span>")
				icon_state = "mulebot0"
	...
	proc/interact(var/mob/user, var/ai=0)
		...
		user << browse("<HEAD><TITLE>Mulebot [suffix ? "([suffix])" : ""]</TITLE>[css_interfaces]</head>[dat]", "window=mulebot;size=350x500")
			...
			switch(href_list["op"])
				...
				if("cellremove")
					if(open && cell && !usr.equipped())
						...
						usr.visible_message("<span style=\"color:orange\">[usr] removes the power cell from [src].</span>", "<span style=\"color:orange\">You remove the power cell from [src].</span>")
						updateDialog()
				...
				if("cellinsert")
					if(open && !cell)
						...
						if(istype(C))
							...
							usr.visible_message("<span style=\"color:orange\">[usr] inserts a power cell into [src].</span>", "<span style=\"color:orange\">You insert the power cell into [src].</span>")
							updateDialog()
				...
				if("wirecut")
					if(istype(usr.equipped(), /obj/item/wirecutters))
						...
						boutput(usr, "<span style=\"color:orange\">You need wirecutters!</span>")
				if("wiremend")
					if(istype(usr.equipped(), /obj/item/wirecutters))
						var/wirebit = text2num(href_list["wire"])
						if (wirebit == wire_mobavoid)
				...
				if("wirepulse")
					if(istype(usr.equipped(), /obj/item/device/multitool))
						switch(href_list["wire"])
							if("1","2")
								boutput(usr, "<span style=\"color:orange\">[bicon(src)] The charge light flickers.</span>")
							if("4")
								boutput(usr, "<span style=\"color:orange\">[bicon(src)] The external warning lights flash briefly.</span>")
							if("8")
								boutput(usr, "<span style=\"color:orange\">[bicon(src)] The load platform clunks.</span>")
							if("16", "32")
								boutput(usr, "<span style=\"color:orange\">[bicon(src)] The drive motor whines briefly.</span>")
							else
								boutput(usr, "<span style=\"color:orange\">[bicon(src)] You hear a radio crackle.</span>")
					else
						boutput(usr, "<span style=\"color:orange\">You need a multitool!</span>")
