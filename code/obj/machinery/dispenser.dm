...
/obj/machinery/dispenser_disease
	...
	attack_hand(mob/user as mob)
		...
		var/dat = "[css_interfaces]<TT><B>Chemical Dispenser Unit</B><BR><HR><BR>"
	...
	attackby(obj/item/W, mob/user as mob)
		if(istype(W, /obj/item/reagent_containers/glass/vial))
			if(src.active_vial)
				boutput(user, "<span style=\"color:orange\">The dispenser already has a test tube in it</span>")
			else
				boutput(user, "<span style=\"color:orange\">You insert the test tube into the dispenser</span>")
				user.drop_vial()
				W.set_loc(src)
				src.active_vial = W
			src.updateUsrDialog()
			...
			boutput(user, "<span style=\"color:orange\">[W] is too big to fit in!</span>")
			return
