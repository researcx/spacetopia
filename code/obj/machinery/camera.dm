	...
	if (istype(W, /obj/item/wirecutters))
		...
		for(var/mob/O in mobs)
			if (istype(O, /mob/living/silicon/ai))
				...
				O << browse(text("<HTML><HEAD><TITLE>[]</TITLE>[css_interfaces]</head><BODY><TT>[]</TT></BODY></HTML>", X.name, X.info), text("window=[]", X.name))
...
/obj/machinery/camera/motion/attackby(W as obj, mob/user as mob)
	...
	if (istype(W, /obj/item/screwdriver))
		var/turf/T = user.loc
		boutput(user, text("<span style=\"color:orange\">[]ing the access hatch... (this is a long process)</span>", (locked) ? "Open" : "Clos"))
		sleep(100)
		if ((user.loc == T && user.equipped() == W && !( user.stat )))
			src.locked ^= 1
			boutput(user, text("<span style=\"color:orange\">The access hatch is now [].</span>", (locked) ? "closed" : "open"))
