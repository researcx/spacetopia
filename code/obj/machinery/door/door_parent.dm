		...
		if (C.brainloss >= 60)
			...
			if (prob(40))
				...
				if (!istype(C.head, /obj/item/clothing/head/helmet))
					...
					boutput(C, "<span style=\"color:orange\">Your helmet protected you from injury!</span>")
...
/obj/machinery/door/proc/close(var/unsafe = 0)
	...
	spawn(-1)
		...
		for(var/mob/living/L in get_turf(src))
			playsound(get_turf(src), "sound/effects/fleshbr1.ogg", 100, 1)
			L.emote("scream")
			L.TakeDamage("All", rand(30, 50), 0, 0, DAMAGE_CRUSH)
			L.stunned += 5
			L.weakened += 5
			L.stuttering += 10
...
/obj/machinery/door/unpowered/wood/attackby(obj/item/I as obj, mob/user as mob)
	...
	if(istype(I, /obj/item/device/key) && src.density)
		...
		src.visible_message("<span style=\"color:orange\"><B>[user] [src.locked ? "locks" : "unlocks"] the door.</B></span>")
		return
