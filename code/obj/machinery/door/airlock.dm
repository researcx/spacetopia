...
//		message_admins("<span style=\"color:orange\"><B>ADMIN: </B>//DEBUG: shock_damage = [shock_damage] PN.avail = [PN.avail] user = [user] netnum = [netnum]</span>")
...
/obj/machinery/door/airlock/attack_hand(mob/user as mob)
	...
	if (src.p_open)
		...
		t1 += text("<br>[]<br>[]<br>[]<br>[]", (src.locked ? "The door bolts have fallen!" : "The door bolts look up."), ((src.arePowerSystemsOn() && !(stat & NOPOWER)) ? "The test light is on." : "The test light is off!"), (src.aiControlDisabled==0 ? "The 'AI control allowed' light is on." : "The 'AI control allowed' light is off."), (src.safety ? "The green light is blinking!" : "The green light is off!"))
	...
	if (istype(C, /obj/item/device/t_scanner) || (istype(C, /obj/item/device/pda2) && istype(C:module, /obj/item/device/pda_module/tray)))
		if(src.isElectrified())
			...
			if(stat & NOPOWER)
				...
				boutput(usr, "<span style=\"color:orange\">[bicon(C)] Regular electrical response received from access panel.</span>")
		return
