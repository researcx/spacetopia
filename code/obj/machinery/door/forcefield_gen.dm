	...
	attack_hand(mob/user as mob)
		...
		if(src.allowed(user, req_only_one_required))
			if(set_active)
				set_active = 0
				turn_off()
				boutput(user, "<span style=\"color:orange\">You deactivate the forcefield.</span>")
			else
				set_active = 1
				if(powered())
					turn_on()
					boutput(user, "<span style=\"color:orange\">You activate the forcefield.</span>")
				else
					boutput(user, "<span style=\"color:orange\">You attempt to activate the forcefield but its not powered.</span>")
