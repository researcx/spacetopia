...
/obj/machinery/shuttle/engine/propulsion/attackby(obj/item/W as obj, mob/user as mob)
	if (istype(W, /obj/item/screwdriver))
		if (src.stat1 == 0)
			boutput(usr, "<span style=\"color:orange\">Resecuring outer frame.</span>")
			playsound(src.loc, "sound/items/Screwdriver.ogg", 100, 1)
			sleep(20)
			boutput(usr, "<span style=\"color:orange\">Outer frame secured.</span>")
			src.stat1 = 1
			return
		if (src.stat1 == 1)
		...
		boutput(usr, "<span style=\"color:orange\">Now plating hull.</span>")
		sleep(20)
		boutput(usr, "<span style=\"color:orange\">Plating secured.</span>")
		qdel(W)
		src.stat2 = 1
		return
...
/obj/machinery/shuttle/engine/propulsion/examine()
	if (src.stat1 == 1 && src.stat2 == 1)
		boutput(usr, "<span style=\"color:orange\">The propulsion engine is working properly!</span>")
	else
