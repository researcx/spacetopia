...
/obj/machinery/sleep_console
	...
	proc/wake_occupant()
		...
		if (ishuman(occupant))
			...
			src.visible_message("<span style=\"color:orange\">The [src.name]'s occupant alarm clock dings!</span>")
		...
		if (href_list["time"])
			if (src.our_sleeper && src.our_sleeper.occupant)
				if (src.our_sleeper.occupant.stat == 2)
					...
					src.visible_message("<span style=\"color:orange\">[usr] [src.timing ? "sets" : "stops"] the [src]'s occupant alarm clock.</span>")
					if (src.timing)
