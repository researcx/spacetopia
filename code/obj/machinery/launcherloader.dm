...
/obj/machinery/computer/barcode
	...
	attackby(var/obj/item/I as obj, user as mob)
		...
		if (istype(I, /obj/item/card/id))
			...
			if(account)
				var/enterpin = input(user, "Please enter your PIN.", "Order Console", 0) as null|num
				if (enterpin == I:pin)
					boutput(user, "<span style=\"color:orange\">Card authorized.</span>")
					src.scan = I
				else
					boutput(user, "<span style=\"color:red\">PIN incorrect.</span>")
					src.scan = null
			else
...
/obj/item/barcodesticker
	...
	proc/attachTo(atom/target)
		if(get_dist(get_turf(target), get_turf(src)) <= 1 && istype(target, /atom/movable))
			...
			usr.visible_message("<span style=\"color:orange\">[usr] puts a [src.name] on [target].</span>")
			if(istype(target, /obj/storage/crate) && scan && account)
				var/obj/storage/crate/C = target
				C.scan = src.scan
				C.account = src.account
				boutput(usr, "<span style=\"color:orange\">[target] has been marked with your account routing information.</span>")
