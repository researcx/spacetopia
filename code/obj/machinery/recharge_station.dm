		...
		if ((usr.contents.Find(src) || src.contents.Find(usr) || ((get_dist(src, usr) <= 1) && istype(src.loc, /turf))))
			...
			if (href_list["rename"])
				...
				if (url_regex && url_regex.Find(newname))
					boutput(usr, "<span style=\"color:orange\"><b>Web/BYOND links are not allowed in ingame chat.</b></span>")
			...
			if (href_list["install"])
				...
				if (istype(O,/obj/item/roboupgrade))
					...
					boutput(R, "<span style=\"color:orange\">You recieved [O]! It can be activated from your panel.</span>")
