/obj/machinery/power/furnace
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if (istype(W, /obj/item/grab))
			if (!src.active)
				...
				if(W:affecting && src.active)
					...
					if(src.fuel >= src.maxfuel)
						...
						boutput(user, "<span style=\"color:orange\">The furnace is now full!</span>")
					return
			...
			..()
			return
		boutput(user, "<span style=\"color:orange\">You load [W] into [src]!</span>")
		user.u_equip(W)
		W.dropped()
		qdel (W)
		if(src.fuel > src.maxfuel)
	...
	MouseDrop_T(atom/movable/O as mob|obj, mob/user as mob)
		...
		if (istype(O, /obj/storage/crate/))
			if (src.fuel >= src.maxfuel)
				...
				return
			user.visible_message("<span style=\"color:orange\">[user] uses the [src]'s automatic ore loader on [O]!</span>", "<span style=\"color:orange\">You use the [src]'s automatic ore loader on [O].</span>")
			var/amtload = 0
			for (var/obj/item/raw_material/M in O.contents)
				...
				if (src.fuel >= src.maxfuel)
					...
					boutput(user, "<span style=\"color:orange\">The furnace is now full!</span>")
					break
			if (amtload) boutput(user, "<span style=\"color:orange\">[amtload] pieces of ore loaded from [O]!</span>")
			...
			if (src.fuel >= src.maxfuel)
				boutput(user, "<span style=\"color:red\">The furnace is already full!</span>")
				return
			user.visible_message("<span style=\"color:orange\">[user] begins quickly stuffing ore into [src]!</span>")
			...
			boutput(user, "<span style=\"color:orange\">You finish stuffing ore into [src]!</span>")

		else if (istype(O, /obj/item/plant/herb/cannabis))
			if (src.fuel >= src.maxfuel)
				boutput(user, "<span style=\"color:red\">The furnace is already full!</span>")
				return
			user.visible_message("<span style=\"color:orange\">[user] begins quickly stuffing weed into [src]!</span>") // four fuckin twenty all day
			var/staystill = user.loc
			for(var/obj/item/plant/herb/cannabis/M in view(1,user))
				...
				sleep(3)
				if (user.loc != staystill) break
			boutput(user, "<span style=\"color:orange\">You finish stuffing weed into [src]!</span>")
			...
			if (C.alive)
				...
				return
			user.visible_message("<span style=\"color:orange\">[user] [pick("crams", "shoves", "pushes", "forces")] [O] into [src]!</span>")
