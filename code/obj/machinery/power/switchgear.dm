...
/obj/machinery/power/switchgear/attack_hand(mob/user)
	...
	if (open)
		...
		if (locked)
			...
			boutput(user, "<span style=\"color:orange\">You open the access panel.</span>")
...
/obj/machinery/power/switchgear/attackby(obj/item/W, mob/user)
	if(istype(W, /obj/item/card/id))
		if (src.allowed(user, req_only_one_required))
			...
			boutput(user, "<span style=\"color:orange\">You [src.locked ? "lock" : "unlock"] the switchgear access panel.</span>")
		else
...
/obj/machinery/power/switchgear/proc/interact(mob/user)
	...
	user.machine = src
	var/t = "[css_interfaces]<TT><B>Switchgear Control</B><BR><A href='?src=\ref[src];close=1'>Close Panel</A><HR>"
