...
/obj/machinery/power/apc/attackby(obj/item/W, mob/user)
	...
	if (istype(W, /obj/item/crowbar))	// crowbar means open or close the cover
		...
		if (istype(W, /obj/item/robojumper))
			...
			if (!user:cell) boutput(user, "<span style=\"color:red\">You have no cell installed!</span>")
			...
			else if (src:cell.charge >= 2500) boutput(user, "<span style=\"color:orange\">The APC cell is already fully charged.</span>")
				...
				user.visible_message("<span style=\"color:orange\">[user] transfers some of their power to [src]!</span>", "<span style=\"color:orange\">You transfer [overspill] charge. The APC is now fully charged.</span>")
			else
				user:cell.charge -= 250
				src:cell.charge += 250
				user.visible_message("<span style=\"color:orange\">[user] transfers some of their power to [src]!</span>", "<span style=\"color:orange\">You transfer 250 charge.</span>")
...
/obj/machinery/power/apc/proc/interact(mob/user)
	...
	user.machine = src
	var/t = "[css_interfaces]<TT><B>Area Power Controller</B> ([area.name])<HR>"
	...
	if((locked || (setup_networkapc > 1)) && (!istype(user, /mob/living/silicon)))
		...
		t += "Main breaker: [operating ? "<A href='?src=\ref[src];breaker=1' class='toggle-off'>Off</A>" : "<A href='?src=\ref[src];breaker=1' class='toggle-on'>On</A>" ]<BR>"
		t += "External power : <B>[ main_status ? (main_status ==2 ? "<FONT COLOR=#004000>Good</FONT>" : "<FONT COLOR=#D09000>Low</FONT>") : "<FONT COLOR=#F00000>None</FONT>"]</B><BR>"
		if(cell)
			t += "Power cell: <B>[round(cell.percent())]%</B>"
			t += " ([charging ? ( charging == 1 ? "Charging" : "Fully charged" ) : "Not charging"])"
			t += " ([chargemode ? "<A href='?src=\ref[src];cmode=1' class='toggle-off'>Off</A>" : "<A href='?src=\ref[src];cmode=1' class='toggle-on'>Auto</A>"])"
		...
		switch(equipment)
			if(0)
				t += "<A href='?src=\ref[src];eqp=2' class='toggle-on'>On</A> <A href='?src=\ref[src];eqp=3' class='toggle-on'>Auto</A>"
			if(1)
				t += "<A href='?src=\ref[src];eqp=1' class='toggle-off'>Off</A> <A href='?src=\ref[src];eqp=2' class='toggle-on'>On</A>"
			if(2)
				t += "<A href='?src=\ref[src];eqp=1' class='toggle-off'>Off</A> <A href='?src=\ref[src];eqp=3' class='toggle-on'>Auto</A>"
			if(3)
				t += "<A href='?src=\ref[src];eqp=1' class='toggle-off'>Off</A> <A href='?src=\ref[src];eqp=2' class='toggle-on'>On</A>"
		t +="<BR>"
		...
		switch(lighting)
			if(0)
				t += "<A href='?src=\ref[src];lgt=2' class='toggle-off'>On</A> <A href='?src=\ref[src];lgt=3' class='toggle-on'>Auto</A>"
			if(1)
				t += "<A href='?src=\ref[src];lgt=1' class='toggle-off'>Off</A> <A href='?src=\ref[src];lgt=2' class='toggle-on'>On</A>"
			if(2)
				t += "<A href='?src=\ref[src];lgt=1' class='toggle-off'>Off</A> <A href='?src=\ref[src];lgt=3' class='toggle-on'>Auto</A>"
			if(3)
				t += "<A href='?src=\ref[src];lgt=1' class='toggle-off'>Off</A> <A href='?src=\ref[src];lgt=2' class='toggle-on'>On</A>"
		t +="<BR>"
		...
		switch(environ)
			if(0)
				t += "<A href='?src=\ref[src];env=2' class='toggle-off'>On</A> <A href='?src=\ref[src];env=3' class='toggle-on'>Auto</A>"
			if(1)
				t += "<A href='?src=\ref[src];env=1' class='toggle-off'>Off</A> <A href='?src=\ref[src];env=2' class='toggle-on'>On</A>"
			if(2)
				t += "<A href='?src=\ref[src];env=1' class='toggle-off'>Off</A> <A href='?src=\ref[src];env=3' class='toggle-on'>Auto</A>"
			if(3)
				t += "<A href='?src=\ref[src];env=1' class='toggle-off'>Off</A> <A href='?src=\ref[src];env=2' class='toggle-on'>On</A>>"
		...
		t += "<BR>Total load: [lastused_light + lastused_equip + lastused_environ] W</PRE>"
		t += "<HR>Cover lock:<BR> [coverlocked ? "<B><A href='?src=\ref[src];lock=1' class='toggle-on'>Engaged</A></B>" : "<B><A href='?src=\ref[src];lock=1' class='toggle-off'>Disengaged</A></B>"]"
	...
	t += "</TT>"
	user << browse(t, "window=apc;size=521x402")
	onclose(user, "apc")
	return
...
/obj/machinery/power/apc/proc/apcelectrocute(mob/user, prb, netnum, bite)
	...
	if (user.bioHolder.HasEffect("resist_electric") == 2)
		...
		boutput(user, "<span style=\"color:orange\">You absorb the electrical shock, healing your body!</span>")
		return
	else if (user.bioHolder.HasEffect("resist_electric") == 1)
		boutput(user, "<span style=\"color:orange\">You feel electricity course through you harmlessly!</span>")
		return
