...
/obj/item/magnetizer
	...
	examine()
		..()
		if (loaded)
			boutput(usr, "<span style=\"color:orange\">The magnetizer is loaded with a plasmastone. Designate the mineral magnet to attach, then designate the lower left tile of the area to magnetize.</span>")
			boutput(usr, "<span style=\"color:orange\">The magnetized area must be a clean shot of space, surrounded by bordering tiles on all sides.</span>")
			boutput(usr, "<span style=\"color:orange\">A small mineral magnet requires an 7x7 area of space, a large one requires a 15x15 area of space.</span>")
		else
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if (istype(W, /obj/item/raw_material/plasmastone) && !loaded)
			loaded = 1
			boutput(user, "<span style=\"color:orange\">You charge the magnetizer with the plasmastone.</span>")
			qdel(W)
		...
		if (!magnet)
			...
			if (!istype(magnet))
				...
				if (!loaded)
					...
					boutput(user, "<span style=\"color:orange\">Magnet locked. Designate lower left tile of target area (excluding the borders).</span>")
			...
			if (dist > 10)
				...
				boutput(user, "<span style=\"color:orange\">Designation successful. The magnet is now fully operational.</span>")
				magnet.target = M
				loaded = 0
				magnet = null
...
/turf/simulated/wall/asteroid
	...
	New(var/loc,var/do_overlays_now = 1)
		src.icon_state = pick("asteroid0","asteroid1","asteroid2","asteroid3","asteroid4","asteroid5","asteroid6","asteroid7","asteroid8","asteroid9","asteroid10","asteroid11","asteroid12")
		..()
		if (do_overlays_now)
			src.space_overlays()
...
/obj/item/mining_tool/power_pick
	...
	attack_self(var/mob/user as mob)
		if (src.process_charges(0))
			if (!src.status)
				boutput(user, "<span style=\"color:orange\">You power up [src].</span>")
				src.power_up()
			else
				boutput(user, "<span style=\"color:orange\">You power down [src].</span>")
				src.power_down()
		else
...
/obj/item/breaching_charge/mining
	...
	emag_act(var/mob/user, var/obj/item/card/emag/E)
		...
		if(!src.emagged && !src.hacked)
			if (user)
				boutput(user, "<span style=\"color:orange\">You short out the timing mechanism!</span>")
	...
	demag(var/mob/user)
		if (!src.emagged)
			return 0
		if (user)
			boutput(user, "<span style=\"color:orange\">You repair the timing mechanism!</span>")
		src.emagged = 0
		src.desc = null
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if (istype(W, /obj/item/device/chargehacker))
			if(!src.emagged && !src.hacked)
				boutput(user, "<span style=\"color:orange\">You short out the attachment mechanism, removing its restrictions!</span>")
				src.desc += " It has been tampered with."
				src.hacked = 1
			else
				boutput(user, "<span style=\"color:red\">This has already been tampered with.</span>")
...
/obj/item/cargotele
	...
	proc/cargoteleport(var/obj/T, var/mob/user)
		...
		boutput(user, "<span style=\"color:orange\">Teleporting [T]...</span>")
		...
		if(do_after(user, 50))
			...
			if (istype(user,/mob/living/silicon/robot))
				...
				if (src.charges == 0)
					...
					boutput(user, "<span style=\"color:orange\">Transfer successful. [src.charges] charges remain.</span>")
		return
