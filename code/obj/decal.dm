...
/obj/decal/poster
	...
	wallsign
		...
		restrooms
			name = "RESTROOMS"
			desc = "Just restrooms, for everyone."
			icon_state = "wall_toilet"
	...
	proc/Sample(var/obj/item/W as obj, var/mob/user as mob)
		...
		if (src.reagents)
			if (W.reagents.total_volume >= W.reagents.maximum_volume - (src.reagents.total_volume - 1))
				...
				user.visible_message("<span style=\"color:orange\"><b>[user]</b> [src.sample_verb]s some of [src] into [W].</span>",\
				"<span style=\"color:orange\">You [src.sample_verb] some of [src] into [W].</span>")
...
/obj/decal/cleanable/blood/gibs
	...
	attack_hand(var/mob/user as mob)
		if (istype(user, /mob/living/carbon/human))
			...
			if (H.job == "Chef")
				user.visible_message("<span style=\"color:orange\"><b>[H]</b> starts rifling through [src] with their hands. What a weirdo.</span>",\
				"<span style=\"color:orange\">You rake through the gibs with your bare hands.</span>")
	...
	Sample(var/obj/item/W as obj, var/mob/user as mob)
		...
		if (W.is_open_container() && W.reagents)
			if (W.reagents.total_volume >= W.reagents.maximum_volume - 2)
				...
				user.visible_message("<span style=\"color:orange\"><b>[user]</b> is splashing the urine puddle into \the [W]. How singular.</span>",\
				"<span style=\"color:orange\">You splash a little urine into \the [W].</span>")
	...
	Sample(var/obj/item/W as obj, var/mob/user as mob)
		...
		if (src.sample_amt && src.sample_reagent)
			if (W.reagents.total_volume >= W.reagents.maximum_volume - (src.sample_amt - 1))
				...
				W.reagents.add_reagent(src.sample_reagent, src.sample_amt)
				user.visible_message("<span style=\"color:orange\"><b>[user]</b> is sticking their fingers into [src] and pushing it into [W]. It's probably best not to ask.</span>",\
				"<span style=\"color:orange\">You [src.sample_verb] some of the puke into [W]. You are absolutely disgusting.</span>")
				W.reagents.handle_reactions()
				playsound(src.loc, "sound/effects/splat.ogg", 50, 1)
				src.sampled = 1
				return 1
...
/obj/decal/cleanable/vomit/spiders
	...
	Sample(var/obj/item/W as obj, var/mob/user as mob)
		...
		if (W.reagents.total_volume >= W.reagents.maximum_volume - (src.sample_amt - 1))
			...
			user.visible_message("<span style=\"color:orange\"><b>[user]</b> is sticking their fingers into [src] and pushing it into [W].<span style=\"color:red\">It [fluff] a bit.[fluff2]</span></span>",\
			"<span style=\"color:orange\">You [src.sample_verb] some of the puke into [W].<span style=\"color:red\">It [fluff] a bit.[fluff2]</span></span>")
			W.reagents.handle_reactions()
			playsound(src.loc, "sound/effects/splat.ogg", 50, 1)
			src.sampled = 1
			return 1
		...
		if (src.sample_amt && src.sample_reagent)
			if (W.reagents.total_volume >= W.reagents.maximum_volume - (src.sample_amt - 1))
				...
				for (var/mob/O in AIviewers(user, null))
					if (O != user)
						O.show_message("<span style=\"color:orange\"><b>[user]</b> is sticking their fingers into [src] and pushing it into [W]. It's all slimy and stringy. Oh god.</span>", 1)
...
/obj/decal/cleanable/nitrotriiodide
	...
	attack_hand(var/mob/user)
		if (src.dry)
			...
			boutput(user, "<span style=\"color:orange\">You poke the mess. It's slightly viscous and smells strange. [prob(25) ? pick("Ew.", "Grody.", "Weird.") : null]</span>")
...
/obj/decal/jukebox
	...
	attack_hand(mob/user as mob)
		user << link("http://tube.d2k5.com/r/spess13")
