...
/obj/mopbucket/get_desc(dist)
	...
	if (!reagents)
		return
	. = "<br><span style=\"color:orange\">It contains:</span>"
	if(reagents.reagent_list.len)
		for(var/current_id in reagents.reagent_list)
			var/datum/reagent/current_reagent = reagents.reagent_list[current_id]
			. += "<br><span style=\"color:orange\">[current_reagent.volume] units of [current_reagent.name]</span>"
	else
		. += "<br><span style=\"color:orange\">Nothing. The answer is nothing.</span>"

/obj/mopbucket/attackby(obj/item/W as obj, mob/user as mob)
	if (istype(W, /obj/item/mop))
		if (src.reagents.total_volume >= 2)
			src.reagents.trans_to(W, 2)
			boutput(user, "<span style=\"color:orange\">You wet the mop</span>")
			playsound(src.loc, "sound/effects/slosh.ogg", 25, 1)
		if (src.reagents.total_volume < 1)
			boutput(user, "<span style=\"color:orange\">Out of water!</span>")
	else
		return ..()
