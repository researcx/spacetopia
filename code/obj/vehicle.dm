...
/obj/vehicle/segway/eject_rider(var/crashed, var/selfdismount)
	...
	if(crashed)
		...
		return
	if(selfdismount)
		boutput(rider, "<span style=\"color:orange\">You dismount from the [src].</span>")
		for (var/mob/C in AIviewers(src))
			if(C == rider)
				continue
...
/obj/vehicle/segway/MouseDrop_T(mob/living/carbon/human/target, mob/user)
	...
	if(target == user && !user.stat)	// if drop self, then climbed in
		...
		boutput(user, "<span style=\"color:orange\">You climb onto the [src].</span>")
	else if(target != user && !user.restrained())
		msg = "[user.name] helps [target.name] onto the [src]!"
		boutput(user, "<span style=\"color:orange\">You help [target.name] onto the [src]!</span>")
	else
		return
...
/obj/ability_button/toggle_buffer
	...
	Click()
		...
		if(istype(my_mob.loc, /obj/vehicle/floorbuffer))
			FB = my_mob.loc
			active = !active
			boutput(my_mob, "<span style=\"color:orange\"><B>You turn [active ? "on" : "off"] the floor buffer's sprayer.</span></B>")
	...
	if(..() && rider)
		...
		spawn(0)
			if (src.reagents.total_volume < 1)
				...
				boutput(rider, "<span style=\"color:orange\"><B>The \"Storage Tank Low\" indicator light starts blinking on the [src.name]'s dashboard.</B></span>")
...
/obj/vehicle/floorbuffer/attackby(obj/item/W as obj, mob/user as mob)
	if(istype(W, /obj/item/reagent_containers) && W.is_open_container() && W.reagents)
		...
		boutput(user, "<span style=\"color:orange\">You empty [trans] units of the solution into the [src.name]'s holding tank.</span>")
		return
	..()
	...
	if(M.bioHolder.HasEffect("hulk"))
		...
		if(rider && prob(40))
			...
			if(src.contents.len)
				...
				boutput(M, "<span style=\"color:orange\">There's nothing inside of the [src].</span>")
				return
	return
...
/obj/vehicle/clowncar/MouseDrop_T(mob/living/carbon/human/target, mob/user)
	...
	if(ishuman(user))
		...
		if(istype(user:wear_mask, /obj/item/clothing/mask/clown_hat))
			clown_tally += 1
	if(clown_tally < 2)
		boutput(user, "<span style=\"color:orange\">You don't feel funny enough to use the [src].</span>")
		return
	...
	if(target == user && !user.stat)	// if drop self, then climbed in
		...
		boutput(user, "<span style=\"color:orange\">You climb into the driver's seat of the [src].</span>")
		...
		boutput(user, "<span style=\"color:orange\">You stuff [target.name] into the back of the [src]!</span>")
	else
		return
...
/obj/vehicle/clowncar/eject_rider(var/crashed, var/selfdismount)
	...
	if(crashed)
		...
		return
	if(selfdismount)
		boutput(rider, "<span style=\"color:orange\">You climb out of the [src].</span>")
		for (var/mob/C in AIviewers(src))
			if(C == rider)
				continue
...
/obj/vehicle/clowncar/attackby(var/obj/item/I, var/mob/user)
	...
	if(istype(G))	// handle grabbed mob
		if(ismob(G.affecting))
			...
			GM.set_loc(src)
			boutput(user, "<span style=\"color:orange\">You stuff [GM.name] into the back of the [src].</span>")
...
/obj/vehicle/adminbus/MouseDrop_T(mob/living/carbon/human/target, mob/user)
	...
	if(!(user.client && user.client.holder))
		boutput(user, "<span style=\"color:orange\">You don't feel cool enough to use the [src].</span>")
		return

	if(target == user && !user.stat)	// if drop self, then climbed in
		if(rider)
			msg = "[user.name] climbs into the front of the [src]."
			boutput(user, "<span style=\"color:orange\">You climb into the front of the [src].</span>")
		else
			rider = target
			msg = "[user.name] climbs into the driver's seat of the [src]."
			boutput(user, "<span style=\"color:orange\">You climb into the driver's seat of the [src].</span>")
...
/obj/vehicle/forklift/MouseDrop_T(atom/movable/A as obj|mob, mob/user as mob)
	...
	if((istype(A, /obj/storage/crate) || istype(A, /obj/storage/cart)) && get_dist(A, src) <= 1 && src.rider == usr && helditems.len != helditems_maximum && !broken)
		...
		helditems.Add(A)
		update_overlays()
		boutput(usr, "<span style=\"color:orange\"><B>You pick up the [A.name].</B></span>")
		for (var/mob/C in AIviewers(src))
			if(C == rider)
				continue
			C.show_message("<span style=\"color:orange\"><B>[src] picks up the [A.name].</B></span>", 1)
		return
	...
	if(istype(A, /mob/living/carbon/human/) && get_dist(usr, src) <= 1  && get_dist(A, usr) <= 1 && !rider)
		if (A == usr)
			...
			boutput(user, "<span style=\"color:orange\">You help [A] onto [src]!</span>")
		A.set_loc(src)
		src.rider = A
		src.update_overlays()
		return
...
/obj/vehicle/forklift/verb/drop_crates()
	...
	if(helditems.len >= 1)
		...
		if(helditems.len == 1)
			...
			for (var/mob/C in AIviewers(src))
				C.show_message("<span style=\"color:orange\"><B>[src] leaves the [O.name] on [src.loc].</B></span>", 1)
			boutput(usr, "<span style=\"color:orange\"><B>You leave the [O.name] on [src.loc].</B></span>")
		if(helditems.len > 1)
			for (var/mob/C in AIviewers(src))
				C.show_message("<span style=\"color:orange\"><B>[src] leaves [helditems.len] crates on [src.loc].</B></span>", 1)
			boutput(usr, "<span style=\"color:orange\"><B>You leave [helditems.len] crates on [src.loc].</B></span>")
...
obj/vehicle/forklift/attackby(var/obj/item/I, var/mob/user)
	...
	if(istype(I,/obj/item/wirecutters))
		if (openpanel && !broken)
			boutput(usr, "<span style=\"color:orange\">You cut [src]'s wires!<span>")
	...
	if(istype(I,/obj/item/cable_coil))
		if (openpanel && broken)
			...
			coil.use(5)
			boutput(usr, "<span style=\"color:orange\">You replace [src]'s wires!</span>")
			broken = 0
