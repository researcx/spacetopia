...
/obj/table/MouseDrop_T(obj/O as obj, mob/user as mob)
	...
	if (istype(O,/obj/item/satchel/))
		if (O.contents.len < 1)
			...
			user.visible_message("<span style=\"color:orange\">[user] dumps out [O]'s contents onto [src]!</span>")
...
/obj/table/attackby(obj/item/W as obj, mob/user as mob, params)
	if (istype(W, /obj/item/grab))
		...
		boutput(user, "<span style=\"color:orange\">Now disassembling table</span>")
...
/obj/table/reinforced/attackby(obj/item/W as obj, mob/user as mob)
	if (istype(W, /obj/item/weldingtool) && W:welding)
		var/turf/T = user.loc
		if(src.status == 2)
			boutput(user, "<span style=\"color:orange\">Now weakening the reinforced table</span>")
			playsound(src.loc, "sound/items/Welder.ogg", 50, 1)
			sleep(50)
			if ((user.loc == T && user.equipped() == W))
				boutput(user, "<span style=\"color:orange\">Table weakened</span>")
				src.status = 1
			else if((istype(user, /mob/living/silicon/robot) && (user.loc == T)))
				boutput(user, "<span style=\"color:orange\">Table weakened</span>")
				src.status = 1
		else
			boutput(user, "<span style=\"color:orange\">Now strengthening the reinforced table</span>")
			playsound(src.loc, "sound/items/Welder.ogg", 50, 1)
			sleep(50)
			if ((user.loc == T && user.equipped() == W))
				boutput(user, "<span style=\"color:orange\">Table strengthened</span>")
				src.status = 2
			else if((istype(user, /mob/living/silicon/robot) && (user.loc == T)))
				boutput(user, "<span style=\"color:orange\">Table strengthened</span>")
				src.status = 2
		...
		var/turf/T = user.loc
		if(src.status == 1)
			boutput(user, "<span style=\"color:orange\">Now disassembling the reinforced table</span>")
			playsound(src.loc, "sound/items/Ratchet.ogg", 50, 1)
			sleep(50)
			if (user.loc == T)
				var/atom/A = new /obj/item/table_parts/reinforced( src.loc )
