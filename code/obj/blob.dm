...
/obj/blob/deposit
	...
	examine()
		...
		if (usr == overmind)
			if (movable)
				boutput(usr, "<span style=\"color:orange\">Clickdrag this onto any standard (not special) blob tile to move the reagent deposit there.</span>")
			...
			for (var/id in src.reagents.reagent_list)
				...
				boutput(usr, "<span style=\"color:orange\">- [R.volume] unit[R.volume != 1 ? "s" : null] of [R.name]</span>")
	...
	replicator
		...
		MouseDrop_T(atom/movable/O as mob|obj, mob/user as mob)
			...
			if (O.type == /obj/blob/deposit)
				...
				O.reagents = null
				boutput(user, "<span style=\"color:orange\">Reagents transferred to the replicator.</span>")
				qdel(O)
				update_reagent_overlay()
				progress.onUpdate()
		...
		onMove(var/obj/blob/B)
			...
			boutput(usr, "<span style=\"color:orange\">Transferred [trans_amt] reagents into a deposit.</span>")
			update_reagent_overlay()
...
/obj/blob/launcher
	...
	MouseDrop_T(atom/movable/O as mob|obj, mob/user as mob)
		...
		if (O.type == /obj/blob/deposit)
			...
			O.reagents = null
			boutput(user, "<span style=\"color:orange\">Reagents transferred to the slime launcher.</span>")
			qdel(O)
