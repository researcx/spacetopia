...
/obj/stool/bed
	...
	proc/buckle_mob(var/mob/living/carbon/C as mob, var/mob/user as mob)
		...
		if (C == user)
			user.visible_message("<span style=\"color:orange\"><b>[C]</b> buckles in!</span>", "<span style=\"color:orange\">You buckle yourself in.</span>")
		else
			user.visible_message("<span style=\"color:orange\"><b>[C]</b> is buckled in by [user].</span>", "<span style=\"color:orange\">You buckle in [C].</span>")
		buckle_in(C)
	...
	proc/unbuckle_mob(var/mob/M as mob, var/mob/user as mob)
		if (M.buckled && !user.restrained())
			if (allow_unbuckle)
				if (M != user)
					user.visible_message("<span style=\"color:orange\"><b>[M]</b> is unbuckled by [user].</span>", "<span style=\"color:orange\">You unbuckle [M].</span>")
				else
					user.visible_message("<span style=\"color:orange\"><b>[M]</b> unbuckles.</span>", "<span style=\"color:orange\">You unbuckle.</span>")
				unbuckle()
			else
	...
	proc/tuck_sheet(var/obj/item/clothing/suit/bedsheet/newSheet as obj, var/mob/user as mob)
		...
		if (!src.Sheet && (newSheet.loc == src.loc || user.find_in_hand(newSheet))) // finally, do we have room for the new sheet, and is the sheet where we are or in the hand of the user?
			...
			if (somebody && somebody.lying)
				user.tri_message("<span style=\"color:orange\"><b>[user]</b> tucks [somebody == user ? "[him_or_her(user)]self" : "[somebody]"] into bed.</span>",\
				user, "<span style=\"color:orange\">You tuck [somebody == user ? "yourself" : "[somebody]"] into bed.</span>",\
				somebody, "<span style=\"color:orange\">[somebody == user ? "You tuck yourself" : "<b>[user]</b> tucks you"] into bed.</span>")
				newSheet.layer = EFFECTS_LAYER_BASE-1
				return
			else
				user.visible_message("<span style=\"color:orange\"><b>[user]</b> tucks [newSheet] into [src].</span>",\
				"<span style=\"color:orange\">You tuck [newSheet] into [src].</span>")
				return
	...
	proc/untuck_sheet(var/mob/user as mob)
		...
		if (user)
			...
			if (src.buckled_guy)
				...
				somebody = locate(/mob/living/carbon) in get_turf(src)
			if (somebody && somebody.lying)
				user.tri_message("<span style=\"color:orange\"><b>[user]</b> untucks [somebody == user ? "[him_or_her(user)]self" : "[somebody]"] from bed.</span>",\
				user, "<span style=\"color:orange\">You untuck [somebody == user ? "yourself" : "[somebody]"] from bed.</span>",\
				somebody, "<span style=\"color:orange\">[somebody == user ? "You untuck yourself" : "<b>[user]</b> untucks you"] from bed.</span>")
				oldSheet.layer = initial(oldSheet.layer)
			else
				user.visible_message("<span style=\"color:orange\"><b>[user]</b> untucks [oldSheet] from [src].</span>",\
				"<span style=\"color:orange\">You untuck [oldSheet] from [src].</span>")
...
/obj/stool/chair
	...
	attack_hand(mob/user as mob)
		...
		for (var/mob/M in src.loc)
			...
			if (H.on_chair == 1)
				if (M == user)
					user.visible_message("<span style=\"color:orange\"><b>[M]</b> steps off [src].</span>", "<span style=\"color:orange\">You step off [src].</span>")
					src.add_fingerprint(user)
					unbuckle()
					return
	...
	MouseDrop_T(mob/M as mob, mob/user as mob)
		...
		if (M == usr)
			if (usr.a_intent == INTENT_GRAB)
				user.visible_message("<span style=\"color:orange\"><b>[M]</b> climbs up on [src]!</span>", "<span style=\"color:orange\">You climb up on [src].</span>")
				buckle_in(M, 1)
			else
				user.visible_message("<span style=\"color:orange\"><b>[M]</b> buckles in!</span>", "<span style=\"color:orange\">You buckle yourself in.</span>")
				buckle_in(M)
		else
			user.visible_message("<span style=\"color:orange\"><b>[M]</b> is buckled in by [user].</span>", "<span style=\"color:orange\">You buckle in [M].</span>")
			buckle_in(M)
		return
...
//		src.overl.dir = src.dir
	...
	white
		name = "comfy white chair"
		icon_state = "chair_comfy-white"
		arm_icon_state = "arm-white"
...
/obj/stool/chair/wooden/round
	icon_state = "chair_wooden_round"


/obj/stool/chair/wooden/red
	name="red cushioned wooden chair"
	icon_state = "chair_wooden_red"
...
/obj/stool/chair/couch
	...
	redwhite
		name = "comfy red and white couch"
		icon_state = "chair_couch-redwhite"
