...
/datum/artifact/healer_bio
	...
	effect_touch(var/obj/O,var/mob/living/user)
		...
		if(istype(user,/mob/living/carbon/))
			...
			boutput(C, "<span style=\"color:orange\">Soothing energy saturates your body, making you feel refreshed and healthy.</span>")
		if (field_range > 0)
			for (var/mob/living/carbon/C in range(field_range,T))
				if (C == user)
					continue
				C.HealDamage("All", heal_amt, heal_amt)
				boutput(C, "<span style=\"color:orange\">Waves of soothing energy wash over you, making you feel refreshed and healthy.</span>")
		spawn(recharge_time)
			recharging = 0
