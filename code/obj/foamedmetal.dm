...
/obj/foamedmetal
	...
	attack_hand(var/mob/user)
		if (user.bioHolder.HasEffect("hulk") || (prob(75 - metal*25)))
			...
			boutput(user, "<span style=\"color:orange\">You hit the metal foam but bounce off it.</span>")
		return
	...
	attackby(var/obj/item/I, var/mob/user)
		...
		if(prob(I.force*20 - metal*25))
			user.visible_message( "<span style=\"color:red\">[user] smashes through the foamed metal.</span>", "<span style=\"color:orange\">You smash through the foamed metal with \the [I].</span>")
			dispose()
		else
			boutput(user, "<span style=\"color:orange\">You hit the metal foam to no effect.</span>")
