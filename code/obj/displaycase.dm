	...
	examine()
		...
		if (src.repair_stage > 0)
			switch (src.repair_stage)
				if (1)
					boutput(usr, "<br><span style=\"color:orange\">The maintenance panel is open, revealing a largely empty circuit board.</span>")
				if (2)
					boutput(usr, "<br><span style=\"color:orange\">The wiring has been replaced, but there's still an empty spot in the circuit board.</span>")
				if (3)
					boutput(usr, "<br><span style=\"color:orange\">A new coil has been inserted, but not secured yet.</span>")
				if (4)
					boutput(usr, "<br><span style=\"color:orange\">The coil has been soldered into place, but there's nothing to focus the laser beam.</span>")
				if (5)
					boutput(usr, "<br><span style=\"color:orange\">A lens has been installed, but the control circuits aren't set up yet.</span>")
				if (6)
					boutput(usr, "<br><span style=\"color:orange\">The gun appears to be missing a power cell.</span>")
				if (7)
					boutput(usr, "<br><span style=\"color:orange\">Power cell installed. The maintenance panel is open.</span>")
		return
	...
	attackby(obj/item/O as obj, mob/user as mob)
		if (istype(O, /obj/item/screwdriver))
			if (src.repair_stage == 0)
				...
				user.visible_message("<span style=\"color:orange\">[user] finishes repairing the [src.name].</span>", "<span style=\"color:orange\">You close the maintenance panel and power up the gun.</span>")
