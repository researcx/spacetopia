/obj/npc/trader
		...
		//////////////////////////////////
			...
			if (src.scan) src.scan = null
				...
				if (istype(I, /obj/item/card/id))
					boutput(usr, "<span style=\"color:orange\">You swipe the ID card in the card reader.</span>")
					var/datum/data/record/account = null
					account = FindBankAccountByName(I:registered)
					if(account)
						var/enterpin = input(usr, "Please enter your PIN number.", "Card Reader", 0) as null|num
						if (enterpin == I:pin)
							boutput(usr, "<span style=\"color:orange\">Card authorized.</span>")
							src.scan = I
						else
	...
	MouseDrop_T(atom/movable/O as obj, mob/user as mob)
		...
		/*if (istype(O, /obj/item))
			user.visible_message("<span style=\"color:orange\">[src] rummages through [user]'s goods.</span>")
			var/staystill = user.loc
			for(var/datum/commodity/N in goods_buy)
				if (N.comtype == O.type)
					user.visible_message("<span style=\"color:orange\">[src] is willing to buy all of [O].</span>")
		...
		if (istype(O, /obj/storage/crate/))
			if (O:locked)
				...
				return
			spawn(1)
				user.visible_message("<span style=\"color:orange\">[src] rummages through [user]'s [O].</span>")
				playsound(src.loc, "rustle", 60, 1)
				var/cratevalue = null
				for (var/obj/M in O.contents)
					//boutput(world, "<span style=\"color:orange\">HELLO I AM [M]</span>")
					//boutput(world, "<span style=\"color:orange\">AND MY TYPE PATH IS [M.type]</span>")
					for(var/datum/commodity/N in src.goods_buy)
						if(M) // fuck the hell off you dirty null.type errors
							if(N.comtype == M.type)
								...
								//boutput(world, "<span style=\"color:orange\">[M] IS GOING TO SELL FOR [N.price] HOT BALLS</span>")
								//boutput(world, "<span style=\"color:orange\">UPDATING CRATE VALUE TO [cratevalue] OR FUCKING ELSE</span>")
								cratevalue += N.price
								qdel( M )
				if(cratevalue)
					boutput(user, "<span style=\"color:orange\">[src] takes what they want from [O]. [cratevalue] credits have been transferred to your account.</span>")
					account.fields["current_money"] += cratevalue
				else
					boutput(user, "<span style=\"color:orange\">[src] finds nothing of interest in [O].</span>")
