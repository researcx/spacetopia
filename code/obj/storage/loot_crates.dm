	...
	attack_hand(mob/user as mob)
		if(istype(lock) && locked)
			...
			if (success_state == 1) // Succeeded
				boutput(user, "<span style=\"color:orange\">The crate unlocks!</span>")
				src.locked = 0
				src.lock = null
				src.trap = null
				src.update_icon()
...
/datum/loot_crate_lock/decacode
	...
	attempt_to_open(var/mob/living/opener)
		boutput(opener, "<span style=\"color:orange\">The crate is locked with a deca-code lock.</span>")
...
/datum/loot_crate_lock/hangman
	...
	attempt_to_open(var/mob/living/opener)
		boutput(opener, "<span style=\"color:orange\">The crate is locked with a password lock. You'll need a multitool to get very far here.</span>")
	...
	unequipped(var/mob/user)
		...
		if (src.twin && istype(src.twin.loc,/mob/living/carbon/human/))
			...
			if (psy.bioHolder && psy.bioHolder.HasEffect("psy_resist"))
				return
			if (psy.gloves == src.twin)
				boutput(user, "<span style=\"color:orange\">The strange hallcuinations suddenly stop. That was weird.</span>")
				boutput(psy, "<span style=\"color:orange\">The strange hallcuinations suddenly stop. That was weird.</span>")
