...
/obj/storage
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if (istype(W, /obj/item/cargotele))
			...
			if (amt)
				user.visible_message("<span style=\"color:orange\">[user] dumps out [W]'s contents into [src]!</span>")
				var/amtload = 0
		...
		if (src.secure)
			if (src.emagged)
				...
				if (src.allowed(user, req_only_one_required) || !src.registered || (istype(W, /obj/item/card/id) && src.registered == I.registered))
					...
					user.visible_message("<span style=\"color:orange\">The locker has been [src.locked ? null : "un"]locked by [user].</span>")
					src.update_icon()
					if (!src.registered)
				...
				if (!src.open)
					src.locked = !src.locked
					user.visible_message("<span style=\"color:orange\">[src] has been [src.locked ? null : "un"]locked by [user].</span>")
					src.update_icon()
					for (var/mob/M in src.contents)
						src.log_me(user, M, src.locked ? "locks" : "unlocks")
					return
	...
	MouseDrop_T(atom/movable/O as mob|obj, mob/user as mob)
		...
		spawn(5)
			if (istype(O, /obj/item/raw_material/))
				...
				"<span style=\"color:orange\">You begin quickly stuffing materials into [src]!</span>")
				...
				user.visible_message("<span style=\"color:orange\">[user] begins quickly stuffing produce into [src]!</span>",\
				"<span style=\"color:orange\">You begin quickly stuffing produce into [src]!</span>")
				var/staystill = user.loc
