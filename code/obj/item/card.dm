...
var/global/list/available_ids = list()
...
/obj/item/card/id/syndicate/attack_self(mob/user as mob)
	if(!src.registered)
		...
		var/ass = copytext(src.sanitize_name(input(user, "What occupation would you like to put on this card?\n Note: This will not grant any access levels other than Maintenance.", "Agent card job assignment", "Civilian"), 1), 1, 100)
		...
		boutput(user, "<span style=\"color:orange\">You successfully forge the ID card.</span>")
	else
		..()
...
/*/obj/item/card/id/gauntlet gauntlet removal
