	...
	*/
	...
	proc/fire_gun(mob/user as mob)
		if(src.shotsLeft > 1)
			...
			boutput(user, "<span style=\"color:orange\">You need to reload the gun.</span>")
			return 0

	proc/reload_gun(mob/user as mob)
		if(src.shotsLeft <= 0)
			user.visible_message("<span style=\"color:orange\">[user] finds a bullet on the ground and loads it into the gun, spinning the cylinder.</span>", "<span style=\"color:orange\">You find a bullet on the ground and load it into the gun, spinning the cylinder.</span>")
			src.shotsLeft = rand(1, shotsMax)
		else if(src.shotsLeft >= 1)
			user.visible_message("<span style=\"color:orange\">[user] spins the cylinder.</span>", "<span style=\"color:orange\">You spin the cylinder.</span>")
			src.shotsLeft = rand(1, shotsMax)
