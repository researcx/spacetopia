/obj/item/raw_material/
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if (istype(W, /obj/item/satchel/mining/))
			if (W:contents.len < W:maxitems)
				...
				boutput(user, "<span style=\"color:orange\">You put [src] in [W].</span>")
				src.desc = "A leather bag. It holds [oreamt]/[W:maxitems] [W:itemstring]."
				if (oreamt == W:maxitems) boutput(user, "<span style=\"color:orange\">[W] is now full!</span>")
	...
	MouseDrop(over_object, src_location, over_location)
		...
		if (isturf(over_object))
			...
			usr.visible_message("<span style=\"color:orange\">[usr.name] begins sorting [src] into a pile!</span>")
			...
			boutput(usr, "<span style=\"color:orange\">You finish sorting [src]!</span>")
...
/obj/machinery/portable_reclaimer
	...
	MouseDrop(over_object, src_location, over_location)
		...
		if(over_object == src)
			...
			boutput(usr, "<span style=\"color:orange\">You reset the reclaimer's output target.</span>")
			return
		...
		if (istype(over_object,/obj/storage/crate/))
			...
			if (C.locked || C.welded)
				...
				boutput(usr, "<span style=\"color:orange\">You set the reclaimer to output to [over_object]!</span>")
			...
			boutput(usr, "<span style=\"color:orange\">You set the reclaimer to output on top of [O]!</span>")
	...
	proc/quickload(var/mob/living/user,var/obj/item/O)
		...
		for(var/obj/item/M in view(1,user))
			...
			if (M.name != O.name)
				continue
			if(!istype(M, /obj/item/cable_coil)) // Hack Hack - Adding the entire checking of wire contents here would be pretty pasta.
				if (!istype(M.material))
					continue
				if (!M.material.material_flags & MATERIAL_CRYSTAL || !M.material.material_flags & MATERIAL_METAL)
					continue
