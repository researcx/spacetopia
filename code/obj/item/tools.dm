...
/obj/item/weldingtool/attackby(obj/item/W as obj, mob/user as mob)
	if (status == 0 && istype(W,/obj/item/screwdriver))
		status = 1
		boutput(user, "<span style=\"color:orange\">The welder can now be attached and modified.</span>")
		...
		status = 0
		boutput(user, "<span style=\"color:orange\">You resecure the welder.</span>")
		return
...
/obj/item/weldingtool/afterattack(obj/O as obj, mob/user as mob)
	if (istype(O, /obj/reagent_dispensers/fueltank) && get_dist(src,O) <= 1)
		...
		boutput(user, "<span style=\"color:orange\">Welder refueled</span>")
		playsound(src.loc, "sound/effects/zzzt.ogg", 50, 1, -6)

	else if (src.welding)
		use_fuel(1)
		if (get_fuel() <= 0)
			boutput(usr, "<span style=\"color:orange\">Need more fuel!</span>")
			src.welding = 0
			src.force = 3
			src.damtype = "brute"
		...
		if (!ismob(O) && O.reagents)
			boutput(usr, "<span style=\"color:orange\">You heat \the [O.name]</span>")
...
/obj/item/weldingtool/attack_self(mob/user as mob)
	...
	if (src.welding)
		if (get_fuel() <= 0)
			boutput(user, "<span style=\"color:orange\">Need more fuel!</span>")
			src.welding = 0
			return 0
		boutput(user, "<span style=\"color:orange\">You will now weld when you attack.</span>")
		src.force = 15
		src.damtype = "fire"
		...
		boutput(user, "<span style=\"color:orange\">Not welding anymore.</span>")
		src.force = 3
		src.damtype = "brute"
		src.icon_state = "welder"
	user.update_inhands()
