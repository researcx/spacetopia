		...
		if (href_list["read"])
			...
			if ((P && P.loc == src))
				if (!( istype(usr, /mob/living/carbon/human) ))
					usr << browse(text("<HTML><HEAD><TITLE>[]</TITLE>[css_interfaces]</head><BODY><TT>[]</TT></BODY></HTML>", P.name, P.display()), text("window=[]", P.name))
					onclose(usr, "[P.name]")
				else
					usr << browse(text("<HTML><HEAD><TITLE>[]</TITLE>[css_interfaces]</head><BODY><TT>[]</TT></BODY></HTML>", P.name, P.display()), text("window=[]", P.name))
					onclose(usr, "[P.name]")
...
/obj/item/f_card/examine()
	...
	..()
	boutput(usr, text("<span style=\"color:orange\">There are [] on the stack!</span>", src.amount))
	usr << browse(text("<HTML><HEAD><TITLE>[]</TITLE>[css_interfaces]</head><BODY><TT>[]</TT></BODY></HTML>", src.name, display()), text("window=[]", src.name))
