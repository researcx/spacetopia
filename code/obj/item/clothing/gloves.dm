...
/obj/item/clothing/gloves
	...
	attackby(obj/item/W, mob/user)
		if (istype(W, /obj/item/cable_coil))
			...
			boutput(user, "<span style=\"color:orange\">You attach the wires to the [src.name].</span>")
			src.stunready = 1
		...
		if (istype(W, /obj/item/cell)) // Moved from cell.dm (Convair880).
			...
			if (src.can_be_charged)
				...
				user.visible_message("<span style=\"color:red\"><b>[user]</b> charges [his_or_her(user)] stun gloves.</span>", "<span style=\"color:orange\">The stun gloves now hold [src.uses]/[src.max_uses] charges!</span>")
			else
				user.visible_message("<span style=\"color:red\"><b>[user]</b> shocks themselves while fumbling around with [C]!</span>", "<span style=\"color:red\">You shock yourself while fumbling around with [C]!</span>")
				C.zap(user)
			return
