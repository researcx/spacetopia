...
/obj/item/clothing/glasses/healthgoggles
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if (istype(W, /obj/item/device/healthanalyzer_upgrade))
			if (src.scan_upgrade)
				...
				boutput(user, "<span style=\"color:orange\">Health scan upgrade installed.</span>")
	...
	attack_self(mob/user as mob)
		if (!src.scan_upgrade)
			...
			boutput(user, "<span style=\"color:orange\">Health scanner [src.health_scan ? "enabled" : "disabled"].</span>")
			return
