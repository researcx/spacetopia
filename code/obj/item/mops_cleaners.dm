...
/obj/item/spraybottle/afterattack(atom/A as mob|obj, mob/user as mob)
	...
	if (src.reagents.total_volume < 1)
		boutput(user, "<span style=\"color:orange\">The spray bottle is empty!</span>")
		return
...
/obj/item/spraybottle/examine()
	...
	set category = "Local"
	..()
	boutput(usr, "<span style=\"color:orange\">It contains:</span>")
	if(!reagents) return
	if(reagents.reagent_list.len)
		for(var/datum/reagent/R in reagents.reagent_list)
			boutput(usr, "<span style=\"color:orange\">[R.volume] units of [R.name]</span>")
	else
		boutput(usr, "<span style=\"color:orange\">Nothing.</span>")
...
/obj/item/mop/examine()
	...
	if(reagents && reagents.total_volume)
		boutput(usr, "<span style=\"color:orange\">[src] is wet!</span>")

/obj/item/mop/afterattack(atom/A, mob/user as mob)
	if (src.reagents.total_volume < 1 || mopcount >= 5)
		boutput(user, "<span style=\"color:orange\">Your mop is dry!</span>")
		return
...
/obj/item/sponge/examine()
	...
	..()
	if(reagents && reagents.total_volume)
		boutput(usr, "<span style=\"color:orange\">The sponge is wet!</span>")

/obj/item/sponge/attack_self(mob/user as mob)
	if(spam_flag)
		return
	var/turf/location = get_turf(user)
	user.visible_message("<span style=\"color:orange\">[user] wrings [src] out.</span>")
	if(location)
...
/obj/item/sponge/afterattack(atom/target, mob/user as mob)
	if(istype (target, /obj/item/reagent_containers/food/drinks) || istype (target, /obj/machinery/bathtub) && src.reagents.total_volume) // dump reagents to these things
		...
		if(src.reagents.total_volume == src.reagents.maximum_volume)
			...
			return
		if(T.reagents)
			boutput(user, "<span style=\"color:orange\">You soak up the mess from [T].</span>")
			T.reagents.trans_to(src, 10)
		if(T.wet)
			boutput(user, "<span style=\"color:orange\">You dry [T] up with [src].</span>")
