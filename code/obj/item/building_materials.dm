...
/obj/item/sheet
	...
	attackby(obj/item/W, mob/user as mob)
		if (istype(W, /obj/item/sheet))
			...
			if (S.amount + src.amount > src.max_stack)
				...
				boutput(user, "<span style=\"color:orange\">You add [S] to the stack. It now has [S.amount] sheets.</span>")
			else
				S.amount += src.amount
				boutput(user, "<span style=\"color:orange\">You add [S] to the stack. It now has [S.amount] sheets.</span>")
				//SN src = null
	...
	before_stack(atom/movable/O as obj, mob/user as mob)
		user.visible_message("<span style=\"color:orange\">[user] begins gathering up [src]!</span>")

	after_stack(atom/movable/O as obj, mob/user as mob, var/added)
		boutput(user, "<span style=\"color:orange\">You finish gathering sheets.</span>")
...
/obj/item/tile
	...
	attack_self(mob/user as mob)
		...
		var/T = user.loc
		if (!( istype(T, /turf) ))
			boutput(user, "<span style=\"color:orange\">You must be on the ground!</span>")
			return
		else
			var/S = T
	...
	before_stack(atom/movable/O as obj, mob/user as mob)
		user.visible_message("<span style=\"color:orange\">[user] begins stacking [src]!</span>")

	after_stack(atom/movable/O as obj, mob/user as mob, var/added)
		boutput(user, "<span style=\"color:orange\">You finish stacking tiles.</span>")
