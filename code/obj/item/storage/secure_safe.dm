...
/obj/item/storage/secure/attackby(obj/item/W as obj, mob/user as mob)
	...
	if (hackable)
		if ((istype(W, /obj/item/screwdriver)) && (src.locked == 1))
			sleep(6)
			src.open =! src.open
			user.show_message("<span style=\"color:orange\">You [src.open ? "open" : "close"] the service panel.</span>")
			return
...
/*
	var/dat = "[css_interfaces]<TT><B>[src.name]</B><BR><br><br>Lock Status: [src.locked ? "LOCKED" : "UNLOCKED"]"
	var/message = "Code"
