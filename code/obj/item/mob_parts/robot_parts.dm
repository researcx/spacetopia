...
/obj/item/parts/robot_parts/head
	...
	examine()
		...
		..()
		if (src.brain)
			boutput(usr, "<span style=\"color:orange\">This head unit has [src.brain] inside. Use a wrench if you want to remove it.</span>")
		else if (src.ai_interface)
			boutput(usr, "<span style=\"color:orange\">This head unit has [src.ai_interface] inside. Use a wrench if you want to remove it.</span>")
		else
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if (istype(W,/obj/item/organ/brain))
			...
			B.set_loc(src)
			src.brain = B
			boutput(user, "<span style=\"color:orange\">You insert the brain.</span>")
			...
			I.set_loc(src)
			src.ai_interface = I
			boutput(user, "<span style=\"color:orange\">You insert [I].</span>")
			playsound(get_turf(src), "sound/weapons/Genhit.ogg", 40, 1)
			return
			...
			if (src.ai_interface)
				boutput(user, "<span style=\"color:orange\">You open the head's compartment and take out [src.ai_interface].</span>")
				user.put_in_hand_or_drop(src.ai_interface)
				src.ai_interface = null
			else if (src.brain)
				boutput(user, "<span style=\"color:orange\">You open the head's compartment and take out [src.brain].</span>")
			...
			if (M.amount >= 2)
				boutput(user, "<span style=\"color:orange\">You reinforce [src.name] with the metal.</span>")
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if (istype(W,/obj/item/sheet) && (src.type == /obj/item/parts/robot_parts/head/sturdy))
			...
			if (!M.reinforcement)
				...
				return
			if (M.amount >= 2)
				boutput(user, "<span style=\"color:orange\">You reinforce [src.name] with the reinforced metal.</span>")
...
/obj/item/parts/robot_parts/chest
	...
	examine()
		set src in oview()
		..()
		if (src.cell) boutput(usr, "<span style=\"color:orange\">This chest unit has a [src.cell] installed. Use a wrench if you want to remove it.</span>")
		else boutput(usr, "<span style=\"color:red\">This chest unit has no power cell.</span>")
		if (src.wires) boutput(usr, "<span style=\"color:orange\">This chest unit has had wiring installed.</span>")
		...
		if(istype(W, /obj/item/cell))
			...
			if (src.ropart_get_damage_percentage(2) > 0) ..()
				...
				if(src.wires)
					...
					coil.use(1)
					src.wires = 1
					boutput(user, "<span style=\"color:orange\">You insert some wire.</span>")
					playsound(get_turf(src), "sound/weapons/Genhit.ogg", 40, 1)
			...
			if(!src.cell)
				...
				return
			playsound(get_turf(src), "sound/items/Ratchet.ogg", 40, 1)
			boutput(user, "<span style=\"color:orange\">You remove the cell from it's slot in the chest unit.</span>")
			...
			boutput(user, "<span style=\"color:orange\">You cut out the wires and remove them from the chest unit.</span>")
...
*/
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if (istype(W, /obj/item/skull))
			...
			boutput(user, "<span style=\"color:orange\">You add [W] to [src]. That's neat.</span>")
			qdel(W)
			qdel(src)
			return
...
/obj/item/parts/robot_parts/robot_frame
	...
	emag_act(var/mob/user, var/obj/item/card/emag/E)
		if(!syndicate)
			syndicate = 1
			if (user)
				boutput(user, "<span style=\"color:orange\">You short out the behavior restrictors on the frame's motherboard.</span>")
			return 1
		else if(user)
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if (istype(W, /obj/item/parts/robot_parts/))
			...
			playsound(get_turf(src), "sound/weapons/Genhit.ogg", 40, 1)
			boutput(user, "<span style=\"color:orange\">You add [P] to the frame.</span>")
			user.drop_item()
			P.set_loc(src)
			src.updateicon()
...
/obj/item/roboupgrade/expand
	...
	upgrade_activate(var/mob/living/silicon/robot/user as mob)
		if (!user) return
		user.max_upgrades++
		boutput(user, "<span style=\"color:orange\">You can now hold up to [user.max_upgrades] upgrades!</span>")
...
/obj/item/roboupgrade/rechargepack
	...
	upgrade_activate(var/mob/living/silicon/robot/user as mob)
		...
		if (user.cell)
			...
			boutput(user, "<span style=\"color:orange\">Cell has been recharged to [user.cell.charge]!</span>")
		else
...
/obj/item/roboupgrade/repairpack
	...
	upgrade_activate(var/mob/living/silicon/robot/user as mob)
		...
		boutput(user, "<span style=\"color:orange\">All components repaired!</span>")
