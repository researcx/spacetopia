...
/obj/item/basketball/attackby(obj/item/W as obj, mob/user as mob)
	if(istype(W, /obj/item/plutonium_core))
		boutput(user, "<span style=\"color:orange\">You insert the [W.name] into the [src.name].</span>")
		user.u_equip(W)
		W.dropped(user)
...
/obj/item/bballbasket
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if (istype(W,/obj/item/wrench) && mounted)
			src.visible_message("<span style=\"color:orange\"><b>[user] removes [src].</b></span>")
			src.pixel_y = 0
			src.pixel_x = 0
			src.anchored = 0
			src.mounted = 0
			...
			src.visible_message("<span style=\"color:orange\"><b>[user]</b> jumps up and tries to dunk [W] into [src]!</span>")
			user.u_equip(W)
		...
		if (!mounted && get_dist(src, target) == 1)
			if (isturf(target) && target.density)
				...
				if (get_dir(src,target) in cardinal)
					src.visible_message("<span style=\"color:orange\"><b>[user] mounts [src] on [target].</b></span>")
					user.drop_item()
	...
	proc/shoot(var/obj/O as obj, var/mob/user as mob)
		...
		if (prob(src.probability)) // It might land!
			if (prob(30)) // It landed cleanly!
				src.visible_message("<span style=\"color:orange\">[O] lands cleanly in [src]!</span>")
				src.basket(O)
				...
				spawn(delay)
					if (O && O.loc == src.loc)
						if (prob(40)) // It goes in!
							src.visible_message("<span style=\"color:orange\">[O] slips into [src]!</span>")
							src.basket(O)
						else
