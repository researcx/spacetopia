...
/obj/item/sheet/glass
	...
	before_stack(atom/movable/O as obj, mob/user as mob)
		user.visible_message("<span style=\"color:orange\">[user] begins stacking glass sheets!</span>")

	after_stack(atom/movable/O as obj, mob/user as mob, var/added)
		boutput(user, "<span style=\"color:orange\">You finish stacking glass.</span>")
	...
	attackby(obj/item/W, mob/user)
		...
		if ( W.type == src.type )
			...
			if (G.amount + src.amount > src.max_stack)
				...
				G.amount = 5
				boutput(user, "<span style=\"color:orange\">You add the glass sheets to the stack. It now has [G.amount] sheets.</span>")
			else
				G.amount += src.amount
				boutput(user, "<span style=\"color:orange\">You add the glass sheet to the stack. It now has [G.amount] sheets.</span>")
				//SN src = null
				qdel(src)
				return
			return
