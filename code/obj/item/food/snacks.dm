	...
	attackby(obj/item/W as obj, mob/user as mob)
		if (istype(W, /obj/item/axe) || istype(W, /obj/item/circular_saw) || istype(W, /obj/item/kitchen/utensil/knife) || istype(W, /obj/item/scalpel) || istype(W, /obj/item/sword) || istype(W,/obj/item/saw) || istype(W,/obj/item/knife_butcher))
			if (src.sliced == 1)
				...
				return
			boutput(user, "<span style=\"color:orange\">You cut the pizza into slices.</span>")
...
/obj/item/reagent_containers/food/snacks/soup/oatmeal
	...
	fun
		...
		heal(var/mob/M)
			...
			boutput(M, "<span style=\"color:orange\">You found a marshmallow [dinosaur] in this bite!</span>")
			..()
...
/obj/item/reagent_containers/food/snacks/surstromming
	...
	heal(var/mob/M)
		if (M.bioHolder.HasEffect("accent_swedish"))
			boutput(M, "<span style=\"color:orange\">It tastes just like the old country!</span>")
				...
				if(6 to 10)
					...
					spawn(0)
						while(prob(75))
							...
							M.stunned+= 2
				if(11 to 15)
					boutput(M, "<span style=\"color:orange\">Huh. That wasn't so bad. <span style=\"color:red\">WAIT NEVERMIND THERE'S THE AFTERTASTE</span></span>")
					M.emote ("cry")
					M.weakened+= 4
				if(16 to 20)
	...
	attack_self(var/mob/user as mob)
		if (src.icon_state == "surs_closed")
			boutput(user, "<span style=\"color:orange\">You pop the lid off the [src].</span>")
			...
			for(var/mob/living/carbon/M in viewers(user, null))
				if (M == user)
					if (user.bioHolder.HasEffect("accent_swedish"))
						boutput(user, "<span style=\"color:orange\">Ahhh, that smells wonderful!</span>")
					else
					...
					if(M.bioHolder.HasEffect("accent_swedish"))
						boutput(M, "<span style=\"color:orange\">Hey, something smells good!</span>")
					else
...
/obj/item/reagent_containers/food/snacks/spaghetti/
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if(istype(W,/obj/item/reagent_containers/food/snacks/condiment/ketchup))
			boutput(user, "<span style=\"color:orange\">You create sperghetti with tomato sauce...</span>")
...
/obj/item/reagent_containers/food/snacks/spaghetti/sauce
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if(istype(W,/obj/item/reagent_containers/food/snacks/condiment/hotsauce))
			boutput(user, "<span style=\"color:orange\">You create sperghetti arrabbiata!</span>")
...
/obj/item/reagent_containers/food/snacks/hotdog
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if (istype(W, /obj/item/reagent_containers/food/snacks/breadslice))
			...
			boutput(user, "<span style=\"color:orange\">You create a corndog...</span>")
			...
			boutput(user, "<span style=\"color:orange\">You create a herbal sausage...</span>")
			src.herb = 1
		...
		if(quality >= 1)
			if(istype(M, /mob/living/carbon/human/))
				...
				if(prob(1))
					...
					boutput(M, "<span style=\"color:orange\">You feel smarter.</span>")
