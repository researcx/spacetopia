	...
	switch (src.det_state)
		if (0)
			if (istype(W, /obj/item/tank/plasma))
				...
				user.show_message("<span style=\"color:orange\">You insert the [W.name] into the slot.</span>")
				...
				user.u_equip(src)
				del(src)
				user.show_message("<span style=\"color:orange\">You sever the connection between the multitool and the igniter. The assembly falls apart.</span>")
			else
		...
		if (1)
			if (istype(W, /obj/item/cable_coil))
				...
				src.part_t = null
				user.show_message("<span style=\"color:orange\">You pry the plasma tank out of the assembly.</span>")
			else if (istype(W, /obj/item/screwdriver))
				src.setDetState(2)
				user.show_message("<span style=\"color:orange\">You secure the plasma tank to the assembly.</span>")
		...
		if (2)
			if (istype(W, /obj/item/cable_coil))
				...
				if (C.amount >= 6)
					...
					src.setDetState(3)
					src.add_fingerprint(user)
					user.show_message("<span style=\"color:orange\">You add the wiring to the assembly.</span>")
				else
				...
				src.setDetState(1)
				user.show_message("<span style=\"color:orange\">You unsecure the plasma tank from the assembly.</span>")
		...
		if (3)
			if (istype(W, /obj/item/device/timer))
				...
				src.add_fingerprint(user)
				user.show_message("<span style=\"color:orange\">You wire the timer failsafe to the assembly, disabling its external controls.</span>")
			else if (istype(W, /obj/item/wirecutters))
				src.setDetState(2)
				var/obj/item/cable_coil/C = new /obj/item/cable_coil(user, 6)
				C.loc = user.loc
				user.show_message("<span style=\"color:orange\">You cut the wiring on the assembly.</span>")
		if (4)
			if (istype(W, /obj/item/wirecutters))
				...
				src.attachments.Cut()
				user.show_message("<span style=\"color:orange\">You disconnect the timer from the assembly, and reenable its external controls.</span>")
			if (istype(W, /obj/item/screwdriver))
				...
				if (target == src.trigger)
					...
					src.trigger.master = null
					src.trigger = null
					user.show_message("<span style=\"color:orange\">You remove the triggering device from the assembly.</span>")
					...
					setDescription()
					user.show_message("<span style=\"color:orange\">You remove the [target] from the assembly.</span>")
				setDescription()
				...
				if (src.trigger)
					...
					user.u_equip(W)
					src.trigger = W
					user.show_message("<span style=\"color:orange\">You attach the [W.name] to the trigger slot.</span>")
					setDescription()
				...
				if (src.radio)
					...
					user.u_equip(W)
					src.radio = W
					user.show_message("<span style=\"color:orange\">You attach the [W.name] to the radio slot.</span>")
				...
				W.layer = null
				user.u_equip(W)
				user.show_message("<span style=\"color:orange\">You stick the note onto the detonator assembly.</span>")
				del(W)
				...
				if (src.attachments.len < 3)
					...
					user.show_message("<span style=\"color:orange\">You attach the [W.name] to an attachment slot.</span>")
					setDescription()
				else
...
/obj/item/assembly/detonator/proc/setDescription()
	...
	if (src.trigger)
		src.desc += "<br><span style=\"color:orange\">There is \an [src.trigger.name] as a detonation trigger.</span>"
	if (src.radio) //WIRE TODO: roll this into the new attachment system
		src.desc += "<br><span style=\"color:orange\">There is a radio attached to the timing wire, it will announce the bomb status when primed.</span>"
	for (var/obj/item/a in src.attachments)
		src.desc += "<br><span style=\"color:orange\">There is \an [a] wired onto the assembly as an attachment.</span>"
