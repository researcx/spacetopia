...
/obj/item/assembly/proximity_bomb/attackby(obj/item/W as obj, mob/user as mob)
	...
	if (!( src.status ))
		src.status = 1
		user.show_message("<span style=\"color:orange\">A pressure hole has been bored to the plasma tank valve. The plasma tank can now be ignited.</span>", 1)
	else
		src.status = 0
		boutput(user, "<span style=\"color:orange\">The hole has been closed.</span>")
