...
/obj/item/rcd/construction/attack_self(mob/user as mob)
	if (mode == 1 || mode == 2)
		...
		boutput(user, "<span style=\"color:orange\">Place a door control on a wall, then place any amount of pod doors on floors.</span>")
		boutput(user, "<span style=\"color:orange\">You can also select an existing door control by whacking it with the RCD.</span>")
...
/obj/item/rcd/construction/afterattack(atom/A, mob/user as mob)
	...
	if (mode == 3)
		...
		if (istype(A, /obj/machinery/r_door_control))
			var/obj/machinery/r_door_control/R = A
			if (findtext(R.id, "rcd_built") != 0)
				boutput(user, "<span style=\"color:orange\">Selected.</span>")
				hangar_id = R.id
				mode = 5
			else
