/obj/item/noisemaker
	...
	attack(mob/M as mob, mob/user as mob, def_zone)
		...
		switch(src.mode)
			if ("honk")
				if (farting_allowed)
					boutput(user, "<span style=\"color:orange\">Mode is now: Farter</span>")
					src.mode = "fart"
				else
					boutput(user, "<span style=\"color:orange\">Mode is now: Burper</span>")
					src.mode = "burp"
			if ("fart")
				boutput(user, "<span style=\"color:orange\">Mode is now: Burper</span>")
				src.mode = "burp"
			if ("burp")
				boutput(user, "<span style=\"color:orange\">Mode is now: Squeaker</span>")
				src.mode = "squeak"
			if ("squeak")
				boutput(user, "<span style=\"color:orange\">Mode is now: Cat</span>")
				src.mode = "cat"
			if ("cat")
				boutput(user, "<span style=\"color:orange\">Mode is now: Harmonica</span>")
				src.mode = "harmo"
			if ("harmo")
				boutput(user, "<span style=\"color:orange\">Mode is now: Vuvuzela</span>")
				src.mode = "vuvuz"
			if ("vuvuz")
				boutput(user, "<span style=\"color:orange\">Mode is now: Banger</span>")
				src.mode = "bang"
			if ("bang")
				boutput(user, "<span style=\"color:orange\">Mode is now: Buzzer</span>")
				src.mode = "buzz"
			if ("buzz")
				boutput(user, "<span style=\"color:orange\">Mode is now: Shooter</span>")
				src.mode = "gunshot"
			if ("gunshot")
				boutput(user, "<span style=\"color:orange\">Mode is now: Siren</span>")
				src.mode = "siren"
			if ("siren")
				boutput(user, "<span style=\"color:orange\">Mode is now: Coo</span>")
				src.mode = "coo"
			if ("coo")
				boutput(user, "<span style=\"color:orange\">Mode is now: Rimshot</span>")
				src.mode = "rimshot"
			if("rimshot")
				boutput(user, "<span style=\"color:orange\">Mode is now: Trombone</span>")
				src.mode = "trombone"
			else
				boutput(user, "<span style=\"color:orange\">Mode is now: Honk</span>")
				src.mode = "honk"
			*/

		if (newmode && rand(1,150) == 1)
			boutput(user, "<span style=\"color:red\">BZZZ SOUND SYNTHESISER ERROR</span>")
			boutput(user, "<span style=\"color:orange\">Mode is now: ???</span>")
			src.mode = pick("un1","un2","un3")
		else if (newmode)
			boutput(user, "<span style=\"color:orange\">Mode is now: [newmode]</span>")
			src.mode = newmode
