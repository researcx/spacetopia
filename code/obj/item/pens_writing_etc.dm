...
/obj/item/hand_labeler
	...
	attack(mob/M, mob/user as mob)
		...
		if (!src.label || !length(src.label))
			if (islist(M.name_suffixes) && M.name_suffixes.len)
				M.remove_suffixes(1)
				M.UpdateName()
				user.visible_message("<span style=\"color:orange\"><b>[user]</b> removes the label from [M].</span>",\
				"<span style=\"color:orange\">You remove the label from [M].</span>")
				return
			else
				return
	...
	afterattack(atom/A, mob/user as mob)
		...
		if (!src.label || !length(src.label))
			if (islist(A.name_suffixes) && A.name_suffixes.len)
				A.remove_suffixes(1)
				A.UpdateName()
				user.visible_message("<span style=\"color:orange\"><b>[user]</b> removes the label from [A].</span>",\
				"<span style=\"color:orange\">You remove the label from [A].</span>")
				return
			else
				return
	...
	attack_self()
		...
		if (!str || !length(str))
			boutput(usr, "<span style=\"color:orange\">Label text cleared.</span>")
			src.label = null
			return
		if (length(str) > 30)
			...
			return
		src.label = "([str])"
		boutput(usr, "<span style=\"color:orange\">You set the text to '[str]'.</span>")

	proc/Label(var/atom/A, var/mob/user, var/no_message = 0)
		if (user && !no_message)
			user.visible_message("<span style=\"color:orange\"><b>[user]</b> puts a label on [A].</span>",\
			"<span style=\"color:orange\">You put a label on [A].</span>")
	...
	attackby(obj/item/P as obj, mob/user as mob)
		...
		if (istype(P, /obj/item/paper) || istype(P, /obj/item/photo))
			if (src.contents.len < 15)
				...
				boutput(user, "<span style=\"color:orange\">Not enough space!!!</span>")
		else
