...
/*
	examine()
		...
		boutput(usr, "<span style=\"color:orange\">[out]</span>")
		return null
*/
	...
	proc/scan(obj/target, mob/user)
		...
		if (!isnull(initial(target.icon)) && !isnull(initial(target.icon_state)) && target.icon && target.icon_state && (istype(target, /obj/item) || istype(target, /obj/shrub) || istype(target, /obj/critter) || istype(target, /obj/machinery/bot))) // cogwerks - added more fun
			...
			boutput(user, "<span style=\"color:orange\">Scanned [target].</span>")
	...
	proc/toggle()
		...
		if (active) //active_dummy)
			...
			cham.loc = src
			boutput(usr, "<span style=\"color:orange\">You deactivate the [src].</span>")
			...
			boutput(usr, "<span style=\"color:orange\">You activate the [src].</span>")
			anim.loc = get_turf(src)
			flick("emppulse",anim)
			spawn (8)
				anim.loc = src //Back in the box with ye
...
/obj/item/device/chameleon/bomb
	...
	toggle()
		...
		if (active)
			active = 0
			playsound(src, "sound/effects/pop.ogg", 100, 1, 1)
			boutput(usr, "<span style=\"color:orange\">You disarm the [src].</span>")
			...
			playsound(src, "sound/effects/pop.ogg", 100, 1, 1)
			src.active = 1
			boutput(usr, "<span style=\"color:orange\">You arm the [src].</span>")
