	...
	clown
		...
		HasEntered(AM as mob|obj) //Clown PDA is slippery.
			...
			if (iscarbon(AM))
				...
				M.pulling = null
				boutput(M, "<span style=\"color:orange\">You slipped on the PDA!</span>")
...
/obj/item/device/pda2/attackby(obj/item/C as obj, mob/user as mob)
	if (istype(C, /obj/item/disk/data/cartridge) && isnull(src.cartridge))
		user.drop_item()
		C.set_loc(src)
		boutput(user, "<span style=\"color:orange\">You insert [C] into [src].</span>")
		src.cartridge = C
		src.updateSelfDialog()
		...
		if (!src.owner)
			...
			boutput(user, "<span style=\"color:orange\">Card scanned.</span>")
			src.updateSelfDialog()
		else
			if (src.ID_card)
				boutput(user, "<span style=\"color:orange\">You swap [ID] and [src.ID_card].</span>")
				src.eject_id_card(user)
				src.insert_id_card(ID, user)
				return
			else if (!src.ID_card)
				src.insert_id_card(ID, user)
				boutput(user, "<span style=\"color:orange\">You insert [ID] into [src].</span>")
