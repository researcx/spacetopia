...
/obj/item/assembly/rad_infra/attackby(obj/item/W as obj, mob/user as mob)
	...
	if (src.status)
		user.show_message("<span style=\"color:orange\">The infrared laser is now secured!</span>", 1)
	else
		user.show_message("<span style=\"color:orange\">The infrared laser is now unsecured!</span>", 1)
