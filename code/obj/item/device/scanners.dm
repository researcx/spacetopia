...
/obj/item/device/healthanalyzer
	...
	attack_self(mob/user as mob)
		if (!src.reagent_upgrade)
			...
			boutput(user, "<span style=\"color:orange\">Reagent scanner [src.reagent_scan ? "enabled" : "disabled"].</span>")
			return
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if (istype(W, /obj/item/device/healthanalyzer_upgrade))
			if (src.reagent_upgrade)
				...
				boutput(user, "<span style=\"color:orange\">Reagent scan upgrade installed.</span>")
	...
	attack(mob/M as mob, mob/user as mob)
		if ((user.bioHolder.HasEffect("clumsy") || user.get_brain_damage() >= 60) && prob(50))
			...
			user.show_message("Analyzing Results for <span style=\"color:orange\">The floor:<br>&emsp; Overall Status: Healthy</span>", 1)
			...
			user.show_message("<span style=\"color:orange\">Body Temperature: ???</span>", 1)
			return
...
/obj/item/device/reagentscanner
	...
	afterattack(atom/A as mob|obj|turf|area, mob/user as mob)
		user.visible_message("<span style=\"color:orange\"><b>[user]</b> scans [A] with [src]!</span>",\
		"<span style=\"color:orange\">You scan [A] with [src]!</span>")
	...
	attack_self(mob/user as mob)
		if (isnull(src.scan_results))
			boutput(user, "<span style=\"color:orange\">No previous scan results located.</span>")
			return
		boutput(user, "<span style=\"color:orange\">Previous scan's results:<br>[src.scan_results]</span>")

	get_desc(dist)
		if (dist < 3)
			if (!isnull(src.scan_results))
				. += "<br><span style=\"color:orange\">Previous scan's results:<br>[src.scan_results]</span>"
...
/obj/item/device/analyzer
	...
	attack_self(mob/user as mob)
		...
		user.visible_message("<span style=\"color:orange\"><b>[user]</b> takes an atmospheric reading of [location].</span>")
	...
	afterattack(atom/A as mob|obj|turf|area, mob/user as mob)
		...
		if (istype(A, /obj) || isturf(A))
			user.visible_message("<span style=\"color:orange\"><b>[user]</b> takes an atmospheric reading of [A].</span>")
...
/obj/item/device/prisoner_scanner/attack(mob/living/carbon/human/M as mob, mob/user as mob)
	...
	//	return
	boutput(user, "<span style=\"color:orange\">You scan in [M]</span>")
...
/obj/item/device/prisoner_scanner/attack_self(mob/user as mob)
	...
	if (src.mode == 1)
		src.mode = 2
		boutput(user, "<span style=\"color:orange\">you switch the record mode to Parolled</span>")
	else if (src.mode == 2)
		src.mode = 3
		boutput(user, "<span style=\"color:orange\">you switch the record mode to Released</span>")
	else
		src.mode = 1
		boutput(user, "<span style=\"color:orange\">you switch the record mode to Incarcerated</span>")
