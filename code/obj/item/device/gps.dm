	...
	Topic(href, href_list)
		...
		if ((usr.contents.Find(src) || usr.contents.Find(src.master) || in_range(src, usr) && istype(src.loc, /turf)))
			...
			if(href_list["getcords"])
				boutput(usr, "<span style=\"color:orange\">Located at: <b>X</b>: [usr.x], <b>Y</b>: [usr.y]</span>")
			if(href_list["track1"])
				boutput(usr, "<span style=\"color:orange\">Tracking enabled.</span>")
				src.allowtrack = 1
			if(href_list["track2"])
				boutput(usr, "<span style=\"color:orange\">Tracking disabled.</span>")
				src.allowtrack = 0
...
/obj/machinery/beacon
	...
	attack_hand()
		enabled = !enabled
		boutput(usr, "<span style=\"color:orange\">You switch the beacon [src.enabled ? "on" : "off"].</span>")
