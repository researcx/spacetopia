/obj/item/shieldwallgen
	...
	verb/increase_range()
		...
		if (!isturf(loc))
			...
			return
		range = min(range+1,3)
		boutput(usr, "<span style=\"color:orange\">Range set to : [range]</span>")
		if(active)
			turn_off()
			turn_on()
	...
	verb/toggle()
		...
		if (!active)
			turn_on()
			boutput(usr, "<span style=\"color:orange\">You activate the generator.</span>")
		else
			turn_off()
			boutput(usr, "<span style=\"color:orange\">You deactivate the generator.</span>")
