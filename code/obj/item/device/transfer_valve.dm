	...
	attackby(obj/item/item, mob/user)
		if(istype(item, /obj/item/tank) || istype(item, /obj/item/clothing/head/butt))
			...
			if(!tank_one)
				...
				user.drop_item()
				item.set_loc(src)
				boutput(user, "<span style=\"color:orange\">You attach \the [item] to the transfer valve</span>")
			else if(!tank_two)
				tank_two = item
				user.drop_item()
				item.set_loc(src)
				boutput(user, "<span style=\"color:orange\">You attach the \the [item] to the transfer valve!</span>")
			...
			user.drop_item()
			item.set_loc(src)
			boutput(user, "<span style=\"color:orange\">You attach the [item] to the valve controls!</span>")
			item.master = src
