...
/obj/item/device/igniter/attackby(obj/item/W as obj, mob/user as mob)
	if ((istype(W, /obj/item/device/radio/signaler) && !( src.status )))
		...
		R.setDetState(0)
		src.add_fingerprint(user)
		user.show_message("<span style=\"color:orange\">You hook up the igniter to the multitool's panel.</span>")

	if (!( istype(W, /obj/item/screwdriver) ))
		return
	src.status = !( src.status )
	if (src.status)
		user.show_message("<span style=\"color:orange\">The igniter is ready!</span>")
	else
		user.show_message("<span style=\"color:orange\">The igniter can now be attached!</span>")
	src.add_fingerprint(user)
	return
...
/obj/item/device/igniter/afterattack(atom/target, mob/user as mob)
	if (!ismob(target) && target.reagents)
		boutput(usr, "<span style=\"color:orange\">You heat \the [target.name]</span>")
