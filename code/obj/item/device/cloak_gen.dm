...
/obj/item/cloak_gen
	...
	proc/turn_on()
		...
		if (usr && ismob(usr))
			boutput(usr, "<span style=\"color:orange\">You activate the cloak field generator.</span>")
	...
	proc/turn_off()
		...
		anchored = 0
		if (usr && ismob(usr))
			boutput(usr, "<span style=\"color:orange\">You deactivate the cloak field generator.</span>")
		for(var/A in fields)
			qdel(A)
...
/obj/item/remote/cloak_gen
	...
	verb/set_pattern()
		...
		if(my_gen.active)
			my_gen.turn_off()
			my_gen.turn_on()
		boutput(usr, "<span style=\"color:orange\">You set the pattern to '[input]'.</span>")
	...
	verb/set_range()
		...
		if(my_gen.active)
			my_gen.turn_off()
			my_gen.turn_on()
		boutput(usr, "<span style=\"color:orange\">You set the range to [my_gen.range].</span>")
	...
	verb/turn_on()
		...
		my_gen.turn_on()
		boutput(usr, "<span style=\"color:orange\">You turn the cloaking field generator on.</span>")

	verb/turn_off()
		set src in view(1)
		if (!istype(usr,/mob/living) || !my_gen || !my_gen.active) return
		my_gen.turn_off()
		boutput(usr, "<span style=\"color:orange\">You turn the cloaking field generator off.</span>")