...
/obj/item/device/glowstick // fuck yeah space rave
	...
	attack_self(mob/user as mob)
		if (!on)
			boutput(user, "<span style=\"color:orange\">You crack [src].</span>")
			on = 1
			...
			if (prob(10))
				user.visible_message("<span style=\"color:orange\"><b>[user]</b> breaks [src]! What [pick("a clutz", "a putz", "a chump", "a doofus", "an oaf", "a jerk")]!</span>")
				playsound(user.loc, "sound/effects/snap.ogg", 50, 1)
				...
				user.visible_message("<span style=\"color:orange\"><b>[user]</b> [pick("fiddles", "faffs around", "goofs around", "fusses", "messes")] with [src].</span>")
