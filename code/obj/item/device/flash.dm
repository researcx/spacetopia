...
/obj/item/device/flash/attackby(obj/item/W as obj, mob/user as mob)
	if (istype(W, /obj/item/cell) && !src.secure)
		boutput(user, "<span style=\"color:orange\">You combine [W] and [src]...</span>")
		...
		boutput(user, "<span style=\"color:orange\">You [src.secure ? "unscrew" : "secure"] the access panel.</span>")
		secure = !secure
		...
		if (src.status == 0)
			...
			if (src.use <= 0)
				boutput(user, "<span style=\"color:orange\">The bulb is in perfect condition.</span>")
			else if (src.use>0 && src.use<5)
				boutput(user, "<span style=\"color:orange\">The bulb is in good condition.</span>")
			else if (src.use>5 && src.use<10)
				boutput(user, "<span style=\"color:orange\">The bulb is in decent condition.</span>")
			else if (src.use>10 && src.use<15)
				boutput(user, "<span style=\"color:orange\">The bulb is in bad condition.</span>")
			else
				boutput(user, "<span style=\"color:orange\">The bulb is in terrible condition.</span>")
	else
		return ..()
