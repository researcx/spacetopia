	...
	attackby(obj/item/W as obj, mob/user as mob)
		if (istype(W, /obj/item/screwdriver))
			if (src.det_time == src.org_det_time)
				...
				user.show_message("<span style=\"color:orange\">You set [src] for a [det_time/10] second detonation time.</span>")
...
/obj/item/pipebomb/frame
	...
	attackby(obj/item/W, mob/user)
		if(istype(W, /obj/item/weldingtool) && state == 1)
			if(W:welding == 1)
				...
				if (W:get_fuel() < 1)
					...
					return
				W:use_fuel(1)
				boutput(user, "<span style=\"color:orange\">You hollow out the pipe.</span>")
				src.state = 2
		...
		if(istype(W, /obj/item/reagent_containers/) && state == 2)
			boutput(user, "<span style=\"color:orange\">You fill the pipe with 20 units of the reagents.</span>")
			src.state = 3
		...
		if(istype(W, /obj/item/cable_coil) && state == 3)
			boutput(user, "<span style=\"color:orange\">You link the cable, fuel and pipes.</span>")
			src.state = 4
		...
		if(istype(W, /obj/item/assembly/time_ignite) && state == 4)
			boutput(user, "<span style=\"color:orange\">You connect the cable to the timer/igniter assembly.</span>")
			var/turf/T = get_turf(src)
