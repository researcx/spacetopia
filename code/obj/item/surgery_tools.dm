			...
			if (ishuman(M))
				...
				if (surgery_status && H.organHolder)
					...
					if (do_mob(user, H, 20 * surgery_status))
						H.visible_message("<span style=\"color:orange\"><b>[user]</b> sutures the incisions on [H == user ? "[H.gender == "male" ? "his" : "her"]" : "[H]'s"] [zone_sel2name[zone]] closed.</span>", \
						"<span style=\"color:orange\">[H == user ? "You suture" : "<b>[user]</b> sutures"] the incisions on your [zone_sel2name[zone]] closed.</span>")
						if (zone == "chest")
					...
					if (do_mob(user, H, 20 * H.bleeding))
						H.visible_message("<span style=\"color:orange\"><b>[user]</b> sutures [H == user ? "[H.gender == "male" ? "his" : "her"]" : "[H]'s"] wounds closed.</span>", \
						"<span style=\"color:orange\">[H == user ? "You suture" : "<b>[user]</b> sutures"] your wounds closed.</span>")
	...
	attack(mob/living/carbon/M as mob, mob/living/carbon/user as mob)
		...
		if (ishuman(M))
			...
			if (H.bleeding)
				...
				src.in_use = 1
				if (do_mob(user, H, 15))
					H.visible_message("<span style=\"color:orange\"><b>[user]</b> dresses [H == user ? "[H.gender == "male" ? "his" : "her"]" : "[H]'s"] wounds.</span>", \
					"<span style=\"color:orange\">[H == user ? "You dress" : "<b>[user]</b> dresses"] your wounds.</span>")
	...
	get_desc(dist)
		...
		if (src.volume >= 0)
			switch (src.volume)
				...
				if (70 to 99)
					. += "<span style=\"color:orange\">It's nearly full.</span>"
				if (100 to INFINITY)
					. += "<span style=\"color:orange\">It's full.</span>"
	...
	attack(mob/living/carbon/M as mob, mob/living/carbon/user as mob)
		...
		if (ishuman(M))
			...
			if (H.blood_volume < 500)
				H.tri_message("<span style=\"color:orange\"><b>[user]</b> attaches [src]'s needle to [H == user ? </span>"[H.gender == "male" ? "his" : "her"]" : "[H]'s"] arm and begins transferring blood.",\
				user, "<span style=\"color:orange\">You attach [src]'s needle to [H == user ? </span>"your" : "[H]'s"] arm and begin transferring blood.",\
				H, "<span style=\"color:orange\">[H == user ? </span>"You attach" : "<b>[user]</b> attaches"] [src]'s needle to your arm and begin transferring blood.")
				src.in_use = 1
				for (var/i)
					if (H.blood_volume >= 500)
						H.visible_message("<span style=\"color:orange\"><b>[H]</b>'s blood transfusion finishes.</span>", \
						"<span style=\"color:orange\">Your blood transfusion finishes.</span>")
						src.in_use = 0
						break
					if (src.volume <= 0)
					...
					if (get_dist(src, H) > 1)
						...
						if (prob(5))
							...
							H.visible_message("<span style=\"color:orange\"><b>[H]</b> looks [fluff].</span>", \
							"<span style=\"color:orange\">You feel [fluff].</span>")
						sleep(5)
			else
			...
			if (W.reagents && W.reagents.has_reagent("blood"))
				...
				if (blood_volume < W.reagents.total_volume)
					...
					if (src.volume > 100 - W:amount_per_transfer_from_this)
						...
						return
					user.visible_message("<span style=\"color:orange\"><b>[user]</b> transfers blood to [src].</span>", \
					"<span style=\"color:orange\">You transfer blood from [W] to [src].</span>")
...
/obj/item/hemostat
	...
	attack(mob/M as mob, mob/user as mob)
		...
		H.tri_message("<span style=\"color:orange\"><b>[user]</b> clamps the bleeders in [H == user ? "[his_or_her(H)]" : "[H]'s"] incision with [src].</span>",\
		user, "<span style=\"color:orange\">You clamp the bleeders in [user == H ? "your" : "[H]'s"] incision with [src].</span>",\
		H, "<span style=\"color:orange\">[H == user ? "You clamp" : "<b>[user]</b> clamps"] the bleeders in your incision with [src].</span>")
