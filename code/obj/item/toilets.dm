...
/obj/item/storage/toilet/attackby(obj/item/W as obj, mob/user as mob)
	...
	if (istype(W, /obj/item/grab))
		...
		user.visible_message("<span style=\"color:orange\">[user] gives [W:affecting] a swirlie!</span>", "<span style=\"color:orange\">You give [W:affecting] a swirlie. It's like Middle School all over again!</span>")
		return
...
/obj/item/storage/toilet/MouseDrop_T(mob/M as mob, mob/user as mob)
	...
	if ((!( istype(M, /mob) ) || get_dist(src, user) > 1 || M.loc != src.loc || user.restrained() || usr.stat))
		return
	if (M == usr)
		user.visible_message("<span style=\"color:orange\">[user] sits on the toilet.</span>", "<span style=\"color:orange\">You sit on the toilet.</span>")
	else
		user.visible_message("<span style=\"color:orange\">[M] is seated on the toilet by [user]!</span>")
	M.anchored = 1
	M.buckled = src
	M.set_loc(src.loc)
...
/obj/item/storage/toilet/attack_hand(mob/user as mob)
	for(var/mob/M in src.loc)
		if (M.buckled)
			if (M != user)
				user.visible_message("<span style=\"color:orange\">[M] is zipped up by [user]. That's... that's honestly pretty creepy.</span>")
			else
				user.visible_message("<span style=\"color:orange\">[M] zips up.</span>", "<span style=\"color:orange\">You zip up.</span>")
//			boutput(world, "[M] is no longer buckled to [src]")
			M.anchored = 0
			M.buckled = null
			src.add_fingerprint(user)
	if((src.clogged < 1) || (src.contents.len < 7) || (user.loc != src.loc))
		user.visible_message("<span style=\"color:orange\">[user] flushes the toilet.</span>", "<span style=\"color:orange\">You flush the toilet.</span>")
		src.clogged = 0
		src.contents.len = 0
	else if((src.clogged >= 1) || (src.contents.len >= 7) || (user.buckled != src.loc))
		src.visible_message("<span style=\"color:orange\">The toilet is clogged!</span>")
