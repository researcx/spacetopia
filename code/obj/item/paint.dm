...
/obj/machinery/vending/paint
	...
	emag_act(var/mob/user, var/obj/item/card/emag/E)
		if(user)
			boutput(user, "<span style=\"color:orange\">You swipe the card along a crack in the machine.</span>")

		if (prob(5))
			var/obj/item/paint_can/rainbow/plaid/P = new/obj/item/paint_can/rainbow/plaid(src.loc)
			if (user)
				boutput(user, "<span style=\"color:orange\">You hear a faint droning sound. Like a tiny set of bagpipes.</span>")
			P.uses = (15 * 7)
			P.generate_icon()
		else
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if(istype(W,/obj/item/paint_can))
			boutput(user, "<span style=\"color:orange\">You refill the paint can.</span>")
			W:uses = 15
			W:generate_icon()
...
/obj/item/paint_can
	...
	afterattack(atom/target as mob|obj|turf, mob/user as mob)
		...
		for(var/mob/O in oviewers(world.view, user))
			O.show_message("<span style=\"color:orange\">[user] paints \the [target].</span>", 1)
