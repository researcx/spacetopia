/obj/item/satchel
	...
	attackby(obj/item/W as obj, mob/user as mob)
		...
		if (src.contents.len < src.maxitems)
			...
			W.set_loc(src)
			W.dropped()
			boutput(user, "<span style=\"color:orange\">You put [W] in [src].</span>")
			var/itemamt = src.contents.len
			src.desc = "A leather bag. It holds [itemamt]/[src.maxitems] [src.itemstring]."
			if (itemamt == src.maxitems) boutput(user, "<span style=\"color:orange\">[src] is now full!</span>")
	...
	attack_self(var/mob/user as mob)
		if (src.contents.len)
			...
			for (var/obj/item/I in src.contents)
				I.set_loc(T)
			boutput(user, "<span style=\"color:orange\">You empty out [src].</span>")
	...
	MouseDrop_T(atom/movable/O as obj, mob/user as mob)
		...
		if (src.contents.len < src.maxitems)
			user.visible_message("<span style=\"color:orange\">[user] begins quickly filling [src]!</span>")
			...
			for(var/obj/item/I in view(1,user))
				...
				if (src.contents.len >= src.maxitems)
					boutput(user, "<span style=\"color:orange\">[src] is now full!</span>")
					break
			boutput(user, "<span style=\"color:orange\">You finish filling [src]!</span>")
		else boutput(user, "<span style=\"color:red\">[src] is full!</span>")
