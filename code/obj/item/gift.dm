...
/obj/item/wrapping_paper/attackby(obj/item/W as obj, mob/user as mob)
	if (!( locate(/obj/table, src.loc) ))
		boutput(user, "<span style=\"color:orange\">You MUST put the paper on a table!</span>")
		return
	if (W.w_class < 4)
		if (user.find_type_in_hand(/obj/item/wirecutters) || user.find_type_in_hand(/obj/item/scissors))
			var/a_used = 2 ** (src.w_class - 1)
			if (src.amount < a_used)
				boutput(user, "<span style=\"color:orange\">You need more paper!</span>")
				return
			else
				src.amount -= a_used
				user.drop_item()
			...
			boutput(user, "<span style=\"color:orange\">You need something to cut [src] with!</span>")
	else
		boutput(user, "<span style=\"color:orange\">The object is FAR too large!</span>")
	return
...
/obj/item/gift/attack_self(mob/user as mob)
	if(!src.gift)
		boutput(user, "<span style=\"color:orange\">The gift was empty!</span>")
		qdel(src)
		return
	user.u_equip(src)
...
/obj/item/a_gift/attack_self(mob/M as mob)
	if (!giftpaths.len)
		boutput(M, "<span style=\"color:orange\">The gift was empty!</span>")
		qdel(src)
		return
...
/obj/spresent/relaymove(mob/user as mob)
	if (user.stat)
		return
	boutput(user, "<span style=\"color:orange\">You can't move.</span>")

/obj/spresent/attackby(obj/item/W as obj, mob/user as mob)

	if (!istype(W, /obj/item/wirecutters))
		boutput(user, "<span style=\"color:orange\">I need wirecutters for that.</span>")
		return

	boutput(user, "<span style=\"color:orange\">You cut open the present.</span>")
