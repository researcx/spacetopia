...
/obj/item/rods
	...
	before_stack(atom/movable/O as obj, mob/user as mob)
		user.visible_message("<span style=\"color:orange\">[user] begins gathering up metal rods!</span>")

	after_stack(atom/movable/O as obj, mob/user as mob, var/added)
		boutput(user, "<span style=\"color:orange\">You finish gathering rods.</span>")
	...
	attack_hand(mob/user as mob)
		if((user.r_hand == src || user.l_hand == src) && src.amount > 1)
			...
			if (splitnum >= amount || splitnum < 1)
				...
				return
			boutput(usr, "<span style=\"color:orange\">You take [splitnum] rods from the stack, leaving [diff] rods behind.</span>")
			src.amount = diff
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if (istype(W, /obj/item/weldingtool))
			...
			if (WELD.welding)
				...
				if (src.amount > 3)
					...
					boutput(user, "<span style=\"color:orange\">You could make up to [makemetal] sheets by welding this stack.</span>")
		...
		if (istype(W, /obj/item/rods))
			...
			if (R.amount + src.amount > src.max_stack)
				...
				boutput(user, "<span style=\"color:orange\">You add the rods to the stack. It now has [R.amount] rods.</span>")
			else
				R.amount += src.amount
				boutput(user, "<span style=\"color:orange\">You add [R.amount] rods to the stack. It now has [R.amount] rods.</span>")
				//SN src = null
				qdel(src)
				return
		return
		...
		if (locate(/obj/grille, usr.loc))
			for(var/obj/grille/G in usr.loc)
				if (G.destroyed)
					...
					boutput(user, "<span style=\"color:orange\">You repair the broken grille.</span>")
					src.amount--
				else
			...
			if (src.amount < 2)
				...
				return
			user.visible_message("<span style=\"color:orange\"><b>[user]</b> begins building a grille.</span>")
			var/turf/T = usr.loc
			spawn(15)
...
/obj/item/sheet/metal
	...
	before_stack(atom/movable/O as obj, mob/user as mob)
		user.visible_message("<span style=\"color:orange\">[user] begins stacking metal sheets!</span>")

	after_stack(atom/movable/O as obj, mob/user as mob, var/added)
		boutput(user, "<span style=\"color:orange\">You finish stacking metal.</span>")
	...
	attack_hand(mob/user as mob)
		if((user.r_hand == src || user.l_hand == src) && src.amount > 1)
			...
			if (splitnum >= amount || splitnum < 1)
				boutput(user, "<span style=\"color:red\">Invalid entry, try again.</span>")
				return
			boutput(usr, "<span style=\"color:orange\">You take [splitnum] sheets from the stack, leaving [diff] sheets behind.</span>")
			src.amount = diff
	...
	attackby(obj/item/sheet/metal/W as obj, mob/user as mob)
		...
		if (W.amount + src.amount > src.max_stack)
			...
			boutput(user, "<span style=\"color:orange\">You add the metal to the stack. It now has [W.amount] sheets.</span>")
		else
			W.amount += src.amount
			boutput(user, "<span style=\"color:orange\">You add the metal to the stack. It now has [W.amount] sheets.</span>")
			//SN src = null
	...
	attack_self(mob/user as mob)
		var/t1 = text("<HTML><HEAD>[css_interfaces]</head><TT>Amount Left: [] <BR>", src.amount)
		var/counter = 1
		var/list/L = list(  )
		...
		if (href_list["make"])
			...
			switch(href_list["make"])
				if("rods")
					...
					boutput(usr, "<span style=\"color:orange\">You could make up to [makerods] rods with the amount of metal you have.</span>")
				...
				if("fl_tiles")
					...
					boutput(usr, "<span style=\"color:orange\">You could make up to [maketiles] tiles with the amount of metal you have.</span>")
				...
				if("construct")
					if (src.amount < 2)
						...
						return
					boutput(usr, "<span style=\"color:orange\">Building wall girders ...</span>")
