			...
			if (usr.job == "Roboticist" || usr.job == "Medical Doctor" || usr.job == "Geneticist" || usr.job == "Medical Director")
				if (src.owner)
					if (src.owner.current)
						. += "<br><span style=\"color:orange\">This brain is still warm.</span>"
					else
...
/obj/item/skull
	...
	attackby(obj/item/W as obj, mob/user as mob)
		...
		if (istype(W, /obj/item/circular_saw))
			user.visible_message("<span style=\"color:orange\">[user] hollows out [src].</span>")
...
/obj/item/organ/head // vOv
	...
	get_desc()
		if (src.ears)
			. += "<br><span style=\"color:orange\">[src.name] has a [bicon(src.ears)] [src.ears.name] by its mouth.</span>"
		...
		if (src.head)
			if (src.head.blood_DNA)
				...
				. += "<br><span style=\"color:orange\">[src.name] has a [bicon(src.head)] [src.head.name] on it.</span>"
		...
		if (src.wear_mask)
			if (src.wear_mask.blood_DNA)
				...
				. += "<br><span style=\"color:orange\">[src.name] has a [bicon(src.wear_mask)] [src.wear_mask.name] on its face.</span>"
		...
		if (src.glasses)
			if (((src.wear_mask && src.wear_mask.see_face) || !src.wear_mask) && ((src.head && src.head.see_face) || !src.head))
				if (src.glasses.blood_DNA)
					...
					. += "<br><span style=\"color:orange\">[src.name] has a [bicon(src.glasses)] [src.glasses.name] on its face.</span>"
