			...
			if (1)
				if (ishuman(src.affecting))
					...
					if (H.bioHolder.HasEffect("fat"))
						boutput(src.assailant, "<span style=\"color:orange\">You can't strangle [src.affecting] through all that fat!</span>")
						return
					for (var/obj/item/clothing/C in list(H.head, H.wear_suit, H.wear_mask, H.w_uniform))
						if (C.body_parts_covered & HEAD)
							boutput(src.assailant, "<span style=\"color:orange\">You have to take off [src.affecting]'s [C.name] first!</span>")
							return
				icon_state = "!reinforce"
				src.state = 2
				if (!src.affecting.buckled)
