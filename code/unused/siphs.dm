	...
	if (istype(W, /obj/item/weapon/tank))
		...
		if (istype(W, /obj/item/weapon/screwdriver))
			...
			if (src.c_status)
				src.anchored = 0
				src.c_status = 0
				user.show_message("<span style=\"color:orange\">You have disconnected the siphon.</span>")
				if(con)
					con.connected = null
			else
				if (con && !con.connected)
					src.anchored = 1
					src.c_status = 3
					user.show_message("<span style=\"color:orange\">You have connected the siphon.</span>")
					con.connected = src
				else
					user.show_message("<span style=\"color:orange\">There is nothing here to connect to the siphon.</span>")
			...
			if (istype(W, /obj/item/weapon/wrench))
				src.alterable = !( src.alterable )
				if (src.alterable)
					boutput(user, "<span style=\"color:orange\">You unlock the interface!</span>")
				else
					boutput(user, "<span style=\"color:orange\">You lock the interface!</span>")
	return
