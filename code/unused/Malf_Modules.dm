	...
	for(var/datum/game_mode/malfunction/AI_Module/small/overload_machine/overload in usr:current_modules)
		if(overload.uses > 0)
			...
			for(var/mob/V in hearers(7, M))
				V.show_message(text("<span style=\"color:orange\">You hear a loud electrical buzzing sound!</span>"))
			spawn(50)
		...
		if(camera.uses > 0)
			if(!C.status)
				...
				for(var/mob/V in hearers(3, C))
					V.show_message(text("<span style=\"color:orange\">You hear a quiet click.</span>"))
			else
