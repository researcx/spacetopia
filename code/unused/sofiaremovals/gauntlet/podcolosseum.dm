...
/obj/screen/colosseumHelp
	...
	clicked(params)
		..()
		boutput(usr, "<span style=\"color:orange\">Press Page Down (or C in WASD mode) to fire your primary weapon.</span>")
		boutput(usr, "<span style=\"color:orange\">Press Page Up (or E in WASD mode) to fire your secondary weapon.</span>")
		boutput(usr, "<span style=\"color:orange\">Press Insert (or Q in WASD mode) to stop the ship.</span>")
		boutput(usr, "<span style=\"color:orange\">Click the ship to get out.</span>")
...
/obj/machinery/colosseum_putt
	...
	attack_hand(mob/user)
		...
		if (!owner)
			if (user.ckey in colosseum_controller.pods_claimed)
				...
				user.set_loc(src)
				boutput(user, "<span style=\"color:orange\">You claim the Colosseum Putt. Get ready to fight!</span>")
			...
			boutput(user, "<span style=\"color:orange\">You board your Colosseum Putt.</span>")
			user.set_loc(src)
			piloting = user
			on_board(user)
		else
	...
	Click(location, control, params)
		...
		if (usr in src.contents)
			usr.set_loc(src.loc)
			boutput(usr, "<span style=\"color:orange\">You exit the Colosseum Putt.</span>")
			piloting = null
			on_exit(usr)
	...
	proc/message_pilot(var/message, var/type = 0)
		if (piloting)
			switch (type)
				if (0)
					boutput(piloting, "<span style=\"color:orange\">[message]</span>")
				if (1)
	...
	attackby(obj/item/W as obj, mob/living/user as mob)
		if (istype(W, /obj/item/weldingtool))
			...
			if (T.welding && T.get_fuel() >= 1)
				T.use_fuel(1)
				visible_message("<span style=\"color:orange\"><b>[user]</b> repairs some dents on [src]!</span>")
