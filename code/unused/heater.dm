	...
	if (istype(W, /obj/item/weapon/tank))
		...
		if (istype(W, /obj/item/weapon/wrench))
			...
			if (src.c_status)
				...
				src.c_status = 0
				user.show_message("<span style=\"color:orange\">You have disconnected the heater.</span>", 1)
				if(con)
					con.connected = null
			else
				if (con && !con.connected)
					src.anchored = 1
					src.c_status = 3
					user.show_message("<span style=\"color:orange\">You have connected the heater.</span>", 1)
					con.connected = src
				else
					user.show_message("<span style=\"color:orange\">There is no connector here to attach the heater to.</span>", 1)
	return
