...
/mob/Login()
	...
	src << browse(null, "window=mapwindow.titlescreen") //closes the player setup window
	winshow(src, "mapwindow.titlescreen", "show=0")
	winshow(src, "window=mapwindow.titlescreen", "show=0")
...
//
	var/t = "<span style=\"color:orange\">Coordinates: [x],[y]<br></span>"
	t+= "<span style=\"color:red\">Temperature: [environment.temperature]<br></span>"
	t+= "<span style=\"color:orange\">Nitrogen: [environment.nitrogen]<br></span>"
	t+= "<span style=\"color:orange\">Oxygen: [environment.oxygen]<br></span>"
	t+= "<span style=\"color:orange\">Plasma : [environment.toxins]<br></span>"
	t+= "<span style=\"color:orange\">Carbon Dioxide: [environment.carbon_dioxide]<br></span>"
	if (environment.trace_gases)
		for(var/datum/gas/trace_gas in environment.trace_gases)
			boutput(usr, "<span style=\"color:orange\">[trace_gas.type]: [trace_gas.moles]<br></span>")
	else
		boutput(usr, "<span style=\"color:orange\">No trace gases.<br></span>")
...
/mob/verb/help()
	...
	//boutput(src, "<span style=\"color:orange\">Please visit the goonstation wiki at <b>http://wiki.ss13.co</b> for more indepth help.</span>")
	return
...
/mob/verb/abandon_mob()
	...
	if ((src.stat != 2 || !( ticker )))
		boutput(usr, "<span style=\"color:orange\"><B>You must be dead to use this!</B></span>")
		return

	logTheThing("diary", usr, null, "used abandon mob.", "game")

	//boutput(usr, "<span style=\"color:orange\"><B>Please roleplay correctly!</B></span>")
...
/mob/verb/togglelocaldeadchat()
	...
	boutput(usr, "<span style=\"color:orange\">[usr.client.local_deadchat ? "Now" : "No longer"] hearing local chat only.</span>")
...
//or if they are being drained of blood
	...
	if (src.health < 0)
		boutput(src, "<span style=\"color:orange\">You have given up life and succumbed to death.</span>")
		src.death()
		if (!src.suiciding)
...
/mob/Stat()
	...
	if (client.statpanel == "Status")
		...
		if (src.client.holder)
			...
			stat("Server Load:", "[world.cpu < 90 ? "No" : "Yes"]")
	...
	/*if (is_near_gauntlet()) colosseum and gauntlet removal
...
/* ghost emotes wooo also the logging is already taken care of in the emote() procs vOv
	if (istype(src,/mob/living/) && src.stat == 0)
		...
		boutput(src, "<span style=\"color:orange\">You can't emote when you're dead! How would that even work!?</span>")
*/
...
/mob/living/carbon/human/proc/JobEquipSpawned(rank)
	...
	if (JOB.slot_socks)
		src.equip_if_possible(new JOB.slot_socks(src), slot_socks)
	if (JOB.slot_underwear)
		src.equip_if_possible(new JOB.slot_underwear(src), slot_underwear)
	if (JOB.slot_bottom)
		src.equip_if_possible(new JOB.slot_bottom(src), slot_bottom)
	if (JOB.slot_suit)
...
/mob/verb/listen_ooc()
	...
	if (src.client)
		...
		if (src.client.preferences.listen_ooc)
			boutput(src, "<span style=\"color:orange\">You are now listening to messages on the OOC channel.</span>")
		else
			boutput(src, "<span style=\"color:orange\">You are no longer listening to messages on the OOC channel.</span>")
...
/mob/verb/ooc(msg as text)
	...
	for (var/client/C)
		...
		var ooc_class = src.client.api["username"]
		...
		var/rendered = "<span class=\"[ooc_class]\"><span class=\"prefix\">OOC:</span> <span class=\"name\" data-ctx='\ref[src.mind]'>[display_name]:</span> <span class=\"message\">[msg]</span></span>"
...
/mob/verb/listen_looc()
	...
	if (src.client)
		...
		if (src.client.preferences.listen_looc)
			boutput(src, "<span style=\"color:orange\">You are now listening to messages on the LOOC channel.</span>")
		else
			boutput(src, "<span style=\"color:orange\">You are no longer listening to messages on the LOOC channel.</span>")
...
/mob/proc/take_ear_damage(var/amount, var/tempdeaf = 0)
	...
	if (eardeaf != 0)
		...
		if (src.ear_deaf == 0 && deaf_bypass == 0 && suppress_message == 0)
			boutput(src, "<span style=\"color:orange\">The ringing in your ears subsides enough to let you hear again.</span>")
