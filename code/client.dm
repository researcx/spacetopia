...
/client/New()
	...
	if (isbanned)
		...
		if (announce_banlogin) message_admins("<span style=\"color:orange\">Failed Login: <a href='?src=%admin_ref%;action=notes;target=[src.ckey]'>[src]</a> - Banned (IP: [src.address], ID: [src.computer_id])</span>")
		spawn(0)
			var/banstring = {"
								<!doctype html>
								<html>
									...
									<body>
										...
										If you believe you were unjustly banned, head to <a href=\"https://spaceptopia.pw\">IRC</a> and request an unban.
									</body>
								</html>
							"}
...
/*
	spawn(rand(4,18))
		if(proxy_check(src.address))
			...
			if (announce_banlogin) message_admins("<span style=\"color:orange\">Failed Login: [src] - Using a Tor Proxy Exit Node (IP: [src.address], ID: [src.computer_id])</span>")
...
/client/verb/toggle_between_wasd_zqsd()
	...
	boutput(src, "<span style=\"color:orange\">[src.use_azerty ? "ZQSD" : "WASD"] hotkey layout enabled.</span>")
...
/client/verb/togglewasd()
	...
	if (current == "macro" || current == "zqsd")
		...
		boutput(src, "<span style=\"color:orange\">WASD hotkeys enabled.</span>")
	else
		winset(src, "mainwindow", "macro=macro")
		boutput(src, "<span style=\"color:orange\">WASD hotkeys disabled.</span>")
	src.install_macros()
...
/client/verb/togglezqsd()
	...
	if (current == "macro" || current == "wasd")
		...
		boutput(src, "<span style=\"color:orange\">ZQSD hotkeys enabled.</span>")
	else
		winset(src, "mainwindow", "macro=macro")
		boutput(src, "<span style=\"color:orange\">ZQSD hotkeys disabled.</span>")
	src.install_macros()
...
/client/verb/toggletogglewasd()
	...
	if (current == "true")
		...
		boutput(usr, "<span style=\"color:orange\">Tab now toggles WASD/ZQSD.</span>")
	else
		winset(usr, "macro.togglewasd", "is-disabled=true")
		winset(usr, "wasd.togglewasd", "is-disabled=true")
		boutput(usr, "<span style=\"color:orange\">Tab no longer toggles WASD/ZQSD.</span>")
			...
			if (src.mob.restrained())
				for(var/mob/M in range(src.mob, 1))
					if (((M.pulling == src.mob && (!( M.restrained() ) && M.stat == 0)) || locate(/obj/item/grab, src.mob.grabbed_by.len)))
						boutput(src, "<span style=\"color:orange\">You're restrained! You can't move!</span>")
						return 0
			src.moving = 1
...
/client/Click(atom/object, location, control, params)
	...
	//examine code by erika dont steal
	/*var/output = "This is \an [object.name]."
	//world.log << "output var defined"

	if (isitem(object) && object.blood_DNA)
		output = "<span style=\"color:red\">This is a bloody [object.name].</span>"
		if (object.desc)
			output += "<br>[object.desc] It seems to be covered in blood."
	else if (object.desc)
		output += "<br>[object.desc]"

	var/dist = get_dist(object, usr)
	if (istype(usr, /mob/dead/target_observer))
		dist = get_dist(object, usr:target)
	var/extra = object.get_desc(dist, usr)
	if (extra)
		output += " [extra]"

	world.log << params
	//parameters["alt"]
	//world.log << "passed all checks, now to output"
	if (output && parameters["alt"])
		//world.log << "attempting to show tooltip"
		//world.log << output
		var/objx = 1
		var/objy = 30
		var/testparams = "icon-x=[objx];icon-y=[objy];screen-loc=[objx],[objy]"
		usr.client.tooltip.show(object, testparams, title = object.name, content = output)*/
...
/client/Topic(href, href_list)
	...
	switch(href_list["action"])
		if ("priv_msg_irc")
			...
			if (!( t ))
				return
			boutput(usr, "<span style=\"color:orange\" class=\"bigPM\">Admin PM to-<b>[target] (IRC)</b>: [t]</span>")
		...
		if ("priv_msg")
			if(M)
				...
				if (usr.client.holder)
					...
					boutput(usr, "<span style=\"color:orange\" class=\"bigPM\">Admin PM to-<b>[key_name(M, 0, 0)][(M.real_name ? "/"+M.real_name : "")] <A HREF='?src=\ref[usr.client.holder];action=adminplayeropts;targetckey=[M.ckey]' class='popt'><i class='icon-info-sign'></i></A></b>: [t]</span>")
				else
					if (M.client && M.client.holder)
						boutput(M, "<span style=\"color:orange\" class=\"bigPM\">Reply PM from-<b>[key_name(usr, 0, 0)][(usr.real_name ? "/"+usr.real_name : "")] <A HREF='?src=\ref[M.client.holder];action=adminplayeropts;targetckey=[usr.ckey]' class='popt'><i class='icon-info-sign'></i></A></b>: [t]</span>")
					else
						boutput(M, "<span style=\"color:red\" class=\"bigPM\">Reply PM from-<b>[key_name(usr, 0, 0)]</b>: [t]</span>")
					boutput(usr, "<span style=\"color:orange\" class=\"bigPM\">Reply PM to-<b>[key_name(M, 0, 0)]</b>: [t]</span>")
		...
		if("resourcePreloadComplete")
			bout(src, "<span style='color:orange'><b>Preload completed.</b></span>")
...
/client/verb/changes()
	...
	src << link("https://spacetopia.pw")

/client/verb/wiki()
	set category = "Commands"
	set name = "Wiki"
	set desc = "Open the Wiki in your browser"
	src << link("https://wiki.spacetopia.pw")

/client/verb/discord()
	set category = "Commands"
	set name = "Discord Server"
	set desc = "Opens the Spacetopia Discord"
	src << link("https://discord.gg/3znQRmg")
...
/client/verb/map()
	...
	else if (map_setting == "SPACETOPIA")
		src << link("https://wiki.spacetopia.pw/")
	else
